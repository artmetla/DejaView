...
DejaViewSqlParser::DejaViewSqlParser(
    SqlSource source,
...
bool DejaViewSqlParser::Next() {
  DEJAVIEW_CHECK(status_.ok());
  ...
  for (Token token = tokenizer_.Next();; token = tokenizer_.Next()) {
    ...
    if (token.IsTerminal()) {
      ...
      DEJAVIEW_DCHECK(token.str.empty());
      return false;
    }
    ...
    switch (state) {
      ...
      case State::kStmtStart:
        ...
        break;
      case State::kInclude:
        if (token.token_type == TK_DEJAVIEW) {
          state = State::kIncludeDejaView;
        } else {
          return ErrorAtToken(token,
                              "Use 'INCLUDE DEJAVIEW MODULE {include_key}'.");
        }
        break;
      case State::kIncludeDejaView:
        if (token.token_type == TK_MODULE) {
          return ParseIncludeDejaViewModule(*first_non_space_token);
        } else {
          return ErrorAtToken(token,
                              "Use 'INCLUDE DEJAVIEW MODULE {include_key}'.");
        }
      case State::kDrop:
        if (token.token_type == TK_DEJAVIEW) {
          state = State::kDropDejaView;
        } else {
          state = State::kPassthrough;
        }
        break;
      case State::kDropDejaView:
        if (token.token_type == TK_INDEX) {
          return ParseDropDejaViewIndex(*first_non_space_token);
        } else {
          return ErrorAtToken(token, "Only DejaView index can be dropped");
        }
      case State::kCreate:
        if (token.token_type == TK_TRIGGER) {
          ...
          return ErrorAtToken(
              token, "Creating triggers is not supported in DejaViewSQL.");
        }
        if (token.token_type == TK_DEJAVIEW) {
          state = State::kCreateDejaView;
      ...
      case State::kCreateOrReplace:
        state = token.token_type == TK_DEJAVIEW
                    ? State::kCreateOrReplaceDejaView
                    : State::kPassthrough;
        break;
      case State::kCreateOrReplaceDejaView:
      case State::kCreateDejaView:
        bool replace = state == State::kCreateOrReplaceDejaView;
        if (token.token_type == TK_FUNCTION) {
          return ParseCreateDejaViewFunction(replace, *first_non_space_token);
        }
        if (token.token_type == TK_TABLE) {
          return ParseCreateDejaViewTableOrView(replace, *first_non_space_token,
                                                TableOrView::kTable);
        }
        ...
        if (token.token_type == TK_MACRO) {
          return ParseCreateDejaViewMacro(replace);
        }
        if (token.token_type == TK_INDEX) {
          return ParseCreateDejaViewIndex(replace, *first_non_space_token);
        }
...
bool DejaViewSqlParser::ParseIncludeDejaViewModule(
...
bool DejaViewSqlParser::ParseCreateDejaViewTableOrView(
    bool replace,
    Token first_non_space_token,
...
bool DejaViewSqlParser::ParseCreateDejaViewIndex(bool replace,
                                                 Token first_non_space_token) {
  ...
  if (!token.IsTerminal()) {
    return ErrorAtToken(
        token,
        "Expected semicolon after columns list in CREATE DEJAVIEW INDEX.");
  }
...
bool DejaViewSqlParser::ParseDropDejaViewIndex(
  ...
  if (!token.IsTerminal()) {
    return ErrorAtToken(
        token, "Nothing is allowed after table name in DROP DEJAVIEW INDEX");
  }
  statement_sql_ = tokenizer_.Substr(first_non_space_token, token);
...
bool DejaViewSqlParser::ParseCreateDejaViewFunction(
    bool replace,
    Token first_non_space_token) {
...
bool DejaViewSqlParser::ParseCreateDejaViewMacro(bool replace) {
...
bool DejaViewSqlParser::ParseRawArguments(std::vector<RawArgument>& args) {
  enum TokenType {
    kIdOrRp,
    kId,
    kType,
  ...
  for (Token tok = tokenizer_.NextNonWhitespace();;
    ...
    if (expected == kCommaOrRp) {
      DEJAVIEW_CHECK(expected == kCommaOrRp);
...
bool DejaViewSqlParser::ParseArguments(
    std::vector<sql_argument::ArgumentDefinition>& args) {
  std::vector<RawArgument> raw_args;
  if (!ParseRawArguments(raw_args)) {
    return false;
...
DejaViewSqlParser::ResolveRawArgument(RawArgument arg) {
...
bool DejaViewSqlParser::ErrorAtToken(const SqliteTokenizer::Token& token,
                                     const char* error,
                                     ...) {
