...
CREATE DEJAVIEW VIEW thread_state_extended AS
SELECT
  ts,
...
CREATE DEJAVIEW TABLE launch_thread_state_io_wait_dur_sum AS
...
CREATE DEJAVIEW VIEW launch_thread_state_dur_sum AS
...
CREATE OR REPLACE DEJAVIEW FUNCTION main_thread_time_for_launch_and_state(startup_id INT, state STRING)
RETURNS INT AS
SELECT SUM(dur)
...
CREATE OR REPLACE DEJAVIEW FUNCTION main_thread_time_for_launch_in_runnable_state(startup_id INT)
RETURNS INT AS
...
-- of time spent in that state by the main thread of the process being started up.
CREATE OR REPLACE DEJAVIEW FUNCTION main_thread_time_for_launch_state_and_io_wait(startup_id INT, state STRING, io_wait BOOL)
RETURNS INT AS
SELECT SUM(dur)
...
CREATE OR REPLACE DEJAVIEW FUNCTION thread_time_for_launch_state_and_thread(startup_id INT, state STRING, thread_name STRING)
RETURNS INT AS
SELECT SUM(dur)
FROM launch_thread_state_dur_sum l
...
-- startup process.
CREATE OR REPLACE DEJAVIEW FUNCTION time_to_running_state_for_launch(startup_id LONG)
RETURNS PROTO AS
