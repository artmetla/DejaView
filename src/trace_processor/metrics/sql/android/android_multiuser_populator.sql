...
CREATE DEJAVIEW VIEW multiuser_events AS
SELECT
...
CREATE DEJAVIEW VIEW multiuser_timing AS
SELECT
...
CREATE DEJAVIEW VIEW sp_sched AS
SELECT ts, dur, cpu, utid
FROM sched;
-- Get all the cpu frequency slices.
DROP VIEW IF EXISTS sp_frequency;
CREATE DEJAVIEW VIEW sp_frequency AS
SELECT
  ts,
...
CREATE DEJAVIEW VIEW cpu_usage_all AS
SELECT
...
CREATE DEJAVIEW VIEW cpu_usage AS
SELECT
  user_id,
  process_name,
...
CREATE DEJAVIEW VIEW {{output_table_name}} AS
