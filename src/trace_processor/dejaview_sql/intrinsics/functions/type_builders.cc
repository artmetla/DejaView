...
#include "src/trace_processor/dejaview_sql/intrinsics/types/array.h"
#include "src/trace_processor/dejaview_sql/intrinsics/types/counter.h"
#include "src/trace_processor/dejaview_sql/intrinsics/types/node.h"
#include "src/trace_processor/dejaview_sql/intrinsics/types/partitioned_intervals.h"
#include "src/trace_processor/dejaview_sql/intrinsics/types/row_dataframe.h"
#include "src/trace_processor/dejaview_sql/intrinsics/types/struct.h"
...
using Array = std::variant<dejaview_sql::IntArray,
                           dejaview_sql::DoubleArray,
                           dejaview_sql::StringArray>;
...
struct ArrayAgg : public SqliteAggregateFunction<ArrayAgg> {
  ...
  static void Step(sqlite3_context* ctx, int argc, sqlite3_value** argv) {
    DEJAVIEW_DCHECK(argc == kArgCount);
  ...
  static void Final(sqlite3_context* ctx) {
    ...
    switch (array.index()) {
      ...
      default:
        DEJAVIEW_FATAL("%zu is not a valid index", array.index());
    }
  }
};
...
struct NodeAgg : public SqliteAggregateFunction<NodeAgg> {
  ...
  static void Final(sqlite3_context* ctx) {
    ...
    auto nodes = std::make_unique<dejaview_sql::Graph>(
...
struct Struct : public SqliteFunction<Struct> {
  ...
  static void Step(sqlite3_context* ctx, int rargc, sqlite3_value** argv) {
    ...
    if (argc / 2 > dejaview_sql::Struct::kMaxFields) {
      return sqlite::utils::SetError(
          ctx, base::ErrStatus("STRUCT: only at most %d fields are supported",
                               dejaview_sql::Struct::kMaxFields));
    }

    auto s = std::make_unique<dejaview_sql::Struct>();
...
struct RowDataframeAgg : public SqliteAggregateFunction<Struct> {
  ...
  static constexpr int kArgCount = -1;
  struct AggCtx : SqliteAggregateContext<AggCtx> {
    dejaview_sql::RowDataframe dataframe;
  ...
  static void Final(sqlite3_context* ctx) {
    ...
    return sqlite::result::UniquePointer(
        ctx,
        std::make_unique<dejaview_sql::RowDataframe>(
...
struct IntervalTreeIntervalsAgg
  ...
  static void Step(sqlite3_context* ctx, int rargc, sqlite3_value** argv) {
    auto argc = static_cast<uint32_t>(rargc);
    DEJAVIEW_DCHECK(argc >= kMinArgCount);
    auto& agg_ctx = AggCtx::GetOrCreateContextForStep(ctx);
    ...
    dejaview_sql::Partition new_partition;
  ...
  static void Final(sqlite3_context* ctx) {
    ...
    return sqlite::result::UniquePointer(
        ctx,
        std::make_unique<dejaview_sql::PartitionedTable>(
            std::move(raw_agg_ctx.get()->partitions)),
        dejaview_sql::PartitionedTable::kName);
  }
};

struct CounterPerTrackAgg
    : public SqliteAggregateFunction<dejaview_sql::PartitionedCounter> {
  static constexpr char kName[] = "__intrinsic_counter_per_track_agg";
  static constexpr int kArgCount = 4;
  struct AggCtx : SqliteAggregateContext<AggCtx> {
    dejaview_sql::PartitionedCounter tracks;
  };
  ...
  static void Final(sqlite3_context* ctx) {
    ...
    return sqlite::result::UniquePointer(
        ctx,
        std::make_unique<dejaview_sql::PartitionedCounter>(
            std::move(raw_agg_ctx.get()->tracks)),
        dejaview_sql::PartitionedCounter::kName);
  }
};
