...
DEJAVIEW_CONFIG = struct(
    # This is used to refer to deps within dejaview's BUILD files.
    # In standalone and bazel-based embedders use '//', because dejaview has its
    # own repository, and //xxx would be relative to @dejaview//xxx.
    # In Google internal builds, instead, this is set to //third_party/dejaview,
    # because dejaview doesn't have its own repository there.
    root = "//",

    # These variables map dependencies to dejaview third-party projects. This is
    # to allow dejaview embedders (e.g. gapid) and google internal builds to
    ...
    deps = struct(
        ...
        # dejaview_cc_library target. It needs to expose a
        # "dejaview_build_flags.h" file that can be included via:
        # #include "dejaview_build_flags.h".
        build_config = ["//:build_config_hdr"],

        # Target exposing the DEJAVIEW_VERSION_STRING() and
        # DEJAVIEW_VERSION_SCM_REVISION() macros. This is overridden in google
        # internal builds.
        version_header = ["//:cc_dejaview_version_header"],

        # Target exposing platform-specific functionality for base. This is
        # overriden in Google internal builds.
        base_platform = ["//:dejaview_base_default_platform"],

        zlib = ["@dejaview_dep_zlib//:zlib"],
        expat = ["@dejaview_dep_expat//:expat"],
        jsoncpp = ["@dejaview_dep_jsoncpp//:jsoncpp"],
        linenoise = ["@dejaview_dep_linenoise//:linenoise"],
        sqlite = ["@dejaview_dep_sqlite//:sqlite"],
        sqlite_ext_percentile = ["@dejaview_dep_sqlite_src//:percentile_ext"],
        ...
        #     here and DEJAVIEW_LLVM_DEMANGLE to false in your
        #     dejaview_build_flags.h. Then the implementation will use a
        ...
        llvm_demangle = ["@dejaview_dep_llvm_demangle//:llvm_demangle"],
    ),

    # This struct allows embedders to customize the cc_opts for DejaView
    # 3rd party dependencies. They only have an effect if the dependencies are
    # initialized with the DejaView build files (i.e. via dejaview_deps()).
    deps_copts = struct(
        zlib = [],
        expat = [],
        jsoncpp = [],
    ...
    # to rules like cc_binary. Prefixed rules (e.g. dejaview_cc_binary) will
