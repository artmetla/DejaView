...
INCLUDE DEJAVIEW MODULE wattson.curves.device;
INCLUDE DEJAVIEW MODULE wattson.device_infos;

-- 1D LUT
CREATE DEJAVIEW TABLE _filtered_curves_1d_raw AS
...
CREATE DEJAVIEW TABLE _filtered_curves_1d AS
...
CREATE DEJAVIEW INDEX freq_1d ON _filtered_curves_1d(policy, freq_khz, idle);

-- 2D LUT; with dependency on another CPU
CREATE DEJAVIEW TABLE _filtered_curves_2d_raw AS
SELECT
...
CREATE DEJAVIEW TABLE _filtered_curves_2d AS
...
CREATE DEJAVIEW INDEX freq_2d
ON _filtered_curves_2d(freq_khz, other_policy, other_freq_khz, idle);

-- L3 cache LUT
CREATE DEJAVIEW TABLE _filtered_curves_l3_raw AS
SELECT
  cp.policy as other_policy,
  dc.freq_khz,
  dc.other_freq_khz,
...
CREATE DEJAVIEW TABLE _filtered_curves_l3 AS
SELECT
