...
template <typename T>
DEJAVIEW_ALWAYS_INLINE void FillMessage_Simple(T* msg) {
...
template <typename T>
DEJAVIEW_ALWAYS_INLINE void FillMessage_Nested(T* msg, int depth = 0) {
  benchmark::DoNotOptimize(g_fake_input_simple);
...
DEJAVIEW_ALWAYS_INLINE void Clobber(benchmark::State& state) {
