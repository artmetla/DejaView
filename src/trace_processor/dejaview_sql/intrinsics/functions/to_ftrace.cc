...
#include "protos/dejaview/trace/ftrace/cgroup.pbzero.h"
#include "protos/dejaview/trace/ftrace/clk.pbzero.h"
#include "protos/dejaview/trace/ftrace/dpu.pbzero.h"
#include "protos/dejaview/trace/ftrace/filemap.pbzero.h"
 ...
 private:
  ...
  void WriteKernelFnValue(const Variadic& value) {
    if (value.type == Variadic::Type::kUint) {
      ...
      DEJAVIEW_DFATAL("Invalid field type %d", static_cast<int>(value.type));
    }
  }
  ...
  std::optional<uint32_t> FieldIdToRow(uint32_t field_id) {
    DEJAVIEW_DCHECK(field_id > 0);
    DEJAVIEW_DCHECK(field_id < field_id_to_arg_index_->size());
...
void ArgsSerializer::SerializeArgs() {
  ...
  if (event_name_ == "sched_switch") {
    ...
    WriteArgForField(SS::kPrevStateFieldNumber, [this](const Variadic& value) {
      DEJAVIEW_DCHECK(value.type == Variadic::Type::kInt);
    ...
    WriteValueForField(BT::kFlagsFieldNumber, [this](const Variadic& value) {
      DEJAVIEW_DCHECK(value.type == Variadic::Type::kUint);
      writer_->AppendHexInt(value.uint_value);
    });
    ...
    WriteValueForField(P::kBufFieldNumber, [this](const Variadic& value) {
      DEJAVIEW_DCHECK(value.type == Variadic::Type::kString);
...
void SystraceSerializer::SerializePrefix(uint32_t raw_row,
  ...
  if (DEJAVIEW_LIKELY(pid_chars < 5)) {
