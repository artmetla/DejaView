...
-- periods.
CREATE DEJAVIEW TABLE android_suspend_state(
  -- Timestamp
  ts INT,
  -- Duration
  dur INT,
...
FROM suspend_slice
ORDER BY ts; -- Order by will cause DejaView table to index by ts.
...
CREATE DEJAVIEW FUNCTION _extract_duration_without_suspend(
  -- Timestamp of event.
  ts INT,
  -- Duration of event.
  dur INT)
