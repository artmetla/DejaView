...
CREATE DEJAVIEW TABLE max_ts_per_process AS
...
CREATE DEJAVIEW VIEW original_rail_mode_slices AS
SELECT slice.id,
  slice.ts,
  CASE
...
CREATE DEJAVIEW VIEW trace_has_realistic_length AS
...
CREATE DEJAVIEW VIEW rail_mode_slices AS
...
CREATE DEJAVIEW VIEW overall_rail_mode_slices AS
SELECT s.ts,
  s.end_ts,
...
CREATE DEJAVIEW TABLE combined_overall_rail_slices AS
...
CREATE DEJAVIEW VIEW not_animating_slices AS
...
CREATE DEJAVIEW VIEW input_begin_end_slices AS
SELECT prefix,
...
CREATE DEJAVIEW VIEW unified_input_pair_increments AS
SELECT ts,
  scroll_increment
  + pinch_increment
  + touch_increment
...
CREATE DEJAVIEW VIEW initial_paired_increment AS
SELECT ts,
...
CREATE DEJAVIEW VIEW simple_input_slices AS
SELECT id,
  name,
  ts,
  dur
...
CREATE DEJAVIEW VIEW simple_input_increments AS
SELECT ts,
  1 AS increment
...
CREATE DEJAVIEW VIEW all_input_increments AS
SELECT ts,
...
CREATE DEJAVIEW VIEW all_input_totals AS
SELECT ts,
...
CREATE DEJAVIEW VIEW all_input_slices AS
...
CREATE DEJAVIEW VIEW rail_mode_animation_slices AS
...
CREATE DEJAVIEW VIEW has_modified_rail_slices AS
SELECT (
    SELECT value
...
CREATE DEJAVIEW VIEW unmerged_modified_rail_slices AS
SELECT ROW_NUMBER() OVER () AS id,
  ts,
  dur,
  mode
...
CREATE DEJAVIEW TABLE modified_rail_slices AS
