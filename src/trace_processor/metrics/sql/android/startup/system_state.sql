...
CREATE OR REPLACE DEJAVIEW FUNCTION dur_of_process_running_concurrent_to_launch(
  startup_id INT,
  process_glob STRING
)
RETURNS INT AS
...
CREATE OR REPLACE DEJAVIEW FUNCTION count_slices_concurrent_to_launch(startup_id INT, slice_glob STRING)
RETURNS INT AS
SELECT COUNT(1)
FROM slice
JOIN (
