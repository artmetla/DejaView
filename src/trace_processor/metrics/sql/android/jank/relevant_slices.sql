...
CREATE OR REPLACE DEJAVIEW FUNCTION vsync_from_name(slice_name STRING)
RETURNS STRING AS
SELECT CAST(STR_SPLIT($slice_name, " ", 1) AS INTEGER);

CREATE OR REPLACE DEJAVIEW FUNCTION gpu_completion_fence_id_from_name(slice_name STRING)
RETURNS STRING AS
SELECT
  CASE
    WHEN
...
CREATE DEJAVIEW TABLE android_jank_cuj_do_frame_slice AS
SELECT
  cuj.cuj_id,
  main_thread.upid,
  main_thread.utid,
...
CREATE DEJAVIEW TABLE android_jank_cuj_draw_frame_slice AS
SELECT
  cuj_id,
  render_thread.upid,
  render_thread.utid,
...
CREATE DEJAVIEW TABLE android_jank_cuj_gpu_completion_fence AS
SELECT
  cuj_id,
  vsync,
...
CREATE DEJAVIEW TABLE android_jank_cuj_hwc_release_fence AS
SELECT
  cuj_id,
  vsync,
  draw_frame.id AS draw_frame_slice_id,
...
CREATE DEJAVIEW TABLE android_jank_cuj_hwc_release_slice AS
SELECT
  fence.cuj_id,
  vsync,
  slice.*,
...
CREATE DEJAVIEW TABLE android_jank_cuj_gpu_completion_slice AS
SELECT
  fence.cuj_id,
  vsync,
  slice.*,
...
CREATE DEJAVIEW TABLE android_jank_cuj_app_to_sf_match AS
SELECT
  cuj_id,
...
CREATE OR REPLACE DEJAVIEW FUNCTION find_android_jank_cuj_sf_main_thread_slice(
...
CREATE DEJAVIEW TABLE android_jank_cuj_sf_commit_slice AS
SELECT * FROM FIND_ANDROID_JANK_CUJ_SF_MAIN_THREAD_SLICE('commit *');

DROP TABLE IF EXISTS android_jank_cuj_sf_composite_slice;
CREATE DEJAVIEW TABLE android_jank_cuj_sf_composite_slice AS
...
CREATE DEJAVIEW TABLE android_jank_cuj_sf_on_message_invalidate_slice AS
...
CREATE DEJAVIEW VIEW android_jank_cuj_sf_root_slice AS
...
CREATE DEJAVIEW TABLE android_jank_cuj_sf_gpu_completion_fence AS
SELECT
  cuj_id,
  vsync,
...
CREATE DEJAVIEW TABLE android_jank_cuj_sf_gpu_completion_slice AS
SELECT
  fence.cuj_id,
  vsync,
  slice.*,
...
CREATE DEJAVIEW TABLE android_jank_cuj_sf_draw_layers_slice AS
