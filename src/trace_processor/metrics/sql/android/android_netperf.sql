...
CREATE DEJAVIEW VIEW rx_packets AS
SELECT
  ts,
...
CREATE DEJAVIEW VIEW gro_rx_packet_count AS
SELECT
  s.name AS dev,
  COUNT(1) AS cnt
FROM slice s
...
CREATE DEJAVIEW VIEW tx_packets AS
SELECT
  ts,
...
CREATE DEJAVIEW VIEW net_devices AS
SELECT DISTINCT dev
FROM tx_packets
UNION
SELECT DISTINCT dev
...
CREATE DEJAVIEW VIEW tcp_retransmitted_count AS
SELECT
  COUNT(1) AS cnt
FROM slice s
LEFT JOIN track t
...
CREATE DEJAVIEW VIEW kfree_skb_count AS
SELECT
  MAX(value) AS cnt
FROM counter c
LEFT JOIN track t
...
CREATE DEJAVIEW VIEW device_per_core_ingress_traffic AS
SELECT
  dev,
...
CREATE DEJAVIEW VIEW device_per_core_egress_traffic AS
SELECT
  dev,
  AndroidNetworkMetric_CorePacketStatistic(
    'id', cpu,
...
CREATE DEJAVIEW VIEW device_total_ingress_traffic AS
SELECT
  dev,
  MIN(ts) AS start_ts,
  MAX(ts) AS end_ts,
...
CREATE DEJAVIEW VIEW device_total_egress_traffic AS
SELECT
  dev,
  MIN(ts) AS start_ts,
  MAX(ts) AS end_ts,
...
CREATE DEJAVIEW VIEW device_traffic_statistic AS
SELECT
...
CREATE DEJAVIEW VIEW net_rx_actions AS
SELECT
  s.ts,
  s.dur,
...
CREATE DEJAVIEW VIEW net_tx_actions AS
SELECT
  s.ts,
  s.dur,
  CAST(SUBSTR(t.name, 13, 1) AS int) AS cpu
...
CREATE DEJAVIEW VIEW ipi_actions AS
SELECT
  s.ts,
  s.dur,
  CAST(SUBSTR(t.name, 13, 1) AS int) AS cpu
...
CREATE DEJAVIEW VIEW cpu_freq_view AS
SELECT
  cpu,
  ts,
  LEAD(ts, 1, trace_end())
...
CREATE DEJAVIEW VIEW total_net_rx_action_statistic AS
SELECT
  COUNT(1) AS times,
  SUM(dur) AS runtime,
...
CREATE DEJAVIEW VIEW total_net_tx_action_statistic AS
SELECT
  COUNT(1) AS times,
  SUM(dur) AS runtime,
  AVG(dur) AS avg_runtime
...
CREATE DEJAVIEW VIEW total_ipi_action_statistic AS
SELECT
  COUNT(1) AS times,
  SUM(dur) AS runtime,
  AVG(dur) AS avg_runtime
...
CREATE DEJAVIEW VIEW activated_cores_net_rx AS
SELECT DISTINCT
  cpu
FROM net_rx_actions;

DROP VIEW IF EXISTS activated_cores_net_tx;
CREATE DEJAVIEW VIEW activated_cores_net_tx AS
SELECT DISTINCT
  cpu
FROM net_tx_actions;

DROP VIEW IF EXISTS per_core_net_rx_action_statistic;
CREATE DEJAVIEW VIEW per_core_net_rx_action_statistic AS
SELECT
...
CREATE DEJAVIEW VIEW per_core_net_tx_action_statistic AS
SELECT
...
CREATE DEJAVIEW VIEW android_netperf_output AS
