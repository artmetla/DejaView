...
-- Raw ftrace events
CREATE DEJAVIEW TABLE _raw_dmabuf_events AS
SELECT
...
CREATE DEJAVIEW TABLE _gralloc_binders AS
...
CREATE DEJAVIEW TABLE _attributed_dmabufs AS
SELECT
  r.inode,
  r.ts,
  r.buf_size,
...
CREATE DEJAVIEW FUNCTION _alloc_source(is_alloc BOOL, inode INT, ts INT)
RETURNS INT AS
SELECT attr_utid
...
CREATE DEJAVIEW TABLE android_dmabuf_allocs (
