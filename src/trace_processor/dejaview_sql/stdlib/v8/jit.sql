...
-- engine.
CREATE DEJAVIEW VIEW v8_isolate(
...
CREATE DEJAVIEW VIEW v8_js_script (
...
CREATE DEJAVIEW VIEW v8_wasm_script (
...
CREATE DEJAVIEW VIEW v8_js_function (
...
CREATE DEJAVIEW VIEW _v8_js_code(
  -- Unique id
  id UINT,
...
-- TODO(carlscab): Make public once `_jit_code` is public too
CREATE DEJAVIEW VIEW _v8_internal_code(
  -- Unique id
  id UINT,
...
-- TODO(carlscab): Make public once `_jit_code` is public too
CREATE DEJAVIEW VIEW _v8_wasm_code(
  -- Unique id
  id UINT,
  -- Associated jit code. Joinable with `_jit_code.jit_code_id`.
  jit_code_id UINT,
...
-- TODO(carlscab): Make public once `_jit_code` is public too
CREATE DEJAVIEW VIEW _v8_regexp_code(
  -- Unique id
  id UINT,
  -- Associated jit code. Joinable with `_jit_code.jit_code_id`.
  jit_code_id UINT,
