...
const std::string& LineInfoReader::GetExpandedFilename(size_t index) {
  if (index >= filenames_.size()) {
    QEMU_LOG() << "filename index out of range\n";
    exit(1);
  }
...
void LineInfoReader::SeekToOffset(uint64_t offset, uint8_t address_size) {
  ...
  if (sizes_.dwarf_version() == 4) {
    ...
    if (params_.maximum_operations_per_instruction == 0) {
      QEMU_LOG() << "DWARF line info had maximum_operations_per_instruction=0\n";
      exit(1);
    }
  } else {
  ...
  if (params_.line_range == 0) {
    QEMU_LOG() << "line_range of zero will cause divide by zero\n";
    exit(1);
  }
  ...
  while (true) {
    ...
    if (file_name.directory_index >= include_directories_.size()) {
      QEMU_LOG() << "directory index out of range\n";
      exit(1);
    }
...
bool LineInfoReader::ReadLineInfo() {
  ...
  while (true) {
    ...
    if (op >= params_.opcode_base) {
      ...
      info_.line +=
          static_cast<uint32_t>(params_.line_base + (AdjustedOpcode(op) % params_.line_range));
      if (!shadow_) {
        remaining_ = data;
        return true;
      }
      ...
      switch (op) {
        case DW_LNS_extended_op: {
          ...
          switch (extended_op) {
            ...
            default:
              ...
              SkipBytes(len, &data);
              fprintf(stderr,
                      "warning: unknown DWARF line table extended "
                      "opcode: %d\n",
                      extended_op);
              break;
          }
        ...
        case DW_LNS_advance_line:
          info_.line += static_cast<uint32_t>(ReadLEB128<int32_t>(&data));
          break;
        case DW_LNS_set_file:
          info_.file = ReadLEB128<uint32_t>(&data);
          if (info_.file >= filenames_.size()) {
            QEMU_LOG() << "filename index too big\n";
            exit(1);
          }
          break;
        ...
        default:
          ...
          fprintf(stderr,
                  "warning: unknown DWARF line table opcode: %d\n",
                  op);
          break;
      }
    }
