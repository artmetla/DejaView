...
const PACKAGE_LOOKUP = `
  create or replace dejaview table package_name_lookup as
  with installed as (
  ...
  create or replace dejaview macro add_package_name(src TableOrSubquery) returns TableOrSubquery as (
...
const RADIO_TRANSPORT_TYPE = `
  create or replace dejaview view radio_transport_data_conn as
  ...
  create or replace dejaview view radio_transport_nr_state as
  ...
  create or replace dejaview view radio_transport as
  select
    ts, dur,
    case data_conn_val
...
const NETWORK_SUMMARY = `
  create or replace dejaview table network_summary as
  with base as (
      select
...
const MODEM_RIL_CHANNELS_PREAMBLE = `
  CREATE OR REPLACE DEJAVIEW FUNCTION EXTRACT_KEY_VALUE(source STRING, key_name STRING) RETURNS STRING AS
...
const KERNEL_WAKELOCKS = `
  create or replace dejaview table kernel_wakelocks as
...
const HIGH_CPU = `
  create or replace dejaview table high_cpu as
...
const BT_ACTIVITY = `
  create dejaview table bt_activity as
  with step1 as (
    select
...
class AndroidLongBatteryTracing implements DejaViewPlugin {
  ...
  async addDeviceState(ctx: Trace, features: Set<string>): Promise<void> {
    ...
    const e = ctx.engine;
    await e.query(`INCLUDE DEJAVIEW MODULE android.battery_stats;`);
    await e.query(`INCLUDE DEJAVIEW MODULE android.suspend;`);
    await e.query(`INCLUDE DEJAVIEW MODULE counters.intervals;`);
  ...
  async addNetworkSummary(ctx: Trace, features: Set<string>): Promise<void> {
    ...
    const e = ctx.engine;
    await e.query(`INCLUDE DEJAVIEW MODULE android.battery_stats;`);
    await e.query(`INCLUDE DEJAVIEW MODULE android.network_packets;`);
...
export const plugin: PluginDescriptor = {
  pluginId: 'dev.dejaview.AndroidLongBatteryTracing',
