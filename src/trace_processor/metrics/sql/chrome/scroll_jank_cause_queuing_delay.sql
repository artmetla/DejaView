...
CREATE DEJAVIEW VIEW blocking_tasks_no_threadcontroller_active AS
SELECT
  slice.*,
...
CREATE DEJAVIEW TABLE chrome_annotated_threads_and_processes AS
SELECT
...
CREATE DEJAVIEW TABLE blocking_chrome_tasks_without_threadpool AS
SELECT
  slice.*,
...
CREATE DEJAVIEW TABLE blocking_tasks_queuing_delay AS
SELECT
...
CREATE DEJAVIEW VIEW all_descendant_blocking_tasks_queuing_delay AS
SELECT
...
CREATE DEJAVIEW TABLE all_descendant_blocking_tasks_queuing_delay_with_cpu_time AS
SELECT
...
CREATE DEJAVIEW VIEW counted_descendant_blocking_tasks_queuing_delay AS
SELECT
  base.*,
...
CREATE DEJAVIEW VIEW blocking_tasks_queuing_delay_with_invalid_depth AS
SELECT
  base.*,
  (
...
CREATE DEJAVIEW VIEW descendant_blocking_tasks_queuing_delay AS
SELECT
  id,
  ts,
  dur,
...
-- otherwise.
CREATE OR REPLACE DEJAVIEW FUNCTION get_first_slice_name_or_null(name STRING)
...
CREATE OR REPLACE DEJAVIEW FUNCTION get_java_slice_summary_or_null(name STRING)
...
CREATE OR REPLACE DEJAVIEW FUNCTION unknown_event_or_empty_string(name STRING,
                                                   cat STRING,
...
CREATE OR REPLACE DEJAVIEW FUNCTION top_level_name(name STRING, function STRING, file STRING)
RETURNS STRING AS
...
CREATE DEJAVIEW VIEW scroll_jank_cause_queuing_delay_temp AS
SELECT
...
CREATE DEJAVIEW VIEW scroll_jank_cause_queuing_delay_average_no_jank_time AS
SELECT
  location,
...
CREATE DEJAVIEW VIEW scroll_jank_cause_queuing_delay_average_no_jank_time_restricted AS
SELECT
  restricted_location,
...
CREATE DEJAVIEW VIEW scroll_jank_cause_queuing_delay_unannotated AS
SELECT
  base.*,
...
CREATE DEJAVIEW VIEW scroll_jank_cause_queuing_delay AS
SELECT
  base.*,
  'QueuingDelay.'
  || CASE WHEN jank THEN 'Jank' ELSE 'NoJank' END || '.BlockingTasksUs.'
