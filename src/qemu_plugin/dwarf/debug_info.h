...
//         });
//       } else {
//         die_reader.Children(cu, abbrev);
//       }
//     }
//   }

#ifndef SRC_QEMU_PLUGIN_DWARF_DEBUG_INFO_H_
#define SRC_QEMU_PLUGIN_DWARF_DEBUG_INFO_H_
...
class CompilationUnitSizes {
 public:
  ...
  uint16_t dwarf_version() const { return dwarf_version_; }

  void SetAddressSize(uint8_t address_size) {
    if (address_size != 4 && address_size != 8) {
      QEMU_LOG() << "Unexpected address size: " << address_size << "\n";
      exit(1);
    }
...
class DIEReader {
 public:
  ...
  void ReadChildren(const CU& cu, const AbbrevTable::Abbrev* code, std::function<void(const AbbrevTable::Abbrev *)> f);
...
#endif  // SRC_QEMU_PLUGIN_DWARF_DEBUG_INFO_H_
