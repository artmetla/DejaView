...
CREATE DEJAVIEW VIEW android_statsd_atoms(
...
-- Information about DejaView triggers, extracted from statsd atoms, which
-- happened during the trace.
--
-- This requires the `android.statsd` data-source to be enabled and the
-- `ATOM_DEJAVIEW_TRIGGER` push atom to be configured.
CREATE DEJAVIEW TABLE _android_statsd_dejaview_triggers(
...
SELECT
  ts,
  extract_arg(arg_set_id, 'dejaview_trigger.trigger_name') AS trigger_name
FROM android_statsd_atoms
WHERE name = 'dejaview_trigger';
