...
CREATE OR REPLACE DEJAVIEW FUNCTION android_jank_cuj_table_set_slice(table_set STRING)
RETURNS STRING AS
SELECT slice_table_name
FROM android_jank_cuj_table_set ts
WHERE ts.name = $table_set;

CREATE OR REPLACE DEJAVIEW FUNCTION android_jank_cuj_table_set_frame_boundary(
  table_set STRING
)
RETURNS STRING AS
...
CREATE OR REPLACE DEJAVIEW FUNCTION android_jank_cuj_table_set_cuj_boundary(
  table_set STRING
)
RETURNS STRING AS
...
CREATE OR REPLACE DEJAVIEW FUNCTION android_jank_cuj_table_set_frame(table_set STRING)
RETURNS STRING AS
...
CREATE OR REPLACE DEJAVIEW FUNCTION android_jank_cuj_slice_overlaps(ts LONG,
                                                        dur LONG,
                                                        ts_second LONG,
                                                        dur_second LONG)
RETURNS BOOL AS
