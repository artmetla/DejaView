...
CREATE DEJAVIEW MACRO _heap_graph_type_path_hash(tab TableOrSubquery)
...
CREATE DEJAVIEW MACRO _heap_graph_path_hash_aggregate(tab TableOrSubquery)
RETURNS TableOrSubquery
AS (
  with x AS (
    SELECT
...
--
-- Note that |tab| *must* be a DejaView (e.g. not a subquery) for this macro
-- to work.
CREATE DEJAVIEW MACRO _heap_graph_path_hashes_to_class_tree(tab TableOrSubquery)
RETURNS TableOrSubquery
AS (
  SELECT
    graph_sample_ts,
