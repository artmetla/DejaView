...
std::string OpToString(int op) {
  switch (op) {
    ...
    case SQLITE_INDEX_CONSTRAINT_ISNOTNULL:
      ...
      return " is not ";
    default:
      DEJAVIEW_FATAL("Operator to string conversion not impemented for %d", op);
  }
}
...
std::string EscapedSqliteValueAsString(sqlite3_value* value) {
  switch (sqlite3_value_type(value)) {
    ...
    case SQLITE_NULL:
      return " null";
    default:
      DEJAVIEW_FATAL("Unknown value type %d", sqlite3_value_type(value));
  }
}
...
base::Status SpanJoinOperatorModule::TableDefinition::Create(
  ...
  DEJAVIEW_DCHECK(ts_idx < cols.size());
  DEJAVIEW_DCHECK(dur_idx < cols.size());
...
SpanJoinOperatorModule::Query::Query(SpanJoinOperatorModule::State* state,
  ...
  DEJAVIEW_DCHECK(!defn_->IsPartitioned() ||
...
base::Status SpanJoinOperatorModule::Query::NextSliceState() {
  switch (state_) {
    ...
    case State::kEof: {
      DEJAVIEW_DFATAL("Called Next when EOF");
...
int SpanJoinOperatorModule::Filter(sqlite3_vtab_cursor* cursor,
                                   ...
                                   int,
                                   sqlite3_value** argv) {
  DEJAVIEW_TP_TRACE(metatrace::Category::QUERY_DETAILED, "SPAN_JOIN_XFILTER");
...
int SpanJoinOperatorModule::Column(sqlite3_vtab_cursor* cursor,
  ...
  DEJAVIEW_DCHECK(c->t1.IsReal() || c->t2.IsReal());
