...
#ifndef INCLUDE_DEJAVIEW_TRACING_TRACK_EVENT_CATEGORY_REGISTRY_H_
#define INCLUDE_DEJAVIEW_TRACING_TRACK_EVENT_CATEGORY_REGISTRY_H_
...
// DEJAVIEW_DEFINE_CATEGORIES for registering your own categories.
struct DEJAVIEW_EXPORT_COMPONENT Category {
...
class DEJAVIEW_EXPORT_COMPONENT DynamicCategory final {
 public:
...
// DEJAVIEW_DEFINE_CATEGORIES for building the registry.
class DEJAVIEW_EXPORT_COMPONENT TrackEventCategoryRegistry {
 public:
  ...
  const Category* GetCategory(size_t index) const {
    DEJAVIEW_DCHECK(index < category_count_);
 ...
 private:
  ...
  static constexpr size_t CheckIsValidCategoryIndex(size_t index) {
    // Relies on DEJAVIEW_CHECK() (and the surrounding lambda) being a
    ...
    return index != kInvalidCategoryIndex ? index : \
        /* Invalid category -- add it to DEJAVIEW_DEFINE_CATEGORIES(). */ [] {
        DEJAVIEW_CHECK(
            false &&
...
#endif  // INCLUDE_DEJAVIEW_TRACING_TRACK_EVENT_CATEGORY_REGISTRY_H_
