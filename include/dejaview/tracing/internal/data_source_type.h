#ifndef INCLUDE_DEJAVIEW_TRACING_INTERNAL_DATA_SOURCE_TYPE_H_
#define INCLUDE_DEJAVIEW_TRACING_INTERNAL_DATA_SOURCE_TYPE_H_
...
// runtime.
class DEJAVIEW_EXPORT_COMPONENT DataSourceType {
 public:
  ...
  bool TracePrologue(
    ...
    if (DEJAVIEW_UNLIKELY(!*tls_state)) {
    ...
    if (DEJAVIEW_UNLIKELY((*tls_state)->root_tls->is_in_trace_point))
      return false;
 ...
 private:
  void CreateIncrementalState(
      internal::DataSourceInstanceThreadLocalState* tls_inst,
      uint32_t instance_index) {
    DEJAVIEW_DCHECK(create_incremental_state_fn_ != nullptr);
  ...
  void FirstActiveInstance(
    ...
    for (; iterator->i < kMaxDataSourceInstances; iterator->i++) {
      ...
      if (DEJAVIEW_UNLIKELY(!tls_inst.trace_writer)) {
...
#endif  // INCLUDE_DEJAVIEW_TRACING_INTERNAL_DATA_SOURCE_TYPE_H_
