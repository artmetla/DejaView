...
INCLUDE DEJAVIEW MODULE wattson.arm_dsu;
INCLUDE DEJAVIEW MODULE wattson.cpu_freq_idle;
INCLUDE DEJAVIEW MODULE wattson.curves.utils;
INCLUDE DEJAVIEW MODULE wattson.device_infos;

-- Helper macro to do pivot function without policy information
CREATE DEJAVIEW MACRO _stats_wo_policy_subquery(
...
CREATE DEJAVIEW MACRO _stats_w_policy_subquery(
  cpu Expr,
...
CREATE DEJAVIEW TABLE _stats_cpu4567 AS
SELECT
  ii.ts,
  ii.dur,
...
CREATE DEJAVIEW TABLE _suspend_slice AS
SELECT
