...
void AbbrevTable::ReadAbbrevs(string_view data) {
  ...
  while (true) {
    ...
    if (code == 0) {
      abbrev_data_ = string_view(start, static_cast<size_t>(data.data() - start));
      return;
    }
    ...
    if (abbrev.code) {
      QEMU_LOG() << "DWARF data contained duplicate abbrev code\n";
      exit(1);
    }
    ...
    switch (has_child) {
      ...
      case DW_children_no:
        ...
        break;
      default:
        QEMU_LOG() << "DWARF has_child is neither true nor false: " << has_child
                   << ", code=" << abbrev.code << " tag=" << abbrev.tag << "\n";
        exit(1);
    }
...
std::string_view CompilationUnitSizes::ReadInitialLength(
  ...
  if (remaining->size() < len) {
    QEMU_LOG() << "short DWARF compilation unit\n";
    exit(1);
  }
...
bool CUIter::NextCU(InfoReader& reader, CU* cu) {
  ...
  size_t initial_length_len = static_cast<size_t>(data.data() - entire_unit.data());
...
void CU::ReadHeader(string_view entire_unit, string_view data,
  ...
  if (unit_sizes_.dwarf_version() > 5) {
    QEMU_LOG() << "Data is in DWARF " << unit_sizes_.dwarf_version() << " format which we don't understand";
    exit(1);
  }
...
const AbbrevTable::Abbrev* DIEReader::ReadCode(const CU& cu) {
  ...
  if (!cu.unit_abbrev_->GetAbbrev(code, &ret)) {
    QEMU_LOG() << "couldn't find abbreviation for code\n";
    exit(1);
  }
  if (ret->has_child) {
    depth_++;
...
void DIEReader::ReadChildren(const CU& cu, const AbbrevTable::Abbrev* abbrev, std::function<void(const AbbrevTable::Abbrev *)> f) {
  if (!abbrev->has_child) {
    return;
  }
