...
INCLUDE DEJAVIEW MODULE chrome.vsync_intervals;
...
CREATE DEJAVIEW VIEW {{prefix}}_begin_and_end AS
SELECT
  slice.name,
  slice.id,
  slice.ts,
...
CREATE DEJAVIEW VIEW joined_{{prefix}}_begin_and_end AS
SELECT
...
CREATE DEJAVIEW VIEW gesture_update AS
SELECT
  EXTRACT_ARG(arg_set_id, "chrome_latency_info.trace_id") AS trace_id,
  EXTRACT_ARG(arg_set_id, 'chrome_latency_info.{{id_field}}')
  AS {{id_field}},
...
CREATE DEJAVIEW VIEW {{id_field}}_update AS
SELECT
  begin_id,
  begin_ts,
  begin_dur,
...
CREATE DEJAVIEW VIEW {{prefix}}_jank_maybe_null_prev_and_next AS
SELECT
  *,
...
CREATE DEJAVIEW VIEW {{prefix}}_jank AS
SELECT
  id AS slice_id,
...
CREATE DEJAVIEW VIEW {{prefix}}_jank_output AS
SELECT
  {{proto_name}}(
