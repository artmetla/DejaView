...
CREATE DEJAVIEW TABLE android_jank_cuj_hardcoded_thread_names AS
SELECT
  cuj.cuj_id,
  thread.utid as utid
FROM thread
...
CREATE DEJAVIEW TABLE android_jank_cuj_main_thread_overrides AS
SELECT
  cuj.cuj_id,
...
CREATE DEJAVIEW TABLE android_jank_cuj_main_thread AS
...
CREATE OR REPLACE DEJAVIEW FUNCTION android_jank_cuj_app_thread(thread_name STRING)
...
CREATE OR REPLACE DEJAVIEW FUNCTION android_jank_cuj_sf_thread(thread_name STRING)
...
CREATE DEJAVIEW TABLE android_jank_cuj_sf_gpu_completion_thread AS
SELECT * FROM ANDROID_JANK_CUJ_SF_THREAD('GPU completion');

DROP TABLE IF EXISTS android_jank_cuj_sf_render_engine_thread;
CREATE DEJAVIEW TABLE android_jank_cuj_sf_render_engine_thread AS
