...
CREATE DEJAVIEW VIEW all_qurg2 AS
SELECT
  ts,
  track_id,
  name,
...
CREATE DEJAVIEW VIEW non_zero_qurg2 AS
SELECT
  *
FROM all_qurg2
...
CREATE DEJAVIEW VIEW simpleperf_event_raw AS
SELECT
...
CREATE DEJAVIEW VIEW simpleperf_event_per_process AS
SELECT
  e.name,
  t.upid,
  RepeatedField(
...
CREATE DEJAVIEW VIEW simpleperf_event_metric AS
SELECT
...
CREATE DEJAVIEW VIEW android_simpleperf_output AS
