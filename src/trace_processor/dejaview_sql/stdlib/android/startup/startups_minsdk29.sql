...
INCLUDE DEJAVIEW MODULE android.startup.startup_events;

-- Marks the beginning of the trace and is equivalent to when the statsd startup
-- logging begins.
CREATE DEJAVIEW VIEW _activity_intent_received AS
...
-- activity starts.
CREATE DEJAVIEW TABLE _activity_intent_recv_spans AS
SELECT
  ts,
...
-- a startup.
CREATE DEJAVIEW VIEW _startup_partitions AS
...
CREATE DEJAVIEW VIEW _activity_intent_startup_successful AS
SELECT ts FROM slice
...
CREATE DEJAVIEW TABLE _startups_minsdk29 AS
SELECT
  lpart.ts,
  le.ts_end,
