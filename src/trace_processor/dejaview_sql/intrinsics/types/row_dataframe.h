...
#ifndef SRC_TRACE_PROCESSOR_DEJAVIEW_SQL_INTRINSICS_TYPES_ROW_DATAFRAME_H_
#define SRC_TRACE_PROCESSOR_DEJAVIEW_SQL_INTRINSICS_TYPES_ROW_DATAFRAME_H_
...
namespace dejaview::trace_processor::dejaview_sql {
...
struct RowDataframe {
  dejaview_sql::StringArray column_names;
  std::vector<uint32_t> id_to_cell_index;
  // Cell = a value at a row + column index.
  std::vector<dejaview_sql::Value> cells;
  std::optional<uint32_t> id_column_index;

  const dejaview_sql::Value* RowForId(uint32_t id) const {
...
#endif  // SRC_TRACE_PROCESSOR_DEJAVIEW_SQL_INTRINSICS_TYPES_ROW_DATAFRAME_H_
