...
INCLUDE DEJAVIEW MODULE linux.cpu.utilization.general;
...
CREATE DEJAVIEW FUNCTION cpu_process_utilization_per_period(
...
-- period, which is defined as a multiply of |interval|. For this reason
-- first and last period might have lower then real utilization.
CREATE DEJAVIEW FUNCTION cpu_process_utilization_per_second(
  -- Upid of the process.
  upid INT
)
RETURNS TABLE (
...
CREATE DEJAVIEW TABLE cpu_cycles_per_process(
  -- Unique process id
  upid INT,
...
CREATE DEJAVIEW FUNCTION cpu_cycles_per_process_in_interval(
