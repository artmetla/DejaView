...
#ifndef INCLUDE_DEJAVIEW_EXT_BASE_METATRACE_H_
#define INCLUDE_DEJAVIEW_EXT_BASE_METATRACE_H_
...
// A facility to trace execution of the dejaview codebase itself.
...
inline bool IsEnabled(uint32_t tag) {
  ...
  return DEJAVIEW_UNLIKELY((enabled_tags & tag) != 0);
}
...
class RingBuffer {
 public:
  ...
  class ReadIterator {
   public:
    ...
    ~ReadIterator() {
      if (!valid_)
        return;
      DEJAVIEW_DCHECK(cur_ >= RingBuffer::rd_index_);
      DEJAVIEW_DCHECK(cur_ <= RingBuffer::wr_index_);
  ...
  static Record* At(uint64_t index) {
    ...
    DEJAVIEW_DCHECK(index >= rd_index_);
    DEJAVIEW_DCHECK(index <= wr_index_);
  ...
  static ReadIterator GetReadIterator() {
    DEJAVIEW_DCHECK(RingBuffer::IsOnValidTaskRunner());
...
inline void TraceCounter(uint32_t tag, uint16_t id, int32_t value) {
  ...
  auto enabled_tags = g_enabled_tags.load(std::memory_order_relaxed);
  if (DEJAVIEW_LIKELY((enabled_tags & tag) == 0))
    return;
...
#define DEJAVIEW_METATRACE_UID2(a, b) a##b
#define DEJAVIEW_METATRACE_UID(x) DEJAVIEW_METATRACE_UID2(metatrace_, x)

#define DEJAVIEW_METATRACE_SCOPED(TAG, ID)                                \
  ::dejaview::metatrace::ScopedEvent DEJAVIEW_METATRACE_UID(__COUNTER__)( \
      ::dejaview::metatrace::TAG, ::dejaview::metatrace::ID)

#define DEJAVIEW_METATRACE_COUNTER(TAG, ID, VALUE)                \
  ::dejaview::metatrace::TraceCounter(::dejaview::metatrace::TAG, \
                                      ::dejaview::metatrace::ID,  \
                                      static_cast<int32_t>(VALUE))

}  // namespace metatrace
}  // namespace dejaview

#endif  // INCLUDE_DEJAVIEW_EXT_BASE_METATRACE_H_
