...
CREATE DEJAVIEW FUNCTION _partition_start(ts INT, size INT) RETURNS INT AS
-- Division of two ints would result in floor(ts/size).
SELECT ($ts/$size)*$size;

-- Returns the number of partitions required to cover all of the trace
-- timestamps.
CREATE DEJAVIEW FUNCTION _partition_count(size INT) RETURNS INT AS
SELECT
...
CREATE DEJAVIEW FUNCTION _partitions(size INT)
...
-- table.
CREATE DEJAVIEW MACRO _interval_partitions(
...
CREATE DEJAVIEW MACRO _cpu_avg_utilization_per_period(
...
CREATE DEJAVIEW VIEW _cpu_freq_for_metrics AS
SELECT
    id,
    ts,
    dur,
...
CREATE DEJAVIEW VIEW _sched_without_id AS
...
CREATE DEJAVIEW TABLE _cpu_freq_per_thread_no_id
AS
SELECT *
FROM _cpu_freq_per_thread_span_join;

CREATE DEJAVIEW VIEW _cpu_freq_per_thread AS
SELECT
  _auto_id AS id,
  ts,
  dur,
