...
int DEJAVIEW_EXPORT_ENTRYPOINT TriggerDejaViewMain(int argc, char** argv) {
  ...
  // trigger_dejaview can be called by apps so it's command line API needs to
  ...
  // exiting.
  if (seen_unknown_arg) {
    DEJAVIEW_ELOG("Ignoring unknown arguments. See --help for usage.");
  }
  ...
  if (triggers_to_activate.empty()) {
    DEJAVIEW_ELOG("At least one trigger must the specified.");
    return PrintUsage(argv[0]);
  }

  android_stats::MaybeLogTriggerEvents(
      DejaViewTriggerAtom::kTriggerDejaViewTrigger, triggers_to_activate);
  ...
  if (!finished_with_success) {
    android_stats::MaybeLogTriggerEvents(
        DejaViewTriggerAtom::kTriggerDejaViewTriggerFail, triggers_to_activate);
    return 1;
  }
  return 0;
}
