...
#ifndef INCLUDE_DEJAVIEW_EXT_BASE_SMALL_VECTOR_H_
#define INCLUDE_DEJAVIEW_EXT_BASE_SMALL_VECTOR_H_
...
class SmallVector {
 public:
  ...
  ~SmallVector() {
    clear();
    if (DEJAVIEW_UNLIKELY(is_using_heap()))
      AlignedFree(begin_);
  ...
  SmallVector(SmallVector&& other) noexcept(
    ...
    if (other.is_using_heap()) {
      ...
      DEJAVIEW_DCHECK(other_size <= capacity());
  ...
  SmallVector& operator=(const SmallVector& other) {
    if (DEJAVIEW_UNLIKELY(this == &other))
      return *this;
    this->~SmallVector();
  ...
  void emplace_back(Args&&... args) {
    T* end = end_;
    if (DEJAVIEW_UNLIKELY(end == end_of_storage_))
      end = Grow();
 ...
 private:
  DEJAVIEW_NO_INLINE T* Grow(size_t desired_capacity = 0) {
...
#endif  // INCLUDE_DEJAVIEW_EXT_BASE_SMALL_VECTOR_H_
