...
#ifndef INCLUDE_DEJAVIEW_PROTOZERO_SCATTERED_STREAM_WRITER_H_
#define INCLUDE_DEJAVIEW_PROTOZERO_SCATTERED_STREAM_WRITER_H_
...
class DEJAVIEW_EXPORT_COMPONENT ScatteredStreamWriter {
 public:
  class DEJAVIEW_EXPORT_COMPONENT Delegate {
   public:
  ...
  inline void WriteBytes(const uint8_t* src,
                         size_t size) DEJAVIEW_NO_SANITIZE_UNDEFINED {
    ...
    uint8_t* const end = write_ptr_ + size;
    if (DEJAVIEW_LIKELY(end <= cur_range_.end))
  ...
  void Rewind(size_t size, size_t offset) {
    ...
    DEJAVIEW_DCHECK(src >= cur_range_.begin);
    DEJAVIEW_DCHECK(src + size <= cur_range_.end);
    DEJAVIEW_DCHECK(dst >= cur_range_.begin);
    DEJAVIEW_DCHECK(dst + size <= cur_range_.end);
...
#endif  // INCLUDE_DEJAVIEW_PROTOZERO_SCATTERED_STREAM_WRITER_H_
