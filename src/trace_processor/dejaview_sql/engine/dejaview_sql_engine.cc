...
#include "src/trace_processor/dejaview_sql/engine/created_function.h"
#include "src/trace_processor/dejaview_sql/engine/runtime_table_function.h"
...
// ----------------------
//
// The execution of DejaViewSQL statements is the joint responsibility of
// several classes which all are linked together in the following way:
//
//  DejaViewSqlEngine -> DejaViewSqlParser -> DejaViewSqlPreprocessor
//
// The responsibility of each of these classes is as follows:
//
// * DejaViewSqlEngine: this class is responsible for the end-to-end processing
//   of statements. It calls into DejaViewSqlParser to incrementally receive
//   parsed SQL statements and then executes them. If the statement is a
//   DejaViewSQL-only statement, the execution happens entirely in this class.
//   Otherwise, if the statement is a valid SQLite statement, SQLite is called
//   into to perform the execution.
// * DejaViewSqlParser: this class is responsible for taking a chunk of SQL and
//   incrementally converting them into parsed SQL statement. The parser calls
//   into the DejaViewSqlPreprocessor to split the SQL chunk into a statement
//   and perform any macro expansion. It then tries to parse any
//   DejaViewSQL-only statements into their component parts and leaves SQLite
//   statements as-is for execution by SQLite.
// * DejaViewSqlPreprocessor: this class is responsible for taking a chunk of
...
void IncrementCountForStmt(const SqliteEngine::PreparedStatement& p_stmt,
                           DejaViewSqlEngine::ExecutionStats* res) {
  res->statement_count++;
...
// This function is used when the DejaViewSQL has been fully executed by the
// DejaViewSqlEngine and a SqlSoruce is needed for SQLite to execute.
...
DejaViewSqlEngine::DejaViewSqlEngine(StringPool* pool, bool enable_extra_checks)
    : pool_(pool),
      enable_extra_checks_(enable_extra_checks),
      engine_(new SqliteEngine()) {
  // Initialize `dejaview_tables` table, which will contain the names of all of
  // the registered tables.
  char* errmsg_raw = nullptr;
  int err =
      sqlite3_exec(engine_->db(), "CREATE TABLE dejaview_tables(name STRING);",
                   nullptr, nullptr, &errmsg_raw);
  ScopedSqliteString errmsg(errmsg_raw);
  if (err != SQLITE_OK) {
    DEJAVIEW_FATAL("Failed to initialize dejaview_tables: %s", errmsg_raw);
  }
...
DejaViewSqlEngine::PrepareSqliteStatement(SqlSource sql_source) {
  DejaViewSqlParser parser(std::move(sql_source), macros_);
  if (!parser.Next()) {
    return base::ErrStatus("No statement found to prepare");
  }
  auto* sqlite = std::get_if<DejaViewSqlParser::SqliteSql>(&parser.statement());
  if (!sqlite) {
...
void DejaViewSqlEngine::RegisterStaticTable(Table* table,
                                            const std::string& table_name,
                                            Table::Schema schema) {
  // Make sure we didn't accidentally leak a state from a previous table
  // creation.
  DEJAVIEW_CHECK(!static_table_context_->temporary_create_state);
  ...
  base::StackString<1024> sql(
      R"(
        ...
        INSERT INTO dejaview_tables(name) VALUES('%s');
      )",
      table_name.c_str(), table_name.c_str());
  auto status =
      Execute(SqlSource::FromTraceProcessorImplementation(sql.ToStdString()));
  if (!status.ok()) {
    DEJAVIEW_FATAL("%s", status.status().c_message());
  }

  DEJAVIEW_CHECK(!static_table_context_->temporary_create_state);
}

void DejaViewSqlEngine::RegisterStaticTableFunction(
    std::unique_ptr<StaticTableFunction> fn) {
  std::string name = fn->TableName();

  // Make sure we didn't accidentally leak a state from a previous table
  // creation.
  DEJAVIEW_CHECK(!static_table_fn_context_->temporary_create_state);
...
base::StatusOr<DejaViewSqlEngine::ExecutionStats> DejaViewSqlEngine::Execute(
    SqlSource sql) {
...
DejaViewSqlEngine::ExecuteUntilLastStatement(SqlSource sql_source) {
  ...
  // be DejaViewSQL statements which we need to transpile before execution or
  // execute without delegating to SQLite.
  //
  // The logic here is the following:
  //  - We parse the statement as a DejaViewSQL statement.
  ...
  while (parser.Next()) {
    ...
    if (auto* cf = std::get_if<DejaViewSqlParser::CreateFunction>(
            &parser.statement())) {
      RETURN_IF_ERROR(AddTracebackIfNeeded(ExecuteCreateFunction(*cf),
                                           parser.statement_sql()));
      source = RewriteToDummySql(parser.statement_sql());
    } else if (auto* cst = std::get_if<DejaViewSqlParser::CreateTable>(
                   &parser.statement())) {
      RETURN_IF_ERROR(AddTracebackIfNeeded(ExecuteCreateTable(*cst),
                                           parser.statement_sql()));
      source = RewriteToDummySql(parser.statement_sql());
    } else if (auto* create_view = std::get_if<DejaViewSqlParser::CreateView>(
                   &parser.statement())) {
      RETURN_IF_ERROR(AddTracebackIfNeeded(ExecuteCreateView(*create_view),
                                           parser.statement_sql()));
      source = RewriteToDummySql(parser.statement_sql());
    } else if (auto* include = std::get_if<DejaViewSqlParser::Include>(
                   &parser.statement())) {
      RETURN_IF_ERROR(ExecuteInclude(*include, parser));
      source = RewriteToDummySql(parser.statement_sql());
    } else if (auto* macro = std::get_if<DejaViewSqlParser::CreateMacro>(
                   &parser.statement())) {
      auto sql = macro->sql;
      RETURN_IF_ERROR(ExecuteCreateMacro(*macro));
      source = RewriteToDummySql(sql);
    } else if (auto* create_index = std::get_if<DejaViewSqlParser::CreateIndex>(
                   &parser.statement())) {
      RETURN_IF_ERROR(ExecuteCreateIndex(*create_index));
      source = RewriteToDummySql(parser.statement_sql());
    } else if (auto* drop_index = std::get_if<DejaViewSqlParser::DropIndex>(
                   &parser.statement())) {
      ...
      auto* sql =
          std::get_if<DejaViewSqlParser::SqliteSql>(&parser.statement());
      DEJAVIEW_CHECK(sql);
    ...
    {
      DEJAVIEW_TP_TRACE(metatrace::Category::QUERY_TIMELINE, "QUERY_PREPARE");
    ...
    {
      DEJAVIEW_TP_TRACE(metatrace::Category::QUERY_TIMELINE, "STMT_FIRST_STEP",
                        [&res](metatrace::Record* record) {
                          record->AddArg("Original SQL", res->original_sql());
                          record->AddArg("Executed SQL", res->sql());
                        });
...
base::Status DejaViewSqlEngine::RegisterRuntimeFunction(
    bool replace,
  ...
  if (!opt_return_type) {
    return base::ErrStatus(
        "CREATE DEJAVIEW FUNCTION[prototype=%s, return=%s]: unknown return "
        "type specified",
  ...
  if (ctx) {
    if (CreatedFunction::IsValid(ctx) && !replace) {
      return base::ErrStatus(
          "CREATE DEJAVIEW FUNCTION[prototype=%s]: function already exists",
...
DejaViewSqlEngine::CreateTableImpl(
    const char* tag,
...
base::Status DejaViewSqlEngine::ExecuteCreateTable(
    const DejaViewSqlParser::CreateTable& create_table) {
  DEJAVIEW_TP_TRACE(metatrace::Category::QUERY_TIMELINE,
                    "CREATE DEJAVIEW TABLE",
  ...
  base::StatusOr<std::vector<std::string>> maybe_column_names =
      GetColumnNamesFromSelectStatement(stmt, "CREATE DEJAVIEW TABLE");
  ...
  base::StatusOr<std::vector<sql_argument::ArgumentDefinition>>
      effective_schema = ValidateAndGetEffectiveSchema(
          column_names, create_table.schema, "CREATE DEJAVIEW TABLE");
  RETURN_IF_ERROR(effective_schema.status());

  ASSIGN_OR_RETURN(
      auto table,
      CreateTableImpl("CREATE DEJAVIEW TABLE", create_table.name,
  ...
  // creation.
  DEJAVIEW_CHECK(!runtime_table_context_->temporary_create_state);
...
base::Status DejaViewSqlEngine::ExecuteCreateView(
    const DejaViewSqlParser::CreateView& create_view) {
  DEJAVIEW_TP_TRACE(metatrace::Category::QUERY_TIMELINE, "CREATE DEJAVIEW VIEW",
  ...
  if (!create_view.schema.empty()) {
    base::StatusOr<std::vector<std::string>> maybe_column_names =
        GetColumnNamesFromSelectStatement(stmt, "CREATE DEJAVIEW VIEW");
    RETURN_IF_ERROR(maybe_column_names.status());
    std::vector<std::string> column_names = *maybe_column_names;

    base::StatusOr<std::vector<sql_argument::ArgumentDefinition>>
        effective_schema = ValidateAndGetEffectiveSchema(
            column_names, create_view.schema, "CREATE DEJAVIEW VIEW");
    RETURN_IF_ERROR(effective_schema.status());
    ...
    if (enable_extra_checks_) {
      ...
      base::StatusOr<std::unique_ptr<RuntimeTable>> materialized =
          CreateTableImpl("CREATE DEJAVIEW VIEW", create_view.name,
                          std::move(stmt), column_names, *effective_schema,
...
base::Status DejaViewSqlEngine::EnableSqlFunctionMemoization(
    const std::string& name) {
...
base::Status DejaViewSqlEngine::ExecuteInclude(
    const DejaViewSqlParser::Include& include,
    const DejaViewSqlParser& parser) {
  DEJAVIEW_TP_TRACE(
      metatrace::Category::QUERY_TIMELINE, "INCLUDE DEJAVIEW MODULE",
...
base::Status DejaViewSqlEngine::ExecuteCreateIndex(
    const DejaViewSqlParser::CreateIndex& index) {
  DEJAVIEW_TP_TRACE(metatrace::Category::QUERY_TIMELINE,
                    "CREATE DEJAVIEW INDEX",
  ...
  if (!t) {
    return base::ErrStatus("CREATE DEJAVIEW INDEX: Table '%s' not found",
  ...
  for (const std::string& col_name : index.col_names) {
    ...
    if (!opt_col) {
      return base::ErrStatus(
          "CREATE DEJAVIEW INDEX: Column '%s' not found in table '%s'",
...
base::Status DejaViewSqlEngine::ExecuteDropIndex(
    const DejaViewSqlParser::DropIndex& index) {
  DEJAVIEW_TP_TRACE(metatrace::Category::QUERY_TIMELINE, "DROP DEJAVIEW INDEX",
                    [&index](metatrace::Record* record) {
                      record->AddArg("index_name", index.name);
                      record->AddArg("table_name", index.table_name);
                    });
  ...
  Table* t = GetMutableTableOrNull(index.table_name);
  if (!t) {
    return base::ErrStatus("DROP DEJAVIEW INDEX: Table '%s' not found",
                           index.table_name.c_str());
  }
...
base::Status DejaViewSqlEngine::IncludePackageImpl(
...
base::Status DejaViewSqlEngine::IncludeModuleImpl(
...
base::Status DejaViewSqlEngine::ExecuteCreateFunction(
    const DejaViewSqlParser::CreateFunction& cf) {
  DEJAVIEW_TP_TRACE(metatrace::Category::QUERY_TIMELINE,
                    "CREATE DEJAVIEW FUNCTION",
  ...
  // CREATE DEJAVIEW TABLE implementation above: see the comment there for
  // more info on this.
  if (cf.replace) {
    base::StackString<1024> drop("DROP TABLE IF EXISTS %s",
                                 state->prototype.function_name.c_str());
  ...
  // Make sure we didn't accidentally leak a state from a previous function
  // creation.
  DEJAVIEW_CHECK(!runtime_table_fn_context_->temporary_create_state);
...
base::Status DejaViewSqlEngine::ExecuteCreateMacro(
    const DejaViewSqlParser::CreateMacro& create_macro) {
  DEJAVIEW_TP_TRACE(metatrace::Category::QUERY_TIMELINE,
                    "CREATE DEJAVIEW MACRO",
  ...
  DejaViewSqlPreprocessor::Macro macro{
...
DejaViewSqlEngine::GetColumnNamesFromSelectStatement(
...
base::StatusOr<std::vector<sql_argument::ArgumentDefinition>>
DejaViewSqlEngine::ValidateAndGetEffectiveSchema(
    const std::vector<std::string>& column_names,
    const std::vector<sql_argument::ArgumentDefinition>& schema,
    const char* tag) const {
...
const RuntimeTable* DejaViewSqlEngine::GetRuntimeTableOrNull(
    std::string_view name) const {
  auto* state = runtime_table_context_->manager.FindStateByName(name);
  return state ? state->runtime_table.get() : nullptr;
}

RuntimeTable* DejaViewSqlEngine::GetMutableRuntimeTableOrNull(
    std::string_view name) {
  auto* state = runtime_table_context_->manager.FindStateByName(name);
  return state ? state->runtime_table.get() : nullptr;
}

const Table* DejaViewSqlEngine::GetStaticTableOrNull(
    std::string_view name) const {
  auto* state = static_table_context_->manager.FindStateByName(name);
  return state ? state->static_table : nullptr;
}

Table* DejaViewSqlEngine::GetMutableStaticTableOrNull(std::string_view name) {
  auto* state = static_table_context_->manager.FindStateByName(name);
  return state ? state->static_table : nullptr;
}
