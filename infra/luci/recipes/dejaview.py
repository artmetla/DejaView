...
def UploadArtifact(api, ctx, platform, out_dir, artifact):
  ...
  api.gsutil.upload(source_path, 'dejaview-luci-artifacts', gcs_target_path)

  # Uploads also the .pdb (debug symbols) to GCS.
  pdb_path = exe_dir.join(artifact_ext + '.pdb')
  if api.platform.is_win:
    api.gsutil.upload(pdb_path, 'dejaview-luci-artifacts',
                      gcs_target_path + '.pdb')

  # Create the CIPD package definition from the artifact path.
  cipd_pkg_name = 'dejaview/{}/{}'.format(artifact['name'], platform)
  ...
  for target in ('android', 'linux', 'mac', 'win'):
    ...
    yield (api.test('ci_' + target) + api.platform.name(host) +
           api.buildbucket.ci_build(
               project='dejaview',
               builder='dejaview-official-builder-%s' % target,
               git_repo='android.googlesource.com/platform/external/dejaview',
           ))
