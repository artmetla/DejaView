...
"""
Functions to fetch pre-pinned DejaView prebuilts.
...
The intended usage is:
  ...
  from dejaview.prebuilts.manifests.traceconv import TRACECONV_MANIFEST
  bin_path = get_dejaview_prebuilt(TRACECONV_MANIFEST)
...
def download_or_get_cached(file_name, url, sha256):
  ...
  ~/.local/share/dejaview/prebuilts/$tool_name. On subsequent invocations it
  just runs the cached version.
  """
  dir = os.path.join(
      os.path.expanduser('~'), '.local', 'share', 'dejaview', 'prebuilts')
...
def get_dejaview_prebuilt(manifest, soft_fail=False, arch=None):
...
def run_dejaview_prebuilt(manifest):
  bin_path = get_dejaview_prebuilt(manifest)
