...
CREATE DEJAVIEW FUNCTION _descendant_slice_end(
...
CREATE DEJAVIEW FUNCTION _get_presentation_timestamp(
...
CREATE DEJAVIEW TABLE chrome_event_latencies(
  ...
  event_type STRING,
  -- DejaView track this slice is found on.
  track_id INT
) AS
SELECT
  slice.id,
...
CREATE DEJAVIEW TABLE _gesture_scroll_events_no_scroll_id
AS
SELECT
  name,
  ts,
...
CREATE DEJAVIEW FUNCTION chrome_get_most_recent_scroll_begin_id(
...
CREATE DEJAVIEW TABLE chrome_gesture_scroll_events(
  -- Slice Id for the EventLatency scroll event.
  id INT,
  -- Slice name.
  name STRING,
