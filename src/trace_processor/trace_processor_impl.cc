...
#include "src/trace_processor/dejaview_sql/engine/table_pointer_module.h"
#include "src/trace_processor/dejaview_sql/intrinsics/functions/base64.h"
#include "src/trace_processor/dejaview_sql/intrinsics/functions/clock_functions.h"
#include "src/trace_processor/dejaview_sql/intrinsics/functions/counter_intervals.h"
...
#include "src/trace_processor/dejaview_sql/intrinsics/functions/dominator_tree.h"
#include "src/trace_processor/dejaview_sql/intrinsics/functions/graph_scan.h"
#include "src/trace_processor/dejaview_sql/intrinsics/functions/graph_traversal.h"
#include "src/trace_processor/dejaview_sql/intrinsics/functions/import.h"
...
#include "src/trace_processor/dejaview_sql/intrinsics/operators/counter_mipmap_operator.h"
#include "src/trace_processor/dejaview_sql/intrinsics/operators/slice_mipmap_operator.h"
#include "src/trace_processor/dejaview_sql/intrinsics/operators/span_join_operator.h"
#include "src/trace_processor/dejaview_sql/intrinsics/operators/window_operator.h"
#include "src/trace_processor/dejaview_sql/intrinsics/table_functions/ancestor.h"
#include "src/trace_processor/dejaview_sql/intrinsics/table_functions/connected_flow.h"
#include "src/trace_processor/dejaview_sql/intrinsics/table_functions/descendant.h"
#include "src/trace_processor/dejaview_sql/intrinsics/table_functions/dfs_weight_bounded.h"
#include "src/trace_processor/dejaview_sql/intrinsics/table_functions/experimental_annotated_stack.h"
#include "src/trace_processor/dejaview_sql/intrinsics/table_functions/experimental_counter_dur.h"
#include "src/trace_processor/dejaview_sql/intrinsics/table_functions/experimental_flamegraph.h"
#include "src/trace_processor/dejaview_sql/intrinsics/table_functions/experimental_flat_slice.h"
#include "src/trace_processor/dejaview_sql/intrinsics/table_functions/experimental_sched_upid.h"
#include "src/trace_processor/dejaview_sql/intrinsics/table_functions/experimental_slice_layout.h"
#include "src/trace_processor/dejaview_sql/intrinsics/table_functions/table_info.h"
#include "src/trace_processor/dejaview_sql/stdlib/stdlib.h"
...
void RegisterFunction(DejaViewSqlEngine* engine,
                      const char* name,
                      int argc,
                      Ptr context = nullptr,
...
base::Status RegisterAllProtoBuilderFunctions(
  ...
  for (uint32_t i = 0; i < pool->descriptors().size(); ++i) {
    // Convert the full name (e.g. .dejaview.protos.TraceMetrics.SubMetric)
...
void BuildBoundsTable(sqlite3* db, std::pair<int64_t, int64_t> bounds) {
  ...
  if (error) {
    DEJAVIEW_ELOG("Error deleting from bounds table: %s", error);
    sqlite3_free(error);
    return;
  }
  ...
  if (error) {
    DEJAVIEW_ELOG("Error inserting bounds table: %s", error);
    sqlite3_free(error);
  }
}
...
class ValueAtMaxTs : public SqliteAggregateFunction<ValueAtMaxTs> {
 public:
  ...
  static void Step(sqlite3_context* ctx, int, sqlite3_value** argv) {
    ...
    if (DEJAVIEW_UNLIKELY(!fn_ctx->initialized)) {
    ...
    if (DEJAVIEW_LIKELY(fn_ctx->max_ts <= ts_int)) {
...
void RegisterValueAtMaxTsFunction(DejaViewSqlEngine& engine) {
  base::Status status =
      engine.RegisterSqliteAggregateFunction<ValueAtMaxTs>(nullptr);
  if (!status.ok()) {
    DEJAVIEW_ELOG("Error initializing VALUE_AT_MAX_TS");
  }
}
...
void InsertIntoTraceMetricsTable(sqlite3* db, const std::string& metric_name) {
  ...
  if (insert_error) {
    DEJAVIEW_ELOG("Error registering table: %s", insert_error);
...
size_t TraceProcessorImpl::RestoreInitialTables() {
  ...
  DEJAVIEW_CHECK(registered_count_before >=
                 sqlite_objects_post_constructor_initialization_);
  ...
  DEJAVIEW_CHECK(registered_count_after ==
                 sqlite_objects_post_constructor_initialization_);
  return static_cast<size_t>(registered_count_before - registered_count_after);
}

Iterator TraceProcessorImpl::ExecuteQuery(const std::string& sql) {
  DEJAVIEW_TP_TRACE(metatrace::Category::API_TIMELINE, "EXECUTE_QUERY",
...
bool TraceProcessorImpl::IsRootMetricField(const std::string& metric_name) {
  std::optional<uint32_t> desc_idx =
      pool_.FindDescriptorIdx(".dejaview.protos.TraceMetrics");
...
base::Status TraceProcessorImpl::RegisterMetric(const std::string& path,
  ...
  if (IsRootMetricField(no_ext_name)) {
    ...
    if (!field_it_and_inserted.second) {
      ...
      DEJAVIEW_DCHECK(prev_path != path);
      return base::ErrStatus(
...
base::Status TraceProcessorImpl::ComputeMetric(
  ...
  auto opt_idx = pool_.FindDescriptorIdx(".dejaview.protos.TraceMetrics");
  ...
  switch (format) {
    case TraceProcessor::MetricResultFormat::kProtoText:
      *metrics_string = protozero_to_text::ProtozeroToText(
          pool_, ".dejaview.protos.TraceMetrics",
...
void TraceProcessorImpl::InitDejaViewSqlEngine() {
  engine_.reset(new DejaViewSqlEngine(context_.storage->mutable_string_pool(),
  ...
  {
    base::Status status = dejaview_sql::RegisterIntervalIntersectFunctions(
        *engine_, context_.storage->mutable_string_pool());
  }
  {
    base::Status status = dejaview_sql::RegisterCounterIntervalsFunctions(
        *engine_, context_.storage->mutable_string_pool());
  }
  ...
  // Import prelude package.
  {
    auto result = engine_->Execute(SqlSource::FromTraceProcessorImplementation(
        "INCLUDE DEJAVIEW MODULE prelude.*"));
    if (!result.status().ok()) {
      DEJAVIEW_FATAL("Failed to import prelude: %s",
...
class StringInterner {
 public:
  StringInterner(protos::pbzero::DejaViewMetatrace& event,
 ...
 private:
  protos::pbzero::DejaViewMetatrace& event_;
