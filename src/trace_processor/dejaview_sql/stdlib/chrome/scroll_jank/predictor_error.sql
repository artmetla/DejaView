...
CREATE DEJAVIEW FUNCTION _get_slow_scroll_delta_threshold()
RETURNS DOUBLE AS
SELECT 7.0;
...
-- "slow".
CREATE DEJAVIEW FUNCTION _get_slow_scroll_janky_threshold()
RETURNS DOUBLE AS
SELECT 1.4;
...
-- neighbors in a sequence of three frames, if the sequence is considered
-- "fast".
CREATE DEJAVIEW FUNCTION _get_fast_scroll_janky_threshold()
RETURNS DOUBLE AS
SELECT 1.2;
...
CREATE DEJAVIEW FUNCTION _get_scroll_jank_threshold(
  d1 DOUBLE,
  d2 DOUBLE,
  d3 DOUBLE
)
...
CREATE DEJAVIEW FUNCTION _get_predictor_jank(
  d1 DOUBLE,
  d2 DOUBLE,
  d3 DOUBLE,
  threshold DOUBLE
...
CREATE DEJAVIEW TABLE _deltas_and_neighbors AS
SELECT
  scroll_id,
...
CREATE DEJAVIEW TABLE _deltas_and_neighbors_with_threshold AS
SELECT
  scroll_id,
  event_latency_slice_id,
  scroll_update_id,
...
-- scroll events.
CREATE DEJAVIEW TABLE chrome_predictor_error(
