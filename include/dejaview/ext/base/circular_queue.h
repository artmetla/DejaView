...
#ifndef INCLUDE_DEJAVIEW_EXT_BASE_CIRCULAR_QUEUE_H_
#define INCLUDE_DEJAVIEW_EXT_BASE_CIRCULAR_QUEUE_H_
...
class CircularQueue {
 public:
  class Iterator {
   public:
    ...
    T* operator->() const {
#if DEJAVIEW_DCHECK_IS_ON()
      DEJAVIEW_DCHECK(generation_ == queue_->generation());
#endif
   ...
   private:
    inline void Add(difference_type offset) {
      ...
      DEJAVIEW_DCHECK(pos_ <= queue_->end_);
    }
  ...
  explicit CircularQueue(const CircularQueue& other) noexcept {
    ...
    for (const auto& e : const_cast<CircularQueue&>(other))
      emplace_back(e);
    DEJAVIEW_DCHECK(size() == other.size());
  }
  ...
  void emplace_back(Args&&... args) {
    increment_generation();
    if (DEJAVIEW_UNLIKELY(size() >= capacity_))
      Grow();
    T* slot = Get(end_++);
  ...
  size_t size() const {
    DEJAVIEW_DCHECK(end_ - begin_ <= capacity_);
 ...
 private:
  ...
  void ChangeCapacity(size_t new_capacity) {
    ...
    DEJAVIEW_CHECK(new_capacity >= size());
...
#endif  // INCLUDE_DEJAVIEW_EXT_BASE_CIRCULAR_QUEUE_H_
