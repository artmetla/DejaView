...
#ifndef INCLUDE_DEJAVIEW_TRACE_PROCESSOR_TRACE_BLOB_VIEW_H_
#define INCLUDE_DEJAVIEW_TRACE_PROCESSOR_TRACE_BLOB_VIEW_H_
...
class alignas(8) TraceBlobView {
 public:
  ...
  explicit TraceBlobView(TraceBlob blob,
    ...
    DEJAVIEW_DCHECK(offset <= std::numeric_limits<uint32_t>::max());
    data_ = blob.data() + offset;
    if (length == kWholeBlob) {
      length_ = static_cast<uint32_t>(blob.size() - offset);
    } else {
      DEJAVIEW_DCHECK(length <= std::numeric_limits<uint32_t>::max());
      DEJAVIEW_DCHECK(offset + length_ <= blob.size());
  ...
  TraceBlobView(RefPtr<TraceBlob> blob, size_t offset, uint32_t length)
    ...
    DEJAVIEW_DCHECK(offset + length_ <= blob_->size());
  }
  ...
  TraceBlobView slice(const uint8_t* data, size_t length) const {
    DEJAVIEW_DCHECK(data >= data_);
    DEJAVIEW_DCHECK(data + length <= data_ + length_);
  ...
  TraceBlobView slice_off(size_t off, size_t length) const {
    DEJAVIEW_DCHECK(off + length <= length_);
...
#endif  // INCLUDE_DEJAVIEW_TRACE_PROCESSOR_TRACE_BLOB_VIEW_H_
