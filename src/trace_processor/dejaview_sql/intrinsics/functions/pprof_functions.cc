...
class AggregateContext {
 ...
 private:
  ...
  base::Status UpdateSampleValue(size_t argc, sqlite3_value** argv) {
    if (argc == 1) {
      DEJAVIEW_CHECK(sample_values_.size() == 1);
      return base::OkStatus();
    }

    DEJAVIEW_CHECK(argc == 1 + (sample_values_.size() * 3));
...
base::Status PprofFunctions::Register(DejaViewSqlEngine& engine,
