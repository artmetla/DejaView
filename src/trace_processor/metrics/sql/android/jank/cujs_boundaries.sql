...
CREATE DEJAVIEW TABLE android_jank_cuj_vsync_boundary AS
SELECT
  cuj.cuj_id,
...
CREATE DEJAVIEW TABLE android_jank_cuj_sf_vsync_boundary AS
SELECT
  cuj_id,
...
CREATE DEJAVIEW TABLE android_jank_cuj_main_thread_frame_boundary AS
...
CREATE DEJAVIEW TABLE android_jank_cuj_main_thread_cuj_boundary AS
SELECT
  cuj_id,
  utid,
  MIN(ts) AS ts,
...
CREATE DEJAVIEW TABLE android_jank_cuj_render_thread_frame_boundary AS
...
CREATE DEJAVIEW TABLE android_jank_cuj_render_thread_cuj_boundary AS
SELECT
  cuj_id,
  utid,
  MIN(ts) AS ts,
...
CREATE DEJAVIEW TABLE android_jank_cuj_boundary AS
-- introducing an intermediate table since we want to calculate dur = ts_end - ts
...
CREATE DEJAVIEW TABLE android_jank_cuj_sf_main_thread_frame_boundary AS
...
CREATE DEJAVIEW TABLE android_jank_cuj_sf_main_thread_cuj_boundary AS
SELECT
  cuj_id,
  utid,
  MIN(ts) AS ts,
...
CREATE DEJAVIEW TABLE android_jank_cuj_sf_render_engine_frame_boundary AS
SELECT
  cuj_id,
  utid,
  vsync,
...
CREATE DEJAVIEW TABLE android_jank_cuj_sf_boundary AS
