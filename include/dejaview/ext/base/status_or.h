...
#ifndef INCLUDE_DEJAVIEW_EXT_BASE_STATUS_OR_H_
#define INCLUDE_DEJAVIEW_EXT_BASE_STATUS_OR_H_
...
class StatusOr {
 public:
  ...
  StatusOr(base::Status status) : StatusOr(std::move(status), std::nullopt) {
    if (status.ok()) {
      ...
      // absl::StatusOr<T>.
      DEJAVIEW_FATAL("base::OkStatus passed to StatusOr: this is not allowd");
    }
  }
 ...
 private:
  StatusOr(base::Status status, std::optional<T> value)
    ...
    DEJAVIEW_DCHECK(!status_.ok() || value_.has_value());
  }
...
#endif  // INCLUDE_DEJAVIEW_EXT_BASE_STATUS_OR_H_
