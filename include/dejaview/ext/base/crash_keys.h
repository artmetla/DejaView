...
#ifndef INCLUDE_DEJAVIEW_EXT_BASE_CRASH_KEYS_H_
#define INCLUDE_DEJAVIEW_EXT_BASE_CRASH_KEYS_H_
...
class CrashKey {
 public:
  ...
  void Set(int64_t value) {
    ...
    if (DEJAVIEW_UNLIKELY(!registered_.load(std::memory_order_relaxed)))
      Register();
  }
  ...
  ScopedClear SetScoped(int64_t value) DEJAVIEW_WARN_UNUSED_RESULT {
    Set(value);
    return ScopedClear(this);
  }

  ScopedClear SetScoped(StringView sv) DEJAVIEW_WARN_UNUSED_RESULT {
    Set(sv);
    return ScopedClear(this);
  }
...
#endif  // INCLUDE_DEJAVIEW_EXT_BASE_CRASH_KEYS_H_
