...
static int RelayServiceMain(int argc, char** argv) {
  ...
  for (;;) {
    ...
    switch (option) {
      ...
      case OPT_SET_SOCKET_PERMISSIONS: {
        ...
        auto parts = dejaview::base::SplitString(std::string(optarg), ":");
  ...
  if (env_prod) {
    ...
    if (!listen_socket_group.empty()) {
      ...
      if (!status.ok()) {
        DEJAVIEW_ELOG("Failed to set socket permissions: %s",
                      status.c_message());
        return 1;
      }
    }
  ...
  DEJAVIEW_ILOG("Started traced_relay, listening on %s, forwarding to %s",
...
int main(int argc, char** argv) {
  return dejaview::RelayServiceMain(argc, argv);
}
