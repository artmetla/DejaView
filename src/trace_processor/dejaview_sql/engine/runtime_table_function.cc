...
int RuntimeTableFunctionModule::Filter(sqlite3_vtab_cursor* cur,
  ...
  DEJAVIEW_CHECK(static_cast<size_t>(argc) == s->prototype.arguments.size());
  DEJAVIEW_TP_TRACE(metatrace::Category::FUNCTION_CALL, "TABLE_FUNCTION_CALL",
...
int RuntimeTableFunctionModule::Column(sqlite3_vtab_cursor* cur,
  ...
  if (DEJAVIEW_LIKELY(s->IsReturnValueColumn(idx))) {
    sqlite::result::Value(ctx, sqlite3_column_value(c->stmt->sqlite_stmt(), N));
    return SQLITE_OK;
  }

  if (DEJAVIEW_LIKELY(s->IsArgumentColumn(idx))) {
  ...
  DEJAVIEW_DCHECK(s->IsPrimaryKeyColumn(idx));
