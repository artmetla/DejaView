...
#ifndef SRC_TRACE_PROCESSOR_DEJAVIEW_SQL_PARSER_DEJAVIEW_SQL_TEST_UTILS_H_
#define SRC_TRACE_PROCESSOR_DEJAVIEW_SQL_PARSER_DEJAVIEW_SQL_TEST_UTILS_H_
...
inline bool operator==(const DejaViewSqlParser::SqliteSql&,
                       const DejaViewSqlParser::SqliteSql&) {
  return true;
}

inline bool operator==(const DejaViewSqlParser::CreateFunction& a,
                       const DejaViewSqlParser::CreateFunction& b) {
  return std::tie(a.returns, a.is_table, a.prototype, a.replace, a.sql) ==
         std::tie(b.returns, b.is_table, b.prototype, b.replace, b.sql);
}

inline bool operator==(const DejaViewSqlParser::CreateTable& a,
                       const DejaViewSqlParser::CreateTable& b) {
  return std::tie(a.name, a.sql) == std::tie(b.name, b.sql);
}

inline bool operator==(const DejaViewSqlParser::CreateView& a,
                       const DejaViewSqlParser::CreateView& b) {
  return std::tie(a.name, a.create_view_sql) ==
         std::tie(b.name, b.create_view_sql);
}

inline bool operator==(const DejaViewSqlParser::Include& a,
                       const DejaViewSqlParser::Include& b) {
  return std::tie(a.key) == std::tie(b.key);
}

constexpr bool operator==(const DejaViewSqlParser::CreateMacro& a,
                          const DejaViewSqlParser::CreateMacro& b) {
  return std::tie(a.replace, a.name, a.sql, a.args) ==
         std::tie(b.replace, b.name, b.sql, b.args);
}

constexpr bool operator==(const DejaViewSqlParser::CreateIndex& a,
                          const DejaViewSqlParser::CreateIndex& b) {
  return std::tie(a.replace, a.name, a.table_name, a.col_names) ==
         std::tie(b.replace, b.name, b.table_name, b.col_names);
}

constexpr bool operator==(const DejaViewSqlParser::DropIndex& a,
                          const DejaViewSqlParser::DropIndex& b) {
...
inline std::ostream& operator<<(std::ostream& stream,
                                const DejaViewSqlParser::Statement& line) {
  if (std::get_if<DejaViewSqlParser::SqliteSql>(&line)) {
    return stream << "SqliteSql()";
  }
  if (auto* fn = std::get_if<DejaViewSqlParser::CreateFunction>(&line)) {
...
inline SqlSource FindSubstr(const SqlSource& source,
  ...
  DEJAVIEW_CHECK(off != std::string::npos);
...
#endif  // SRC_TRACE_PROCESSOR_DEJAVIEW_SQL_PARSER_DEJAVIEW_SQL_TEST_UTILS_H_
