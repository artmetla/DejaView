...
#ifndef INCLUDE_DEJAVIEW_BASE_STATUS_H_
#define INCLUDE_DEJAVIEW_BASE_STATUS_H_
...
class DEJAVIEW_EXPORT_COMPONENT Status {
 public:
  Status() : ok_(true) {}
  explicit Status(std::string msg) : ok_(false), message_(std::move(msg)) {
    DEJAVIEW_CHECK(!message_.empty());
  }
...
Status ErrStatus(const char* format, ...) DEJAVIEW_PRINTF_FORMAT(1, 2);

}  // namespace base
}  // namespace dejaview

#endif  // INCLUDE_DEJAVIEW_BASE_STATUS_H_
