...
#ifndef INCLUDE_DEJAVIEW_TRACE_PROCESSOR_REF_COUNTED_H_
#define INCLUDE_DEJAVIEW_TRACE_PROCESSOR_REF_COUNTED_H_
...
class RefPtr {
 public:
  ...
  T* ReleaseUnsafe() {
    DEJAVIEW_DCHECK(ptr_->refcount_ > 0);
    auto* old_ptr = ptr_;
    ptr_ = nullptr;
    return old_ptr;
  }
  ...
  static RefPtr<T> FromReleasedUnsafe(T* ptr) {
    DEJAVIEW_DCHECK(ptr->refcount_ > 0);
    RefPtr<T> res;
    res.ptr_ = ptr;
    return res;
  }
...
#endif  // INCLUDE_DEJAVIEW_TRACE_PROCESSOR_REF_COUNTED_H_
