...
CREATE DEJAVIEW TABLE _translate_and_scale_scroll_deltas AS
SELECT
...
CREATE DEJAVIEW TABLE _scroll_deltas_with_timestamp AS
SELECT
...
CREATE DEJAVIEW TABLE _scroll_deltas_with_scroll_id AS
SELECT
...
CREATE DEJAVIEW TABLE _scroll_deltas_with_delays AS
SELECT
  deltas.scroll_id,
  delay.total_delta,
  deltas.scroll_update_id,
...
-- a scroll.
CREATE DEJAVIEW TABLE chrome_scroll_input_offsets(
...
CREATE DEJAVIEW TABLE chrome_presented_scroll_offsets(
  -- An ID that ties all EventLatencies in a particular scroll. (implementation
  -- note: This is the EventLatency TraceId of the GestureScrollbegin).
  scroll_id INT,
  -- An ID for this particular EventLatency regardless of it being presented or
