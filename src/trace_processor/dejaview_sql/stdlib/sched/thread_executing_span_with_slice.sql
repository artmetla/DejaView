...
CREATE DEJAVIEW TABLE _critical_path_userspace
AS
SELECT *
FROM
...
CREATE DEJAVIEW TABLE _critical_path_kernel
AS
...
CREATE DEJAVIEW TABLE _critical_path_userspace_adjusted AS
SELECT DISTINCT * FROM _critical_path_userspace_adjusted!(_critical_path_userspace, _wakeup_graph);

CREATE DEJAVIEW TABLE _critical_path_kernel_adjusted AS
...
CREATE DEJAVIEW TABLE _critical_path_merged_adjusted AS
...
CREATE DEJAVIEW TABLE _critical_path_roots AS
  SELECT root_id, min(ts) AS root_ts, max(ts + dur) - min(ts) AS root_dur
  FROM _critical_path_userspace_adjusted
  GROUP BY root_id;

CREATE DEJAVIEW TABLE _critical_path_roots_and_merged AS
...
CREATE DEJAVIEW TABLE _critical_path_all
AS
SELECT
...
CREATE DEJAVIEW VIEW _span_thread_state_view
AS
...
CREATE DEJAVIEW VIEW _span_slice_view
AS
SELECT
  slice_id,
...
CREATE DEJAVIEW TABLE _span_thread_state_slice
AS
SELECT
...
CREATE DEJAVIEW TABLE _critical_path_thread_state_slice_raw
AS
SELECT
  id_0 AS cr_id,
  id_1 AS th_id,
...
CREATE DEJAVIEW TABLE _critical_path_thread_state_slice
AS
SELECT
  raw.ts,
  raw.dur,
...
CREATE DEJAVIEW VIEW _self_view
  AS
  SELECT
...
CREATE DEJAVIEW FUNCTION _critical_path_stack(root_utid INT, ts LONG, dur LONG, enable_process_name INT, enable_thread_name INT, enable_self_slice INT, enable_critical_path_slice INT)
RETURNS
  TABLE(
    id INT,
    ts LONG,
...
CREATE DEJAVIEW FUNCTION _thread_executing_span_critical_path_stack(
...
CREATE DEJAVIEW FUNCTION _critical_path_graph(graph_title STRING, root_utid INT, ts LONG, dur LONG, enable_process_name INT, enable_thread_name INT, enable_self_slice INT, enable_critical_path_slice INT)
...
CREATE DEJAVIEW FUNCTION _thread_executing_span_critical_path_graph(
