...
void DejaViewCmd::SaveTraceIntoIncidentOrCrash() {
  DEJAVIEW_CHECK(save_to_incidentd_);

  const auto& cfg = trace_config_->incident_report_config();
  DEJAVIEW_CHECK(!cfg.destination_package().empty());
  DEJAVIEW_CHECK(!cfg.skip_incidentd());

  if (bytes_written_ == 0) {
    LogUploadEvent(DejaViewStatsdAtom::kNotUploadingEmptyTrace);
    DEJAVIEW_LOG("Skipping write to incident. Empty trace.");
    return;
  }
  ...
  if (bytes_written_ > 4096) {
    ...
    DEJAVIEW_LOG("go/trace-uuid/%s name=\"%s\" size=%" PRIu64,
  ...
  // wrote.
  DEJAVIEW_LAZY_LOAD(android_internal::StartIncidentReport, incident_fn);
  DEJAVIEW_CHECK(incident_fn(cfg.destination_package().c_str(),
                             cfg.destination_class().c_str(),
                             cfg.privacy_level()));
}

void DejaViewCmd::ReportTraceToAndroidFrameworkOrCrash() {
  DEJAVIEW_CHECK(report_to_android_framework_);
  DEJAVIEW_CHECK(trace_out_stream_);

  const auto& cfg = trace_config_->android_report_config();
  DEJAVIEW_CHECK(!cfg.reporter_service_package().empty());
  DEJAVIEW_CHECK(!cfg.skip_report());

  if (bytes_written_ == 0) {
    LogUploadEvent(DejaViewStatsdAtom::kCmdFwReportEmptyTrace);
    DEJAVIEW_LOG("Skipping reporting trace to Android. Empty trace.");
    return;
  }

  LogUploadEvent(DejaViewStatsdAtom::kCmdFwReportBegin);
  ...
  if (!fd) {
    DEJAVIEW_FATAL("Failed to dup fd when reporting to Android");
  }

  base::Uuid uuid(uuid_);
  DEJAVIEW_LAZY_LOAD(android_internal::ReportTrace, report_fn);
  DEJAVIEW_CHECK(report_fn(cfg.reporter_service_package().c_str(),
  ...
  LogUploadEvent(DejaViewStatsdAtom::kCmdFwReportHandoff);
}
...
// errors.
void DejaViewCmd::SaveOutputToIncidentTraceOrCrash() {
  LogUploadEvent(DejaViewStatsdAtom::kUploadIncidentBegin);
  ...
  DEJAVIEW_CHECK(unlink(kTempIncidentTracePath.c_str()) == 0 ||
                 errno == ENOENT);
  ...
  DEJAVIEW_CHECK(fflush(*trace_out_stream_) == 0);
  DEJAVIEW_CHECK(fseek(*trace_out_stream_, 0, SEEK_SET) == 0);
  ...
  for (;;) {
    errno = 0;
    DEJAVIEW_DCHECK(static_cast<size_t>(offset) + remaining == bytes_written_);
    auto wsize = DEJAVIEW_EINTR(sendfile(*staging_fd, fd, &offset, remaining));
    if (wsize < 0) {
      DEJAVIEW_FATAL("sendfile() failed wsize=%zd, off=%" PRId64
    ...
    if (now < start || (now - start).count() > kSendfileTimeoutNs) {
      DEJAVIEW_FATAL("sendfile() timed out wsize=%zd, off=%" PRId64
                     ", initial=%" PRIu64
  ...
  LogUploadEvent(DejaViewStatsdAtom::kUploadIncidentSuccess);
}

// static
base::ScopedFile DejaViewCmd::CreateUnlinkedTmpFile() {
  ...
  if (!fd)
    DEJAVIEW_PLOG("Could not create a temporary trace file in %s", kStateDir);
  return fd;
}
