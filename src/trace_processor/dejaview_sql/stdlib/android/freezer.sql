...
CREATE DEJAVIEW FUNCTION _extract_freezer_pid(name STRING)
RETURNS INT
AS
...
CREATE DEJAVIEW FUNCTION _pid_to_upid(
  -- Pid to convert from.
  pid INT,
...
CREATE DEJAVIEW FUNCTION _translate_unfreeze_reason(reason INT)
RETURNS STRING
AS
SELECT
  CASE
...
CREATE DEJAVIEW TABLE android_freezer_events (
