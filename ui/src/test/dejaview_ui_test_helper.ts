...
export class DejaViewTestHelper {
  ...
  async navigate(fragment: string): Promise<void> {
    ...
    await this.waitForDejaViewIdle();
  ...
  waitForDejaViewIdle(idleHysteresisMs?: number): Promise<void> {
    return this.page.evaluate(
      async (ms) =>
        (window as {} as IdleDetectorWindow).waitForDejaViewIdle(ms),
      idleHysteresisMs,
    );
  }
