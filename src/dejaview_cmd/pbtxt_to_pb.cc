...
#include "src/dejaview_cmd/config.descriptor.h"

#include "protos/dejaview/common/descriptor.gen.h"

namespace dejaview {
constexpr char kConfigProtoName[] = ".dejaview.protos.TraceConfig";
...
const char* FieldToTypeName(const FieldDescriptorProto* field) {
  ...
  // For gcc
  DEJAVIEW_FATAL("Non complete switch");
}
...
std::string Format(const char* fmt, std::map<std::string, std::string> args) {
  ...
  for (const auto& key_value : args) {
    ...
    DEJAVIEW_CHECK(start != std::string::npos);
    result.replace(start, key_value.first.size(), key_value.second);
    DEJAVIEW_CHECK(result.find(key_value.first) == std::string::npos);
  }
  return result;
}
...
class ParserDelegate {
 public:
  ...
  void StringField(Token key, Token value) {
    ...
    const auto& field_type = field->type();
    DEJAVIEW_CHECK(field_type == FieldDescriptorProto::TYPE_STRING ||
    ...
    for (size_t i = 0; i < value.size(); i++) {
      ...
      if (c == '\\') {
        if (i + 1 >= value.size()) {
          ...
          DEJAVIEW_FATAL("Escape at end of string.");
          return;
        }
        char next = txt[++i];
        switch (next) {
  ...
  void IdentifierField(Token key, Token value) {
    ...
    if (field_type == FieldDescriptorProto::TYPE_BOOL) {
      ...
      DEJAVIEW_CHECK(enum_descriptor);
  ...
  bool BeginNestedMessage(Token key, Token value) {
    ...
    DEJAVIEW_CHECK(nested_descriptor);
 ...
 private:
  ...
  void VarIntField(const FieldDescriptorProto* field, Token t) {
    uint32_t field_id = static_cast<uint32_t>(field->number());
    uint64_t n = 0;
    DEJAVIEW_CHECK(ParseInteger(t.txt, &n));
  ...
  bool ParseInteger(base::StringView s, T* number_ptr) {
    uint64_t n = 0;
    DEJAVIEW_CHECK(sscanf(s.ToStdString().c_str(), "%" PRIu64, &n) == 1);
    DEJAVIEW_CHECK(n <= std::numeric_limits<T>::max());
