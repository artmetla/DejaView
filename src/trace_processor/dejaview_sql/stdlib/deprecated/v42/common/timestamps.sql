...
CREATE DEJAVIEW FUNCTION is_spans_overlapping(
  ts1 LONG,
  ts_end1 LONG,
  ts2 LONG,
  ts_end2 LONG)
...
CREATE DEJAVIEW FUNCTION spans_overlapping_dur(
  ts1 LONG,
  dur1 LONG,
  ts2 LONG,
  dur2 LONG
...
CREATE DEJAVIEW FUNCTION ns(nanos INT)
RETURNS INT AS
SELECT time_from_ns($nanos);

CREATE DEJAVIEW FUNCTION us(micros INT)
RETURNS INT AS
SELECT time_from_us($micros);

CREATE DEJAVIEW FUNCTION ms(millis INT)
RETURNS INT AS
SELECT time_from_ms($millis);

CREATE DEJAVIEW FUNCTION seconds(seconds INT)
RETURNS INT AS
SELECT time_from_s($seconds);

CREATE DEJAVIEW FUNCTION minutes(minutes INT)
RETURNS INT AS
SELECT time_from_min($minutes);

CREATE DEJAVIEW FUNCTION hours(hours INT)
RETURNS INT AS
SELECT time_from_hours($hours);

CREATE DEJAVIEW FUNCTION days(days INT)
RETURNS INT AS
