...
CREATE DEJAVIEW VIEW scroll_with_browser_flows AS
SELECT
  scroll.trace_id,
  scroll.scroll_id,
  scroll.ts,
...
CREATE DEJAVIEW VIEW get_bitmap_calls AS
SELECT
  id,
  ts,
  dur,
...
CREATE DEJAVIEW VIEW toolbar_bitmaps AS
SELECT
  slice.id,
  slice.ts,
  slice.dur,
...
CREATE DEJAVIEW VIEW get_bitmaps_and_toolbar AS
SELECT
  bitmap.id AS id,
  bitmap.ts AS ts,
  bitmap.dur AS dur,
...
CREATE DEJAVIEW VIEW blocking_bitmap_tasks AS
SELECT
  scroll.scroll_id,
  scroll.trace_id,
  bitmap.id,
...
CREATE DEJAVIEW VIEW scroll_jank_cause_get_bitmap AS
SELECT
  scroll_id,
  trace_id,
