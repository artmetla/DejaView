...
const PLUGIN_ID = 'dev.dejaview.TimelineSync';
...
/**
 * Synchronizes the timeline of 2 or more dejaview traces.
 *
 ...
 */
class TimelineSync implements DejaViewPlugin {
  ...
  // Used when the url passes ?dev.dejaview.TimelineSync:enable to auto-enable
  ...
  onActivate(ctx: App): void {
    ctx.commands.registerCommand({
      id: `dev.dejaview.SplitScreen#enableTimelineSync`,
      name: 'Enable timeline sync with other DejaView UI tabs',
      callback: () => this.showTimelineSyncDialog(),
    });
    ctx.commands.registerCommand({
      id: `dev.dejaview.SplitScreen#disableTimelineSync`,
      name: 'Disable timeline sync',
      callback: () => this.disableTimelineSync(this._sessionId),
    });
    ctx.commands.registerCommand({
      id: `dev.dejaview.SplitScreen#toggleTimelineSync`,
      name: 'Toggle timeline sync with other DejaViewUI tabs',
    ...
    const m = /dev.dejaview.TimelineSync:enable(=\d+)?/.exec(location.hash);
    if (m !== null) {
  ...
  private onmessage(msg: MessageEvent) {
    ...
    if (!('dejaviewSync' in msg.data)) return;
    const msgData = msg.data as SyncMessage;
    const sync = msgData.dejaviewSync;
    switch (sync.cmd) {
      case 'MSG_ADVERTISE':
