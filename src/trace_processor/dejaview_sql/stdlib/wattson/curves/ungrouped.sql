...
INCLUDE DEJAVIEW MODULE wattson.cpu_split;
...
CREATE DEJAVIEW TABLE _w_independent_cpus_calc
AS
SELECT
  base.ts,
  base.dur,
...
CREATE DEJAVIEW TABLE _get_max_vote
AS
...
CREATE DEJAVIEW TABLE _system_state_curves
AS
SELECT
  base.ts,
  base.dur,
...
-- system state basis
CREATE DEJAVIEW TABLE _system_state_mw
AS
SELECT
  ts,
  dur,
