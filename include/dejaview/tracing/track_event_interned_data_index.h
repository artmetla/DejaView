...
#ifndef INCLUDE_DEJAVIEW_TRACING_TRACK_EVENT_INTERNED_DATA_INDEX_H_
#define INCLUDE_DEJAVIEW_TRACING_TRACK_EVENT_INTERNED_DATA_INDEX_H_
...
//
//   static void Add(dejaview::protos::pbzero::InternedData*,
//                   size_t iid,
...
class TrackEventInternedDataIndex
 ...
 public:
  ...
  static size_t Get(internal::TrackEventIncrementalState* incremental_state,
    ...
    size_t iid;
    if (DEJAVIEW_LIKELY(index_for_field->index_.LookUpOrInsert(&iid, value))) {
      DEJAVIEW_DCHECK(iid);
      return iid;
    }
  ...
  static InternedDataType* GetOrCreateIndexForField(
    ...
    for (const auto& entry : incremental_state->interned_data_indices) {
      if (entry.first == FieldNumber) {
        ...
        if (strcmp(DEJAVIEW_DEBUG_FUNCTION_IDENTIFIER(),
                   entry.second->type_id_)) {
          DEJAVIEW_FATAL(
              "Interned data accessed under different types! Previous type: "
              "%s. New type: %s.",
              entry.second->type_id_, DEJAVIEW_DEBUG_FUNCTION_IDENTIFIER());
        }
...
#endif  // INCLUDE_DEJAVIEW_TRACING_TRACK_EVENT_INTERNED_DATA_INDEX_H_
