...
#ifndef INCLUDE_DEJAVIEW_EXT_BASE_THREADING_CHANNEL_H_
#define INCLUDE_DEJAVIEW_EXT_BASE_THREADING_CHANNEL_H_
...
class Channel {
 public:
  ...
  explicit Channel(uint32_t capacity_hint) : elements_(capacity_hint) {
    DEJAVIEW_DCHECK(capacity_hint > 0);
  ...
  DEJAVIEW_WARN_UNUSED_RESULT ReadResult ReadNonBlocking() {
  ...
  DEJAVIEW_WARN_UNUSED_RESULT WriteResult WriteNonBlocking(T&& element) {
    std::lock_guard<std::mutex> lock(mutex_);
    if (is_closed_) {
...
#endif  // INCLUDE_DEJAVIEW_EXT_BASE_THREADING_CHANNEL_H_
