...
CREATE DEJAVIEW TABLE _binder_txn_merged AS
...
CREATE DEJAVIEW TABLE _oom_score AS
  SELECT
    process.upid,
...
CREATE DEJAVIEW INDEX _oom_score_idx ON _oom_score(upid, ts);
...
CREATE DEJAVIEW VIEW _sync_binder_metrics_by_txn AS
...
-- is server side.
CREATE DEJAVIEW VIEW android_sync_binder_thread_state_by_txn(
...
-- row is on the client or server side. 'binder_txn' is client side and 'binder_reply'
-- is server side.
CREATE DEJAVIEW VIEW android_sync_binder_blocked_functions_by_txn(
  -- slice id of the binder txn
  binder_txn_id INT,
  -- Client ts
  client_ts INT,
...
CREATE DEJAVIEW TABLE _async_binder_reply AS
...
CREATE DEJAVIEW TABLE _binder_async_txn_raw AS
SELECT
  slice.id AS binder_txn_id,
...
CREATE DEJAVIEW TABLE _binder_async_txn AS
SELECT
...
CREATE DEJAVIEW VIEW _async_binder_metrics_by_txn AS
SELECT binder.*, client_oom.value AS client_oom_score, server_oom.value AS server_oom_score
...
-- It returns data about the client and server ends of every binder transaction async.
CREATE DEJAVIEW TABLE android_binder_txns(
...
CREATE DEJAVIEW FUNCTION android_binder_outgoing_graph(
...
-- The weights of each node represent the wall execution time in the server_process.
CREATE DEJAVIEW FUNCTION android_binder_incoming_graph(
...
-- The weights of each node represent the wall execution time in the server_process.
CREATE DEJAVIEW FUNCTION android_binder_graph(
