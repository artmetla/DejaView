...
#include "src/trace_processor/dejaview_sql/preprocessor/preprocessor_grammar_interface.h"
...
using MacroImpl =
    std::variant<DejaViewSqlPreprocessor::Macro*, Stringify, Apply>;
...
extern "C" struct PreprocessorGrammarState {
  ...
  const base::FlatHashMap<std::string, DejaViewSqlPreprocessor::Macro>& macros;
...
void ExecuteSqlMacro(State* state,
                     ...
                     SqliteTokenizer::Token name,
                     SqliteTokenizer::Token rp) {
  auto& sql_macro = std::get<DejaViewSqlPreprocessor::Macro*>(macro.impl);
...
extern "C" void OnPreprocessorMacroEnd(State* state,
  ...
  switch (macro.impl.index()) {
    case base::variant_index<MacroImpl, DejaViewSqlPreprocessor::Macro*>():
    ...
    case base::variant_index<MacroImpl, Apply>():
      ...
      break;
    default:
      DEJAVIEW_FATAL("Unknown variant type");
  }
...
extern "C" void OnPreprocessorEnd(State* state) {
  auto& frame = state->stack.back();
  DEJAVIEW_CHECK(!frame.active_macro);
  ...
  switch (frame.type.index()) {
    ...
    case base::variant_index<Frame::Type, Frame::Root>():
      break;
    default:
      DEJAVIEW_FATAL("Unknown frame type");
  }
}

}  // namespace

DejaViewSqlPreprocessor::DejaViewSqlPreprocessor(
    SqlSource source,
    const base::FlatHashMap<std::string, Macro>& macros)
    : global_tokenizer_(std::move(source)), macros_(&macros) {}

bool DejaViewSqlPreprocessor::NextStatement() {
  ...
  if (tok.IsTerminal()) {
    DEJAVIEW_DCHECK(tok.token_type != TK_SEMI);
    return false;
  }
