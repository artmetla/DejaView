...
CREATE DEJAVIEW TABLE _input_message_sent
AS
SELECT
...
CREATE DEJAVIEW TABLE _input_message_received
AS
SELECT
  STR_SPLIT(STR_SPLIT(slice.name, '=', 3), ')', 0) AS event_type,
  STR_SPLIT(STR_SPLIT(slice.name, '=', 2), ',', 0) AS event_seq,
...
CREATE DEJAVIEW TABLE _input_read_time
AS
SELECT
  name,
...
CREATE DEJAVIEW TABLE _event_seq_to_input_event_id
AS
SELECT
...
CREATE DEJAVIEW TABLE _input_event_id_to_android_frame
AS
SELECT
...
CREATE DEJAVIEW TABLE _app_frame_to_surface_flinger_frame
AS
SELECT
...
CREATE DEJAVIEW TABLE _first_non_dropped_frame_after_input
AS
SELECT
...
CREATE DEJAVIEW TABLE android_input_events (
...
CREATE DEJAVIEW VIEW android_key_events(
...
CREATE DEJAVIEW VIEW android_motion_events(
  -- ID of the trace entry
  id INT,
  -- The randomly-generated ID associated with each input event processed
  -- by Android Framework, used to track the event through the input pipeline
...
CREATE DEJAVIEW VIEW android_input_event_dispatch(
  -- ID of the trace entry
  id INT,
