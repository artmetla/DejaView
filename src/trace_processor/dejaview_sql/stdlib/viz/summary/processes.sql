...
CREATE DEJAVIEW TABLE _process_track_summary AS
SELECT upid, SUM(cnt) AS slice_count
FROM process_track
JOIN _slice_track_summary USING (id)
GROUP BY upid;

CREATE DEJAVIEW TABLE _heap_profile_allocation_summary AS
SELECT upid, COUNT() AS allocation_count
FROM heap_profile_allocation
GROUP BY upid;

CREATE DEJAVIEW TABLE _heap_profile_graph_summary AS
SELECT upid, COUNT() AS graph_object_count
FROM heap_graph_object
GROUP BY upid;

CREATE DEJAVIEW TABLE _thread_process_grouped_summary AS
SELECT
  upid,
...
CREATE DEJAVIEW TABLE _process_available_info_summary AS
WITH r AS (
  SELECT
    upid,
