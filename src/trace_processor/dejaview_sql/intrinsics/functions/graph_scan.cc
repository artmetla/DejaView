...
#include "src/trace_processor/dejaview_sql/intrinsics/types/value.h"
...
base::Status InitToOutputAndStepTable(const dejaview_sql::RowDataframe& inits,
                                      const dejaview_sql::Graph& graph,
  ...
  for (uint32_t i = 0; i < inits.size(); ++i) {
    ...
    for (uint32_t j = 1; j < inits.column_names.size(); ++j) {
      switch (cell[j].index()) {
        ...
        case dejaview_sql::ValueIndex<double>(): {
        ...
        case dejaview_sql::ValueIndex<std::string>(): {
...
struct GraphAggregatingScanner {
  ...
  StringPool* pool;
  const dejaview_sql::Graph& graph;
  const dejaview_sql::RowDataframe& inits;
  ...
  while (!stack.empty()) {
    ...
    switch (source.visit_state) {
      ...
      case NodeState::kWaitingForDescendants:
        ...
        for (uint32_t dest_id : GetEdges(source_id)) {
          DEJAVIEW_DCHECK(state[dest_id].visit_state == NodeState::kDone);
...
base::StatusOr<std::unique_ptr<RuntimeTable>> GraphAggregatingScanner::Run() {
  ...
  //      query and also CREATE DEJAVIEW TABLE: the code here is very similar to
  //      the code in DejaViewSqlEngine.
...
struct GraphAggregatingScan : public SqliteFunction<GraphAggregatingScan> {
  ...
  static void Step(sqlite3_context* ctx, int argc, sqlite3_value** argv) {
    ...
    const auto* init = sqlite::value::Pointer<dejaview_sql::RowDataframe>(
    ...
    const auto* nodes =
        sqlite::value::Pointer<dejaview_sql::Graph>(argv[0], "GRAPH");
    GraphAggregatingScanner scanner{
        user_data->engine,
        user_data->pool,
        nodes ? *nodes : dejaview_sql::Graph(),
        *init,
        reduce,
        {},
        {},
...
struct GraphScan : public SqliteFunction<GraphScan> {
  ...
  static void Step(sqlite3_context* ctx, int argc, sqlite3_value** argv) {
    ...
    const auto* raw_graph =
        sqlite::value::Pointer<dejaview_sql::Graph>(argv[0], "GRAPH");
    const auto& graph = raw_graph ? *raw_graph : dejaview_sql::Graph();
...
base::Status RegisterGraphScanFunctions(DejaViewSqlEngine& engine,
                                        StringPool* pool) {
