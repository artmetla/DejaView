...
#ifndef INCLUDE_DEJAVIEW_PROTOZERO_PACKED_REPEATED_FIELDS_H_
#define INCLUDE_DEJAVIEW_PROTOZERO_PACKED_REPEATED_FIELDS_H_
...
class PackedBufferBase {
 ...
 protected:
  void GrowIfNeeded() {
    DEJAVIEW_DCHECK(write_ptr_ >= storage_begin_ && write_ptr_ <= storage_end_);
    if (DEJAVIEW_UNLIKELY(write_ptr_ + kMaxElementSize > storage_end_)) {
      GrowSlowpath();
    }
  }
...
class PackedFixedSizeInt : public PackedBufferBase {
 public:
  void Append(T value) {
    ...
    GrowIfNeeded();
    DEJAVIEW_DCHECK(reinterpret_cast<size_t>(write_ptr_) % alignof(T) == 0);
...
#endif  // INCLUDE_DEJAVIEW_PROTOZERO_PACKED_REPEATED_FIELDS_H_
