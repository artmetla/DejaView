...
CREATE DEJAVIEW VIEW android_surfaceflinger_event AS
SELECT
  'slice' AS track_type,
...
CREATE DEJAVIEW VIEW surfaceflinger_track AS
...
CREATE DEJAVIEW VIEW gpu_waiting_start AS
SELECT
...
CREATE DEJAVIEW VIEW gpu_waiting_end AS
SELECT
  CAST(SUBSTR(s.name, 28) AS UINT32) AS fence_id,
  dur,
  ts + dur AS end_ts
...
CREATE DEJAVIEW VIEW gpu_waiting_span AS
SELECT
  fence_id,
  ts,
  dur
...
CREATE DEJAVIEW VIEW display_ids AS
...
CREATE DEJAVIEW VIEW metrics_per_display AS
...
CREATE DEJAVIEW VIEW android_surfaceflinger_output AS
SELECT
