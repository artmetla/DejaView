...
#ifndef SRC_DEJAVIEW_CMD_DEJAVIEW_CMD_H_
#define SRC_DEJAVIEW_CMD_DEJAVIEW_CMD_H_
...
class DejaViewCmd : public Consumer {
 public:
  DejaViewCmd();
  ~DejaViewCmd() override;
 ...
 private:
...
#endif
  void LogUploadEvent(DejaViewStatsdAtom atom);
  void LogUploadEvent(DejaViewStatsdAtom atom, const std::string& trigger_name);
  void LogTriggerEvents(DejaViewTriggerAtom atom,
                        const std::vector<std::string>& trigger_names);

  base::UnixTaskRunner task_runner_;

  std::unique_ptr<RateLimiter> limiter_;
  std::unique_ptr<dejaview::TracingService::ConsumerEndpoint>
      consumer_endpoint_;
  ...
  // The aux threads used to invoke secondary instances of DejaViewCmd to create
  ...
  base::WeakPtrFactory<DejaViewCmd> weak_factory_{this};
};

}  // namespace dejaview

#endif  // SRC_DEJAVIEW_CMD_DEJAVIEW_CMD_H_
