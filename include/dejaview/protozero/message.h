...
#ifndef INCLUDE_DEJAVIEW_PROTOZERO_MESSAGE_H_
#define INCLUDE_DEJAVIEW_PROTOZERO_MESSAGE_H_
...
class DEJAVIEW_EXPORT_COMPONENT Message {
 public:
  ...
  void AppendTinyVarInt(uint32_t field_id, int32_t value) {
    DEJAVIEW_DCHECK(0 <= value && value < 0x80);
    if (nested_message_)
      EndNestedMessage();
 ...
 private:
  ...
  void WriteToStream(const uint8_t* src_begin, const uint8_t* src_end) {
    ...
    DEJAVIEW_DCHECK(src_begin <= src_end);
...
#endif  // INCLUDE_DEJAVIEW_PROTOZERO_MESSAGE_H_
