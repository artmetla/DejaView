...
-- in BTP.
INCLUDE DEJAVIEW MODULE chrome.metadata;
INCLUDE DEJAVIEW MODULE chrome.scroll_jank.scroll_jank_v3_cause;
INCLUDE DEJAVIEW MODULE chrome.scroll_jank.utils;

-- Given a slice id, returns the name of the slice.
CREATE DEJAVIEW FUNCTION _slice_name_from_id(
...
CREATE DEJAVIEW TABLE chrome_gesture_scroll_updates(
...
CREATE DEJAVIEW TABLE _presented_gesture_scrolls AS
SELECT
  id,
  ts,
  dur,
...
CREATE DEJAVIEW TABLE chrome_presented_gesture_scrolls(
...
-- prediction.
CREATE DEJAVIEW TABLE chrome_scroll_updates_with_deltas(
  -- The id of the scroll update event.
  scroll_update_id INT,
...
CREATE DEJAVIEW TABLE chrome_full_frame_view(
  -- ID of the frame.
  id INT,
...
CREATE DEJAVIEW TABLE chrome_full_frame_delta_view(
  -- ID of the frame.
  id INT,
  -- Start timestamp of the frame.
  ts INT,
...
-- a single row.
CREATE DEJAVIEW TABLE chrome_merged_frame_view(
  -- ID of the frame.
  id INT,
  -- The timestamp of the last presented input.
  max_start_ts INT,
...
CREATE DEJAVIEW TABLE chrome_frame_info_with_delay(
...
CREATE DEJAVIEW TABLE chrome_vsyncs(
...
CREATE DEJAVIEW TABLE chrome_janky_frames_no_cause(
...
CREATE DEJAVIEW TABLE chrome_janky_frames_no_subcause(
  -- Time elapsed since the previous frame was presented, will be more than |VSYNC| in this view.
  delay_since_last_frame DOUBLE,
  -- Event latency id of the presented frame.
  event_latency_id INT,
...
CREATE DEJAVIEW TABLE chrome_janky_frames(
...
CREATE DEJAVIEW TABLE chrome_unique_frame_presentation_ts(
...
CREATE DEJAVIEW TABLE chrome_janky_frames_percentage(
...
CREATE DEJAVIEW TABLE chrome_frames_per_scroll(
  -- The ID of the scroll.
  scroll_id INT,
...
CREATE DEJAVIEW VIEW chrome_causes_per_scroll(
  -- The ID of the scroll.
  scroll_id INT,
