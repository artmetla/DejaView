...
from dejaview.bigtrace.protos.dejaview.bigtrace.orchestrator_pb2 import BigtraceQueryArgs
from dejaview.bigtrace.protos.dejaview.bigtrace.orchestrator_pb2_grpc import BigtraceOrchestratorStub
...
class Bigtrace:
  ...
  def query(self, traces: List[str], sql_query: str):
    if not traces:
      raise DejaViewException("Trace list cannot be empty")
    if not sql_query:
      raise DejaViewException("SQL query cannot be empty")
    ...
    try:
      ...
      raise DejaViewException(f"gRPC {e.code().name} error - {e.details()}")
