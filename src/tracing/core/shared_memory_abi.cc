...
std::array<uint16_t, SharedMemoryABI::kNumPageLayouts> InitChunkSizes(
  ...
  for (size_t i = 0; i < SharedMemoryABI::kNumPageLayouts; i++) {
    ...
    DEJAVIEW_CHECK(size <= std::numeric_limits<uint16_t>::max());
...
void SharedMemoryABI::Initialize(uint8_t* start,
  ...
  DEJAVIEW_CHECK(chunk_header.chunk_id.load() == kMaxChunkID);

  chunk_header.writer_id.store(static_cast<uint16_t>(-1));
  DEJAVIEW_CHECK(kMaxWriterID <= chunk_header.writer_id.load());

  DEJAVIEW_CHECK(page_size >= kMinPageSize);
  DEJAVIEW_CHECK(page_size <= kMaxPageSize);
  DEJAVIEW_CHECK(page_size % kMinPageSize == 0);
  DEJAVIEW_CHECK(reinterpret_cast<uintptr_t>(start) % kMinPageSize == 0);
  DEJAVIEW_CHECK(size % page_size == 0);
}
...
SharedMemoryABI::Chunk SharedMemoryABI::GetChunkUnchecked(size_t page_idx,
  ...
  DEJAVIEW_DCHECK(chunk_idx < num_chunks);
...
SharedMemoryABI::Chunk SharedMemoryABI::TryAcquireChunk(
  ...
  DEJAVIEW_DCHECK(desired_chunk_state == kChunkBeingRead ||
...
bool SharedMemoryABI::TryPartitionPage(size_t page_idx, PageLayout layout) {
  DEJAVIEW_DCHECK(layout >= kPageDiv1 && layout <= kPageDiv14);
...
size_t SharedMemoryABI::ReleaseChunk(Chunk chunk,
                                     ChunkState desired_chunk_state) {
  DEJAVIEW_DCHECK(desired_chunk_state == kChunkComplete ||
  ...
  for (int attempt = 0; attempt < kRetryAttempts; attempt++) {
    ...
    DEJAVIEW_CHECK(chunk.size() == page_chunk_size);
    ...
    DEJAVIEW_CHECK(chunk_state == expected_chunk_state);
    uint32_t next_layout = layout;
    next_layout &= ~(kChunkMask << (chunk_idx * kChunkShift));
    next_layout |= (desired_chunk_state << (chunk_idx * kChunkShift));
  ...
  DEJAVIEW_DFATAL("Too much contention on page.");
...
SharedMemoryABI::Chunk::Chunk(uint8_t* begin, uint16_t size, uint8_t chunk_idx)
  ...
  DEJAVIEW_CHECK(reinterpret_cast<uintptr_t>(begin) % kChunkAlignment == 0);
  DEJAVIEW_CHECK(size > 0);
}
...
std::pair<size_t, size_t> SharedMemoryABI::GetPageAndChunkIndex(
  ...
  DEJAVIEW_DCHECK(chunk.begin() >= start_);
  DEJAVIEW_DCHECK(chunk.end() <= start_ + size_);
  ...
  DEJAVIEW_DCHECK(offset >= sizeof(PageHeader));
  DEJAVIEW_DCHECK(offset % kChunkAlignment == 0);
  DEJAVIEW_DCHECK((offset - sizeof(PageHeader)) % chunk.size() == 0);
  const size_t chunk_idx = (offset - sizeof(PageHeader)) / chunk.size();
  DEJAVIEW_DCHECK(chunk_idx < kMaxChunksPerPage);
  DEJAVIEW_DCHECK(chunk_idx < GetNumChunksForLayout(GetPageLayout(page_idx)));
