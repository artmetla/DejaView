...
#ifndef INCLUDE_DEJAVIEW_TRACING_INTERNAL_TRACK_EVENT_DATA_SOURCE_H_
#define INCLUDE_DEJAVIEW_TRACING_INTERNAL_TRACK_EVENT_DATA_SOURCE_H_
...
class StopArgsImpl : public DataSourceBase::StopArgs {
 public:
  ...
  // the stop signal.
  // See the comment in include/dejaview/tracing/data_source.h for more info.
...
// dejaview::TraceTimestamp struct. By specialising this template and defining
...
// dejaview.protos.ClockSnapshot. However, custom clock IDs (>=64) are
...
template <
    ...
    typename CanBeConvertedToNsCheck =
        decltype(::dejaview::TraceTimestampTraits<typename base::remove_cvref_t<
...
struct IsValidEventNameType
    : std::is_same<dejaview::DynamicString, typename std::decay<T>::type> {};
...
inline ::dejaview::DynamicString DecayEventNameType(
    ::dejaview::DynamicString name) {
  return name;
}

inline ::dejaview::StaticString DecayEventNameType(
    ::dejaview::StaticString name) {
  return name;
}

// Convert all static strings of different length to StaticString to avoid
// unnecessary template instantiations.
inline ::dejaview::StaticString DecayEventNameType(const char* name) {
  return ::dejaview::StaticString{name};
}
...
struct CategoryTraits {
  ...
  static size_t GetStaticIndex(const CategoryType&) {
    DEJAVIEW_DCHECK(false);  // Not reached.
...
class TrackEventDataSource
 ...
 public:
  ...
  static void CallIfCategoryEnabled(size_t category_index,
                                    Callback callback) DEJAVIEW_ALWAYS_INLINE {
  ...
  static void TraceForCategory(uint32_t instances,
                               ...
                               Arguments&&... args) DEJAVIEW_ALWAYS_INLINE {
  ...
  static void SetTrackDescriptor(const Track& track,
    ...
    DEJAVIEW_DCHECK(track.uuid == desc.uuid());
 ...
 private:
  ...
  static void TraceForCategoryBody(
      ...
      const EventNameType& event_name,
      dejaview::protos::pbzero::TrackEvent::Type type,
      Arguments&&... args) DEJAVIEW_NO_INLINE {
  ...
  static void TraceForCategoryBody(
      ...
      dejaview::protos::pbzero::TrackEvent::Type type,
      CounterTrack track,
      ValueType value) DEJAVIEW_ALWAYS_INLINE {
    DEJAVIEW_DCHECK(type == dejaview::protos::pbzero::TrackEvent::TYPE_COUNTER);
  ...
  static void TraceForCategoryBody(
      ...
      CounterTrack track,
      TimestampType timestamp,
      ValueType value) DEJAVIEW_NO_INLINE {
    DEJAVIEW_DCHECK(type == dejaview::protos::pbzero::TrackEvent::TYPE_COUNTER);
        ...
        [&](EventContext event_ctx) {
          if (std::is_integral<ValueType>::value) {
            ...
            if (track.is_incremental()) {
              ...
              DEJAVIEW_DCHECK(incr_state != nullptr);
              auto prv_value =
...
// folded by the linker.
  ...
  static void TraceForCategoryLegacyBody(
    ...
    TraceForCategoryImplNoTimestamp(
        ...
        [&](dejaview::EventContext ctx) DEJAVIEW_NO_THREAD_SAFETY_ANALYSIS {
          using ::dejaview::internal::TrackEventLegacy;
  ...
  static void TraceForCategoryLegacyWithIdBody(
    ...
    TraceForCategoryImplNoTimestamp(
        ...
        [&](dejaview::EventContext ctx) DEJAVIEW_NO_THREAD_SAFETY_ANALYSIS {
          using ::dejaview::internal::TrackEventLegacy;
          ::dejaview::internal::LegacyTraceId trace_id{legacy_id};
  ...
  static dejaview::EventContext WriteTrackEventImpl(
      typename Base::TraceContext& ctx,
      const CategoryType& category,
      const EventNameType& event_name,
      dejaview::protos::pbzero::TrackEvent::Type type,
      const TrackType& track,
      const TraceTimestamp& trace_timestamp) DEJAVIEW_ALWAYS_INLINE {
  ...
  template <typename CategoryType,
            ...
            typename TrackTypeCheck =
                typename std::enable_if<IsValidTrack<TrackType>()>::type>
  static dejaview::EventContext WriteTrackEvent(
      typename Base::TraceContext& ctx,
      const CategoryType& category,
      const EventNameType& event_name,
      dejaview::protos::pbzero::TrackEvent::Type type,
      const TrackType& track,
      const TimestampType& timestamp) DEJAVIEW_NO_INLINE {
    TraceTimestamp trace_timestamp = ::dejaview::TraceTimestampTraits<
  ...
  static dejaview::EventContext WriteTrackEvent(
      ...
      const EventNameType& event_name,
      dejaview::protos::pbzero::TrackEvent::Type type,
      const TrackType& track) DEJAVIEW_NO_INLINE {
  ...
  static void TraceWithInstances(uint32_t instances,
                                 const CategoryType& category,
                                 Lambda lambda) DEJAVIEW_ALWAYS_INLINE {
    using CatTraits = CategoryTraits<CategoryType>;
    if (CatTraits::kIsDynamic) {
...
#endif  // INCLUDE_DEJAVIEW_TRACING_INTERNAL_TRACK_EVENT_DATA_SOURCE_H_
