...
#ifndef INCLUDE_DEJAVIEW_PROTOZERO_MESSAGE_ARENA_H_
#define INCLUDE_DEJAVIEW_PROTOZERO_MESSAGE_ARENA_H_
...
class DEJAVIEW_EXPORT_COMPONENT MessageArena {
 public:
  MessageArena();
  ~MessageArena();
  ...
  void DeleteLastMessage(Message* msg) {
    DEJAVIEW_DCHECK(!blocks_.empty() && blocks_.front().entries > 0);
    DEJAVIEW_DCHECK(&blocks_.front().storage[blocks_.front().entries - 1] ==
  ...
  void Reset() {
    DEJAVIEW_DCHECK(!blocks_.empty());
    blocks_.resize(1);
    auto& block = blocks_.front();
    block.entries = 0;
    DEJAVIEW_ASAN_POISON(block.storage, sizeof(block.storage));
  }
 ...
 private:
  ...
  struct Block {
    ...
    Block() { DEJAVIEW_ASAN_POISON(storage, sizeof(storage)); }
...
#endif  // INCLUDE_DEJAVIEW_PROTOZERO_MESSAGE_ARENA_H_
