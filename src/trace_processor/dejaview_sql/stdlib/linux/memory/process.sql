...
INCLUDE DEJAVIEW MODULE linux.memory.general;

-- All memory counters tables.

CREATE DEJAVIEW VIEW _anon_rss AS
SELECT
  ts,
  dur,
  upid,
...
CREATE DEJAVIEW VIEW _file_rss AS
SELECT
  ts,
  dur,
  upid,
...
CREATE DEJAVIEW VIEW _shmem_rss AS
SELECT
  ts,
  dur,
  upid,
...
CREATE DEJAVIEW TABLE _memory_rss_and_swap_per_process_table AS
SELECT
  ts, dur, upid,
...
CREATE DEJAVIEW VIEW memory_rss_and_swap_per_process(
  -- Timestamp
  ts INT,
  -- Duration
  dur INT,
