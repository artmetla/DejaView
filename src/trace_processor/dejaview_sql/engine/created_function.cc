...
base::Status CheckNoMoreRows(sqlite3_stmt* stmt,
  ...
  DEJAVIEW_DCHECK(ret == SQLITE_DONE);
...
base::StatusOr<SqlValue> EvaluateScalarStatement(
  ...
  DEJAVIEW_DCHECK(ret == SQLITE_ROW);
  ...
  if (result.type == SqlValue::kBytes && result.bytes_value == nullptr) {
    DEJAVIEW_DCHECK(result.bytes_count == 0);
    result.bytes_value = "";
  }
...
class RecursiveCallUnroller {
 public:
  RecursiveCallUnroller(DejaViewSqlEngine* engine,
                        sqlite3_stmt* stmt,
                        const FunctionPrototype& prototype,
                        Memoizer& memoizer)
      : engine_(engine),
  ...
  base::Status Run(Memoizer::MemoizedArgs initial_args) {
    DEJAVIEW_TP_TRACE(metatrace::Category::FUNCTION_CALL,
    ...
    while (!first_pass_.empty() || !second_pass_.empty()) {
      ...
      DEJAVIEW_TP_TRACE(metatrace::Category::FUNCTION_CALL, "SQL_FUNCTION_CALL",
                        [&](metatrace::Record* r) {
                          r->AddArg("Function", prototype_.function_name);
...
class State : public CreatedFunction::Context {
 public:
  explicit State(DejaViewSqlEngine* engine) : engine_(engine) {}
  ~State() override;
  ...
  void Reset(FunctionPrototype prototype,
    ...
    DEJAVIEW_DCHECK(!is_valid_);
    DEJAVIEW_DCHECK(stmts_.empty());
  ...
  DejaViewSqlEngine* engine() const { return engine_; }
 ...
 private:
  ...
  // DejaView SQL functions support recursion. Given that each function call in
...
void CreatedFunction::Reset(Context* ctx, DejaViewSqlEngine* engine) {
  ctx->~Context();
