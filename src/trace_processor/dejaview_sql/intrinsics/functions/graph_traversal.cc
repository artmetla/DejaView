...
#include "src/trace_processor/dejaview_sql/intrinsics/functions/tables_py.h"
...
struct Dfs : public SqliteAggregateFunction<Dfs> {
  ...
  static void Step(sqlite3_context* ctx, int argc, sqlite3_value** argv) {
    ...
    auto* start_ids =
        sqlite::value::Pointer<dejaview_sql::IntArray>(argv[1], "ARRAY<LONG>");
    if (!start_ids) {
      return sqlite::result::UniquePointer(ctx, std::move(table), "TABLE");
    }
    DEJAVIEW_DCHECK(!start_ids->empty());
