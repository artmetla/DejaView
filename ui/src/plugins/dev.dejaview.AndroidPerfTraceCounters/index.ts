...
class AndroidPerfTraceCounters implements DejaViewPlugin {
  async onTraceLoad(ctx: Trace): Promise<void> {
    const resp = await ctx.engine.query(PERF_TRACE_COUNTERS_PRECONDITION);
    if (resp.numRows() === 0) return;
    ctx.commands.registerCommand({
      id: 'dev.dejaview.AndroidPerfTraceCounters#ThreadRuntimeIPC',
...
export const plugin: PluginDescriptor = {
  pluginId: 'dev.dejaview.AndroidPerfTraceCounters',
