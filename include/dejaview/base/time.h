...
#ifndef INCLUDE_DEJAVIEW_BASE_TIME_H_
#define INCLUDE_DEJAVIEW_BASE_TIME_H_
...
inline TimeNanos GetWallTimeNs() {
  ...
  DEJAVIEW_CHECK(!__builtin_umulll_overflow(nanoseconds, timebase_info.numer,
                                            &nanoseconds));
  ...
  DEJAVIEW_CHECK(!__builtin_uaddll_overflow(
...
inline TimeNanos GetThreadCPUTimeNs() {
  ...
  if (kr != KERN_SUCCESS) {
    DEJAVIEW_DFATAL("Failed to get CPU time.");
    return TimeNanos(0);
  }
...
inline TimeNanos GetThreadCPUTimeNs() {
  struct timespec ts = {};
  DEJAVIEW_CHECK(clock_gettime(CLOCK_THREAD_CPUTIME_ID, &ts) == 0);
  return FromPosixTimespec(ts);
}
#endif

#elif DEJAVIEW_BUILDFLAG(DEJAVIEW_OS_WASM)
...
inline TimeNanos GetTimeInternalNs(clockid_t clk_id) {
  struct timespec ts = {};
  DEJAVIEW_CHECK(clock_gettime(clk_id, &ts) == 0);
  return FromPosixTimespec(ts);
}
...
inline int64_t TimeGm(struct tm* tms) {
  ...
  if (tms)  // Kinda if (true), but avoids "mark as noreturn" errors.
    DEJAVIEW_FATAL("timegm not supported");
  return -1;
#else
...
inline int64_t MkTime(int year, int month, int day, int h, int m, int s) {
  DEJAVIEW_DCHECK(year >= 1900);
  DEJAVIEW_DCHECK(month > 0 && month <= 12);
  DEJAVIEW_DCHECK(day > 0 && day <= 31);
  struct tm tms {};
...
#endif  // INCLUDE_DEJAVIEW_BASE_TIME_H_
