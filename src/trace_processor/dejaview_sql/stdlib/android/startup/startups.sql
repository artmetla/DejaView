...
INCLUDE DEJAVIEW MODULE android.startup.startups_maxsdk28;
INCLUDE DEJAVIEW MODULE android.startup.startups_minsdk29;
INCLUDE DEJAVIEW MODULE android.startup.startups_minsdk33;
INCLUDE DEJAVIEW MODULE android.version;
...
CREATE DEJAVIEW TABLE android_startups(
  -- Startup id.
  startup_id INT,
...
CREATE DEJAVIEW TABLE _startup_indicator_slices AS
SELECT ts, name, track_id
FROM slice
WHERE name IN ('bindApplication', 'activityStart', 'activityResume');

CREATE DEJAVIEW FUNCTION _startup_indicator_slice_count(start_ts LONG,
                                                                end_ts LONG,
                                                                utid INT,
                                                                name STRING)
RETURNS INT AS
...
CREATE DEJAVIEW TABLE android_startup_processes(
  -- Startup id.
  startup_id INT,
...
-- activity start.
CREATE DEJAVIEW VIEW android_startup_threads(
  -- Startup id.
  startup_id INT,
  -- Timestamp of start.
  ts INT,
...
CREATE DEJAVIEW VIEW android_thread_slices_for_all_startups(
...
CREATE DEJAVIEW FUNCTION android_slices_for_startup_and_slice_name(
  -- Startup id.
  startup_id INT,
  -- Glob of the slice.
  slice_name STRING)
...
CREATE DEJAVIEW FUNCTION android_binder_transaction_slices_for_startup(
  -- Startup id.
  startup_id INT,
...
CREATE DEJAVIEW FUNCTION android_sum_dur_for_startup_and_slice(
  -- Startup id.
  startup_id LONG,
  -- Slice name.
  slice_name STRING)
...
CREATE DEJAVIEW FUNCTION android_sum_dur_on_main_thread_for_startup_and_slice(
  -- Startup id.
  startup_id LONG,
  -- Slice name.
  slice_name STRING)
