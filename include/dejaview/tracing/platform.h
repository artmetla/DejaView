...
#ifndef INCLUDE_DEJAVIEW_TRACING_PLATFORM_H_
#define INCLUDE_DEJAVIEW_TRACING_PLATFORM_H_
...
// primitives that cannot be implemented by the dejaview codebase and must be
...
class DEJAVIEW_EXPORT_COMPONENT PlatformThreadLocalObject {
 public:
  // Implemented by dejaview internal code. The embedder must call this when
...
class DEJAVIEW_EXPORT_COMPONENT Platform {
 public:
  ...
  static void SetCurrentProcessId(base::PlatformProcessId process_id) {
    DEJAVIEW_CHECK(!process_id_);
    DEJAVIEW_DCHECK(!Tracing::IsInitialized());
  ...
  // DejaView requires only one thread-local object overall (obviously, one
  // instance per-thread) from the embedder.
  using ThreadLocalObject = ::dejaview::PlatformThreadLocalObject;
...
#endif  // INCLUDE_DEJAVIEW_TRACING_PLATFORM_H_
