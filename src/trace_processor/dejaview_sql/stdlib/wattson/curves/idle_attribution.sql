...
CREATE DEJAVIEW TABLE _thread_process_slices AS
SELECT
  sched.ts,
  sched.dur,
  sched.cpu,
...
-- Helper macro so DejaView tables can be used with interval intersect
CREATE DEJAVIEW MACRO _ii_table(tab TableOrSubquery)
...
CREATE DEJAVIEW TABLE _idle_exits AS
SELECT
  ts,
  dur,
  cpu,
...
CREATE DEJAVIEW TABLE _idle_w_threads AS
...
CREATE DEJAVIEW TABLE _idle_transition_cost AS
SELECT
  ii.ts,
  ii.dur,
  threads.cpu,
...
-- either utid or upid
CREATE DEJAVIEW FUNCTION _filter_idle_attribution(ts LONG, dur LONG)
