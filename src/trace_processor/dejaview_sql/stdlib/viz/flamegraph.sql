...
CREATE DEJAVIEW MACRO _viz_flamegraph_hash_coalesce(col ColumnName)
...
CREATE DEJAVIEW MACRO _viz_flamegraph_prepare_filter(
  tab TableOrSubquery,
  show_stack Expr,
  hide_stack Expr,
...
CREATE DEJAVIEW MACRO _viz_flamegraph_filter_frames(
...
CREATE DEJAVIEW MACRO _viz_flamegraph_accumulate(
...
CREATE DEJAVIEW MACRO _viz_flamegraph_s_prefix(col ColumnName)
...
CREATE DEJAVIEW MACRO _viz_flamegraph_upwards_hash(
  source TableOrSubquery,
  filtered TableOrSubquery,
...
-- to the root.
CREATE DEJAVIEW MACRO _viz_flamegraph_downwards_hash(
  source TableOrSubquery,
  filtered TableOrSubquery,
  accumulated TableOrSubquery,
  grouping _ColumnNameList,
...
CREATE DEJAVIEW MACRO _col_list_id(a ColumnName)
...
CREATE DEJAVIEW MACRO _viz_flamegraph_merge_hashes(
...
-- siblings.
CREATE DEJAVIEW MACRO _viz_flamegraph_local_layout(
...
CREATE DEJAVIEW MACRO _viz_flamegraph_global_layout(
  merged TableOrSubquery,
