...
CREATE DEJAVIEW VIEW _binder_client_view
AS
...
CREATE DEJAVIEW VIEW _binder_server_view
AS
...
CREATE DEJAVIEW VIEW _thread_state_view
AS
...
CREATE DEJAVIEW MACRO _binder_flatten_descendants(id ColumnName, ts ColumnName, dur ColumnName,
...
CREATE DEJAVIEW TABLE _binder_server_flat_descendants
AS
...
CREATE DEJAVIEW TABLE _binder_client_flat_descendants
AS
...
CREATE DEJAVIEW TABLE _binder_server_flat_descendants_with_thread_state
AS
SELECT
  ii.ts,
  ii.dur,
...
CREATE DEJAVIEW TABLE _binder_client_flat_descendants_with_thread_state
AS
SELECT
  ii.ts,
  ii.dur,
...
-- 7. IO.
-- 8. State itself.
CREATE DEJAVIEW FUNCTION _binder_reason(
  name STRING,
  state STRING,
  io_wait INT,
  binder_txn_id INT,
...
CREATE DEJAVIEW TABLE android_binder_server_breakdown(
...
CREATE DEJAVIEW TABLE android_binder_client_breakdown(
  -- Client side id of the binder txn. Alias of `slice.id`.
  binder_txn_id INT,
  -- Server side id of the binder txn. Alias of `slice.id`.
  binder_reply_id INT,
