...
INCLUDE DEJAVIEW MODULE graphs.hierarchy;
INCLUDE DEJAVIEW MODULE graphs.scan;

CREATE DEJAVIEW TABLE _callstack_spf_summary AS
SELECT
  id,
  symbol_set_id,
  (
...
CREATE DEJAVIEW TABLE _callstack_spc_raw_forest AS
SELECT
  c.id AS callsite_id,
  s.id AS symbol_id,
  IIF(
...
CREATE DEJAVIEW TABLE _callstack_spc_forest AS
SELECT
  c._auto_id AS id,
...
CREATE DEJAVIEW INDEX _callstack_spc_index
ON _callstack_spc_forest(callsite_id);

CREATE DEJAVIEW MACRO _callstacks_for_stack_profile_samples(
...
CREATE DEJAVIEW MACRO _callstacks_for_cpu_profile_stack_samples(
...
CREATE DEJAVIEW MACRO _callstacks_self_to_cumulative(
