...
#ifndef INCLUDE_DEJAVIEW_TRACE_PROCESSOR_BASIC_TYPES_H_
#define INCLUDE_DEJAVIEW_TRACE_PROCESSOR_BASIC_TYPES_H_
...
constexpr char kMetricProtoRoot[] = "protos/dejaview/metrics/";
...
enum class SoftDropFtraceDataBefore {
  ...
  // into a central dejaview buffer in ring-buffer mode (as opposed to discard
  // mode).
...
struct DEJAVIEW_EXPORT_COMPONENT Config {
...
struct DEJAVIEW_EXPORT_COMPONENT SqlValue {
  ...
  double AsDouble() const {
    DEJAVIEW_CHECK(type == kDouble);
    return double_value;
  }
  int64_t AsLong() const {
    DEJAVIEW_CHECK(type == kLong);
    return long_value;
  }
  const char* AsString() const {
    DEJAVIEW_CHECK(type == kString);
    return string_value;
  }
  const void* AsBytes() const {
    DEJAVIEW_CHECK(type == kBytes);
    return bytes_value;
  }
...
struct SqlPackage {
  ...
  // DEJAVIEW MODULE` statements to the contents of SQL files being executed.
...
#endif  // INCLUDE_DEJAVIEW_TRACE_PROCESSOR_BASIC_TYPES_H_
