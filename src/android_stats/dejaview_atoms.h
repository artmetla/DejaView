...
#ifndef SRC_ANDROID_STATS_DEJAVIEW_ATOMS_H_
#define SRC_ANDROID_STATS_DEJAVIEW_ATOMS_H_

namespace dejaview {

// This must match the values of the DejaViewUploadEvent enum in:
// frameworks/proto_logging/stats/atoms.proto
enum class DejaViewStatsdAtom {
  kUndefined = 0,

  // Checkpoints inside dejaview_cmd before tracing is finished.
  kTraceBegin = 1,
  ...
  // Guardrails inside dejaview_cmd before tracing is finished.
  kOnTimeout = 16,
  ...
  // Checkpoints inside dejaview_cmd after tracing has finished.
  ...
  // Guardrails inside dejaview_cmd after tracing has finished.
  kCmdFwReportEmptyTrace = 50,
  // Will be removed once incidentd is no longer used.
  kUploadIncidentFailure = 10,

  // "Successful" terminal states inside dejaview_cmd.
  ...
  // |DejaViewTriggerAtom| to allow aggregation using a count metric
  // and reduce spam.
  // reserved 12, 13, 14;

  // Contained that a guardrail in dejaview_cmd was hit. Replaced with
  // kCmd* guardrails.
  // reserved 15;

  // Contained status of Dropbox uploads. Removed as DejaView no
  // longer supports uploading traces using Dropbox.
  // reserved 5, 6, 7;

  // Contained status of guardrail state initialization and upload limit in
  // dejaview_cmd. Removed as dejaview no longer manages stateful guardrails
  // reserved 44, 45, 46;
};

// This must match the values of the DejaViewTrigger::TriggerType enum in:
// frameworks/proto_logging/stats/atoms.proto
enum DejaViewTriggerAtom {
  kUndefined = 0,

  kCmdTrigger = 1,
  kCmdTriggerFail = 2,

  kTriggerDejaViewTrigger = 3,
  kTriggerDejaViewTriggerFail = 4,
...
#endif  // SRC_ANDROID_STATS_DEJAVIEW_ATOMS_H_
