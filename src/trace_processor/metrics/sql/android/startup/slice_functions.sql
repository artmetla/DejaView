...
CREATE OR REPLACE DEJAVIEW FUNCTION startup_slice_proto(dur INT)
RETURNS PROTO AS
...
CREATE DEJAVIEW VIEW thread_slices_for_all_launches AS
...
CREATE OR REPLACE DEJAVIEW FUNCTION dur_sum_slice_proto_for_launch(startup_id LONG, slice_name STRING)
RETURNS PROTO AS
...
-- on the main thread.
CREATE OR REPLACE DEJAVIEW FUNCTION dur_sum_main_thread_slice_proto_for_launch(startup_id LONG, slice_name STRING)
RETURNS PROTO AS
SELECT NULL_IF_EMPTY(
  startup_slice_proto(
...
CREATE OR REPLACE DEJAVIEW FUNCTION launch_to_main_thread_slice_proto(startup_id INT, slice_name STRING)
RETURNS PROTO AS
...
CREATE OR REPLACE DEJAVIEW FUNCTION total_gc_time_by_launch(startup_id LONG)
RETURNS INT AS
...
CREATE OR REPLACE DEJAVIEW FUNCTION missing_baseline_profile_for_launch(startup_id LONG, pkg_name STRING)
RETURNS BOOL AS
...
CREATE OR REPLACE DEJAVIEW FUNCTION run_from_apk_for_launch(launch_id LONG)
RETURNS BOOL AS
SELECT EXISTS(
...
CREATE OR REPLACE DEJAVIEW FUNCTION summary_for_optimization_status(
  loc STRING,
  status STRING,
  filter_str STRING,
  reason STRING
...
CREATE OR REPLACE DEJAVIEW FUNCTION binder_transaction_reply_slices_for_launch(
...
CREATE OR REPLACE DEJAVIEW FUNCTION is_unlock_running_during_launch(startup_id LONG)
RETURNS BOOL AS
SELECT EXISTS(
  SELECT slice.name
