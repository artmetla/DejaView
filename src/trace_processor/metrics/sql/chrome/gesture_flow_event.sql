...
CREATE DEJAVIEW VIEW {{prefix}}_latency_info_flow_step_and_ancestors AS
SELECT
  *
FROM (
    SELECT
...
CREATE DEJAVIEW VIEW {{prefix}}_latency_info_flow_step AS
SELECT
  *,
...
CREATE DEJAVIEW VIEW {{prefix}}_max_latency_info_ts_per_trace_id AS
SELECT
  gesture_slice_id,
...
CREATE DEJAVIEW TABLE {{prefix}}_latency_info_flow_null_step_removed AS
SELECT
  ROW_NUMBER() OVER (ORDER BY
...
CREATE DEJAVIEW VIEW {{prefix}}_flow_event AS
SELECT
  curr.trace_id,
  curr.id,
  curr.ts,
