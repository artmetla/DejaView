...
CREATE DEJAVIEW FUNCTION _partition_tree_super_root_fn()
...
CREATE DEJAVIEW FUNCTION _is_libcore_or_array(obj_name STRING)
RETURNS BOOL AS
...
CREATE DEJAVIEW TABLE _heap_graph_dominator_tree_for_partition AS
SELECT
  tree.id,
...
CREATE DEJAVIEW TABLE _heap_object_marked_for_dominated_stats AS
SELECT
  id,
...
CREATE DEJAVIEW TABLE android_heap_graph_class_aggregation (
  -- Process upid
  upid INT,
  -- Heap dump timestamp
  graph_sample_ts INT,
