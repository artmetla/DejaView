...
CREATE DEJAVIEW TABLE top_level_slice AS
SELECT *
FROM slice WHERE
  depth = 0
...
CREATE DEJAVIEW TABLE webview_browser_slices AS
SELECT
  top_level_slice.ts,
  top_level_slice.dur,
  thread_track.utid,
...
CREATE DEJAVIEW TABLE webview_browser_slices_power_summary AS
SELECT
  app_name,
...
CREATE DEJAVIEW TABLE webview_renderer_threads AS
SELECT
  thread.utid AS utid,
  extract_arg(process.arg_set_id, 'chrome.host_app_package_name') AS app_name
FROM process
...
-- in milliampere-seconds.
CREATE DEJAVIEW TABLE webview_renderer_power_summary AS
SELECT
  app_name,
  SUM(dur * COALESCE(power_ma, 0) / 1e9) AS power_mas
...
CREATE DEJAVIEW TABLE webview_renderer_power_per_core_type AS
SELECT
  app_name,
...
CREATE DEJAVIEW TABLE host_app_threads AS
SELECT
  thread.utid AS utid,
  thread.name AS name,
  extract_arg(process.arg_set_id, 'chrome.host_app_package_name') AS app_name
...
-- in milliampere-seconds.
CREATE DEJAVIEW TABLE host_app_power_summary AS
SELECT
  app_name,
  SUM(dur * COALESCE(power_ma, 0) / 1e9) AS power_mas
FROM power_per_thread
...
-- Calculate the power usage of all WebView (host app+browser) processes for each app
-- in milliampere-seconds grouped by core type.
CREATE DEJAVIEW TABLE host_app_power_per_core_type AS
SELECT
  app_name,
  core_type_per_cpu.core_type AS core_type,
  SUM(dur * COALESCE(power_ma, 0) / 1e9) AS power_mas
...
CREATE DEJAVIEW TABLE webview_only_threads AS
SELECT *
...
CREATE DEJAVIEW TABLE webview_only_power_summary AS
SELECT
  app_name,
  SUM(dur * COALESCE(power_ma, 0) / 1e9) AS power_mas
FROM power_per_thread
...
CREATE DEJAVIEW TABLE webview_only_power_per_core_type AS
SELECT app_name,
  core_type_per_cpu.core_type AS core_type,
  SUM(dur * COALESCE(power_ma, 0) / 1e9) AS power_mas
FROM power_per_thread
...
CREATE DEJAVIEW TABLE total_app_power_output AS
SELECT
...
CREATE DEJAVIEW TABLE webview_renderer_power_output AS
SELECT
...
CREATE DEJAVIEW TABLE webview_only_power_output AS
SELECT
...
CREATE DEJAVIEW TABLE total_device_power AS
