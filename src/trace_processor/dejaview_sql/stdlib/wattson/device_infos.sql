...
INCLUDE DEJAVIEW MODULE android.device;

-- Device specific info for deep idle time offsets
CREATE DEJAVIEW TABLE _device_cpu_deep_idle_offsets
AS
...
CREATE DEJAVIEW TABLE _wattson_device_map
AS
...
CREATE DEJAVIEW TABLE _wattson_device AS
WITH soc_model AS (
  SELECT COALESCE(
...
CREATE DEJAVIEW TABLE _cpu_to_policy_map
AS
...
CREATE DEJAVIEW TABLE _dev_cpu_policy_map
AS
SELECT
  cpu, policy
...
CREATE DEJAVIEW TABLE _device_min_volt_vote
AS
...
CREATE DEJAVIEW FUNCTION _get_min_policy_vote()
RETURNS INT AS
SELECT
  vote_tbl.policy
...
CREATE DEJAVIEW FUNCTION _get_min_freq_vote()
RETURNS INT AS
SELECT
 vote_tbl.freq
FROM _device_min_volt_vote as vote_tbl
