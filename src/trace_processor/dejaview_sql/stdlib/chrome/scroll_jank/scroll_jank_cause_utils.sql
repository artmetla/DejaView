...
CREATE DEJAVIEW FUNCTION _get_process_id_for_surfaceflinger()
...
CREATE DEJAVIEW TABLE _process_type_to_name (
...
CREATE DEJAVIEW FUNCTION _get_process_name(
  -- The process type: one of 'Browser' or 'GPU'.
  type STRING
)
-- The process name
...
CREATE DEJAVIEW FUNCTION _get_process_glob(
  -- The process type: one of 'Browser' or 'GPU'.
  type STRING
)
...
CREATE DEJAVIEW FUNCTION _get_process_id_by_type(
  -- The process type: one of 'Browser' or 'GPU'.
  type STRING
)
RETURNS TABLE (
...
CREATE DEJAVIEW FUNCTION _get_renderer_upid_for_event_latency(
...
-- or EventLatency.
CREATE DEJAVIEW FUNCTION _processes_by_type_for_event_latency(
...
CREATE DEJAVIEW FUNCTION chrome_select_scroll_jank_cause_thread(
  -- The slice id of an EventLatency slice.
  event_latency_id INT,
  -- The process type that the thread is on: one of 'Browser', 'Renderer' or
  -- 'GPU'.
