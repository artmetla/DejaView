...
import {
  addAndPinSliceTrack,
  focusOnSlice,
} from '../../dev.dejaview.AndroidCujs/trackUtils';
...
class PinCujScopedJank implements MetricHandler {
  ...
  public match(metricKey: string): CujScopedMetricData | undefined {
    const matcher =
      /dejaview_cuj_(?<process>.*)-(?<cujName>.*)-.*-missed_(?<jankType>frames|sf_frames|app_frames)/;
  ...
  private async cujScopedTrackConfig(
    ...
    CREATE OR REPLACE DEJAVIEW TABLE ${tableWithJankyFramesName} AS
    SELECT
      f.vsync as id,
      f.ts AS ts,
      f.dur as dur
