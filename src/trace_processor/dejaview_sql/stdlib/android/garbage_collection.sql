...
CREATE DEJAVIEW VIEW _gc_slice
AS
WITH concurrent AS (
SELECT
  id AS gc_id,
...
CREATE DEJAVIEW VIEW _gc_heap_counter
AS
SELECT
  c.ts,
...
CREATE DEJAVIEW VIEW _gc_slice_with_final_heap
AS
WITH
  slice_and_heap AS (
...
-- derived earlier.
CREATE DEJAVIEW TABLE _gc_slice_heap
AS
SELECT
  *,
  CASE
...
CREATE DEJAVIEW TABLE android_garbage_collection_events (
