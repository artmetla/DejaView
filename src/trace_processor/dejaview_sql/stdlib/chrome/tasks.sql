...
CREATE DEJAVIEW FUNCTION _has_parent_slice_with_name(
...
CREATE DEJAVIEW FUNCTION _extract_mojo_ipc_hash(slice_id INT)
RETURNS INT AS
...
CREATE DEJAVIEW FUNCTION _extract_frame_type(slice_id INT)
RETURNS INT AS
...
CREATE DEJAVIEW FUNCTION _human_readable_navigation_task_name(
  task_name STRING)
RETURNS STRING AS
SELECT
  CASE
...
CREATE DEJAVIEW FUNCTION _format_scheduler_task_name(task_name STRING)
RETURNS STRING AS
SELECT printf("RunTask(posted_from=%s)", $task_name);

-- Takes the category and determines whether it is "Java" only, as opposed to
-- "toplevel,Java".
CREATE DEJAVIEW FUNCTION _java_not_top_level_category(category STRING)
RETURNS BOOL AS
...
CREATE DEJAVIEW FUNCTION _any_top_level_category(category STRING)
RETURNS BOOL AS
...
CREATE DEJAVIEW FUNCTION _get_java_views_task_type(kind STRING)
RETURNS STRING AS
SELECT
  CASE $kind
...
CREATE DEJAVIEW TABLE _chrome_mojo_slices AS
WITH
...
--                                      capture toolbar screenshot.
CREATE DEJAVIEW TABLE _chrome_java_views AS
WITH
...
CREATE DEJAVIEW VIEW chrome_java_views(
  -- Name of the view.
...
CREATE DEJAVIEW VIEW _chrome_choreographer_tasks
AS
SELECT
  id,
...
CREATE DEJAVIEW FUNCTION _get_posted_from(arg_set_id INT)
RETURNS STRING AS
WITH posted_from as (
  SELECT
...
CREATE DEJAVIEW FUNCTION _select_begin_main_frame_java_slices(
  name STRING)
...
CREATE DEJAVIEW VIEW _chrome_slices_with_java_views AS
WITH
...
CREATE DEJAVIEW TABLE _chrome_scheduler_tasks AS
SELECT
  id
FROM slice
WHERE
...
-- A list of tasks executed by Chrome scheduler.
CREATE DEJAVIEW VIEW chrome_scheduler_tasks(
  -- Slice id.
  id INT,
  -- Type.
  type STRING,
...
-- slice if it exists.
CREATE DEJAVIEW FUNCTION _get_descendant_mojo_slice_candidate(
  slice_id INT
)
RETURNS INT AS
SELECT
...
CREATE DEJAVIEW FUNCTION _descendant_mojo_slice(slice_id INT)
...
CREATE DEJAVIEW TABLE _chrome_tasks AS
WITH
...
CREATE DEJAVIEW VIEW chrome_tasks(
