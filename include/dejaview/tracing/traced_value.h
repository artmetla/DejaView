...
#ifndef INCLUDE_DEJAVIEW_TRACING_TRACED_VALUE_H_
#define INCLUDE_DEJAVIEW_TRACING_TRACED_VALUE_H_
...
//   (preferred for code which depends on dejaview directly)
// - dejaview::TraceFormatTraits<T>::WriteIntoTrace(
//       TracedValue, const T&);
...
// Examples:
//
// TRACE_EVENT("cat", "event", "params", [&](dejaview::TracedValue context)
// {
...
//    return;
//  }
//  dejaview::WriteIntoTrace(std::move(context), *value);
// }
//
// template <typename T>
...
DEJAVIEW_EXPORT_COMPONENT TracedValue
CreateTracedValueFromProto(protos::pbzero::DebugAnnotation*,
                           EventContext* = nullptr);
}

class DEJAVIEW_EXPORT_COMPONENT TracedValue {
 public:
  ...
  TracedArray WriteArray() && DEJAVIEW_WARN_UNUSED_RESULT;

  // Scope which allows multiple key-value pairs to be added.
  TracedDictionary WriteDictionary() && DEJAVIEW_WARN_UNUSED_RESULT;
 ...
 private:
  ...
  // Temporary support for dejaview::DebugAnnotation C++ class before it's going
...
class DEJAVIEW_EXPORT_COMPONENT TracedArray {
 public:
  // implicit
  ...
  TracedDictionary AppendDictionary() DEJAVIEW_WARN_UNUSED_RESULT;
...
class DEJAVIEW_EXPORT_COMPONENT TracedDictionary {
 public:
  // implicit
  ...
  inline TracedDictionary(MessageType* message,
    ...
    static_assert(
        std::is_same<typename FieldMetadata::cpp_field_type,
                     ::dejaview::protos::pbzero::DebugAnnotation>::value,
...
// If dejaview::TraceFormatTraits<T>::WriteIntoTracedValue(TracedValue, const
...
// If dejaview::TraceFormatTraits<T>::WriteIntoTrace(TracedValue, const T&)
// is available, use it.
template <typename T>
...
// dejaview::WriteIntoTracedValue. These templates will fail to resolve if the
...
struct check_traced_value_support<T,
  ...
  static_assert(
      ...
      "dejaview::TracedValue should not be used with incomplete types");
...
// dejaview::WriteIntoTracedValue. This template will always resolve (with
...
// Specialization for DejaView strings.
template <>
struct TraceFormatTraits<dejaview::StaticString> {
  inline static void WriteIntoTrace(TracedValue context,
                                    dejaview::StaticString str) {
    std::move(context).WriteString(str.value);
  }
};

template <>
struct TraceFormatTraits<dejaview::DynamicString> {
  inline static void WriteIntoTrace(TracedValue context,
                                    dejaview::DynamicString str) {
...
struct TraceFormatTraits<std::unique_ptr<T>, check_traced_value_support_t<T>> {
  inline static void WriteIntoTrace(TracedValue context,
    ...
    ::dejaview::WriteIntoTracedValue(std::move(context), value.get());
  }

  template <typename MessageType>
  inline static void WriteIntoTrace(TracedProto<MessageType> message,
                                    const std::unique_ptr<T>& value) {
    ::dejaview::WriteIntoTracedProto(std::move(message), value.get());
  }
};
...
struct TraceFormatTraits<T*, check_traced_value_support_t<T>> {
  inline static void WriteIntoTrace(TracedValue context, T* value) {
    ...
    ::dejaview::WriteIntoTracedValue(std::move(context), *value);
  }
  ...
  inline static void WriteIntoTrace(TracedProto<MessageType> message,
    ...
    ::dejaview::WriteIntoTracedProto(std::move(message), *value);
  }
};
...
#endif  // INCLUDE_DEJAVIEW_TRACING_TRACED_VALUE_H_
