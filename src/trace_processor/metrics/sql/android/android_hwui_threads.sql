...
CREATE DEJAVIEW VIEW {{table_name_prefix}}_main_thread AS
SELECT
...
CREATE DEJAVIEW VIEW {{table_name_prefix}}_render_thread AS
SELECT
  process.name AS process_name,
  thread.utid
FROM thread
...
CREATE DEJAVIEW VIEW {{table_name_prefix}}_gpu_completion_thread AS
SELECT
  process.name AS process_name,
  thread.utid
FROM thread
...
CREATE DEJAVIEW VIEW {{table_name_prefix}}_hwc_release_thread AS
SELECT
  process.name AS process_name,
  thread.utid
FROM thread
...
CREATE DEJAVIEW TABLE {{table_name_prefix}}_main_thread_slices AS
SELECT
  process_name,
  thread.utid,
  slice.*,
...
CREATE DEJAVIEW VIEW {{table_name_prefix}}_do_frame_slices AS
SELECT
  *,
...
CREATE DEJAVIEW TABLE {{table_name_prefix}}_render_thread_slices AS
SELECT
  process_name,
  thread.utid,
  slice.*,
...
CREATE DEJAVIEW VIEW {{table_name_prefix}}_draw_frame_slices AS
SELECT
  *,
  CAST(STR_SPLIT(name, ' ', 1) AS INTEGER) AS vsync
...
CREATE DEJAVIEW VIEW {{table_name_prefix}}_gpu_completion_slices AS
SELECT
  process_name,
  thread.utid,
  slice.*,
...
CREATE DEJAVIEW VIEW {{table_name_prefix}}_hwc_release_slices AS
SELECT
  process_name,
  thread.utid,
  slice.*,
