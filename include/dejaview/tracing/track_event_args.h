...
#ifndef INCLUDE_DEJAVIEW_TRACING_TRACK_EVENT_ARGS_H_
#define INCLUDE_DEJAVIEW_TRACING_TRACK_EVENT_ARGS_H_
...
// inline: TRACE_EVENT(..., dejaview::Flow::ProcessScoped(42));
class Flow {
 public:
  // |flow_id| which is local within a given process (e.g. atomic counter xor'ed
  // with feature-specific value). This value is xor'ed with DejaView's internal
  // process track id to attempt to ensure that it's globally-unique.
  static DEJAVIEW_ALWAYS_INLINE inline Flow ProcessScoped(uint64_t flow_id) {
  ...
  // dejaview::TerminatingFlow::FromPointer(this) from the destructor of the
  // object to avoid accidental conflicts.
  static DEJAVIEW_ALWAYS_INLINE inline Flow FromPointer(void* ptr) {
  ...
  static DEJAVIEW_ALWAYS_INLINE inline Flow Global(uint64_t flow_id) {
    return Flow(flow_id);
  }
...
class TerminatingFlow {
 public:
  ...
  static DEJAVIEW_ALWAYS_INLINE inline TerminatingFlow ProcessScoped(
      uint64_t flow_id) {
    return Global(flow_id ^ Track::process_uuid);
  }

  // See `Flow::FromPointer(void*)`.
  static DEJAVIEW_ALWAYS_INLINE inline TerminatingFlow FromPointer(void* ptr) {
    return ProcessScoped(reinterpret_cast<uintptr_t>(ptr));
  }

  // See `Flow::Global(uint64_t)`.
  static DEJAVIEW_ALWAYS_INLINE inline TerminatingFlow Global(
      uint64_t flow_id) {
    TerminatingFlow tf;
    tf.flow_id_ = flow_id;
    return tf;
...
#endif  // INCLUDE_DEJAVIEW_TRACING_TRACK_EVENT_ARGS_H_
