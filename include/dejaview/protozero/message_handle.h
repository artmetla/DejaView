...
#ifndef INCLUDE_DEJAVIEW_PROTOZERO_MESSAGE_HANDLE_H_
#define INCLUDE_DEJAVIEW_PROTOZERO_MESSAGE_HANDLE_H_
...
class DEJAVIEW_EXPORT_COMPONENT MessageFinalizationListener {
 public:
...
class DEJAVIEW_EXPORT_COMPONENT MessageHandleBase {
 public:
  ~MessageHandleBase() {
    if (message_) {
#if DEJAVIEW_DCHECK_IS_ON()
      DEJAVIEW_DCHECK(generation_ == message_->generation_);
#endif
      FinalizeMessage();
    }
  }
...
#if DEJAVIEW_DCHECK_IS_ON()
    DEJAVIEW_DCHECK(!message_ || generation_ == message_->generation_);
#endif
    return !!message_;
  }
 ...
 private:
  ...
  void reset_message() {
    ...
    DEJAVIEW_DCHECK(message_->is_finalized());
    message_ = nullptr;
    listener_ = nullptr;
  }
...
#endif  // INCLUDE_DEJAVIEW_PROTOZERO_MESSAGE_HANDLE_H_
