...
#include "src/trace_processor/dejaview_sql/intrinsics/table_functions/tables_py.h"

namespace dejaview::trace_processor {
namespace tables {

DejaViewTableInfoTable::~DejaViewTableInfoTable() = default;

}  // namespace tables

namespace {

using TableInfoTable = tables::DejaViewTableInfoTable;
...
TableInfo::TableInfo(StringPool* string_pool, const DejaViewSqlEngine* engine)
    : string_pool_(string_pool), engine_(engine) {}

base::StatusOr<std::unique_ptr<Table>> TableInfo::ComputeTable(
    const std::vector<SqlValue>& arguments) {
  DEJAVIEW_CHECK(arguments.size() == 1);
  if (arguments[0].type != SqlValue::kString) {
    return base::ErrStatus("dejaview_table_info takes table name as a string.");
  }
  ...
  return base::ErrStatus("DejaView table '%s' not found.", table_name.c_str());
}
