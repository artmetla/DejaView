...
#ifndef INCLUDE_DEJAVIEW_EXT_BASE_FLAT_HASH_MAP_H_
#define INCLUDE_DEJAVIEW_EXT_BASE_FLAT_HASH_MAP_H_
...
class FlatHashMap {
 public:
  class Iterator {
   public:
    ...
    Iterator& operator++() {
      DEJAVIEW_DCHECK(idx_ < map_->capacity_);
      ++idx_;
      FindNextNonFree();
      return *this;
    }
  ...
  std::pair<Value*, bool> Insert(Key key, Value value) {
    ...
    size_t probe_len;
    for (;;) {
      DEJAVIEW_DCHECK((capacity_ & (capacity_ - 1)) == 0);  // Must be a pow2.
      ...
      for (probe_len = 0; probe_len < capacity_;) {
        ...
        DEJAVIEW_DCHECK(!(tag_idx == kTombstone && AppendOnly));
      ...
      if (DEJAVIEW_UNLIKELY(size_ >= load_limit_)) {
        MaybeGrowAndRehash(/*grow=*/true);
        continue;
      }
      DEJAVIEW_DCHECK(insertion_slot != kSlotNotFound);
      break;
    }  // for (attempt)

    DEJAVIEW_CHECK(insertion_slot < capacity_);
    ...
    DEJAVIEW_DCHECK(probe_len > 0 && probe_len <= capacity_);
  ...
  bool Erase(const Key& key) {
    if (AppendOnly)
      DEJAVIEW_FATAL("Erase() not supported because AppendOnly=true");
  ...
  void Clear() {
    ...
    if (DEJAVIEW_UNLIKELY(capacity_ == 0))
      return;
 ...
 protected:
  ...
  size_t FindInternal(const Key& key) const {
    ...
    const uint8_t tag = HashToTag(key_hash);
    DEJAVIEW_DCHECK((capacity_ & (capacity_ - 1)) == 0);  // Must be a pow2.
    DEJAVIEW_DCHECK(max_probe_length_ <= capacity_);
    for (size_t i = 0; i < max_probe_length_; ++i) {
      ...
      if (tag_idx == tag && keys_[idx] == key) {
        DEJAVIEW_DCHECK(tag_idx > kTombstone);
        return idx;
      }
    }  // for (idx)
    return kNotFound;
  ...
  void EraseInternal(size_t idx) {
    DEJAVIEW_DCHECK(tags_[idx] > kTombstone);
  ...
  DEJAVIEW_NO_INLINE void MaybeGrowAndRehash(bool grow) {
    ...
    DEJAVIEW_CHECK(new_capacity >= old_capacity);
    Reset(new_capacity);
    ...
    DEJAVIEW_DCHECK(new_size == old_size);
    size_ = new_size;
  }

  // Doesn't call destructors. Use Clear() for that.
  DEJAVIEW_NO_INLINE void Reset(size_t n) {
    DEJAVIEW_DCHECK((n & (n - 1)) == 0);  // Must be a pow2.
  ...
  static inline uint8_t HashToTag(size_t full_hash) {
    ...
    DEJAVIEW_DCHECK(tag > kTombstone);
    return tag;
  }
...
#endif  // INCLUDE_DEJAVIEW_EXT_BASE_FLAT_HASH_MAP_H_
