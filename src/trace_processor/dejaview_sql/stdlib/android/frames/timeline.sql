...
INCLUDE DEJAVIEW MODULE android.frames.timeline_maxsdk28;
...
CREATE DEJAVIEW FUNCTION _get_frame_table_with_id(
...
CREATE DEJAVIEW TABLE android_frames_choreographer_do_frame(
    -- `slice.id`
    id INT,
    -- Frame id
    frame_id INT,
...
-- notifications).
CREATE DEJAVIEW TABLE android_frames_draw_frame(
    -- `slice.id`
    id INT,
    -- Frame id
    frame_id INT,
...
CREATE DEJAVIEW TABLE _distinct_from_actual_timeline_slice AS
SELECT
...
-- We are getting the first slice with one frame id.
CREATE DEJAVIEW TABLE _distinct_from_expected_timeline_slice AS
SELECT
    cast_int!(name) AS frame_id,
    id,
    count() AS count
...
CREATE DEJAVIEW TABLE android_frames(
    -- Frame id.
    frame_id INT,
...
-- most one row.
CREATE DEJAVIEW FUNCTION android_first_frame_after(
    -- Timestamp.
    ts INT)
RETURNS TABLE (
    -- Frame id.
