...
INCLUDE DEJAVIEW MODULE chrome.speedometer_2_1;
INCLUDE DEJAVIEW MODULE chrome.speedometer_3;

CREATE DEJAVIEW FUNCTION _chrome_speedometer_version()
RETURNS STRING
AS
WITH
  num_measures AS (
...
CREATE DEJAVIEW TABLE chrome_speedometer_measure(
...
-- a bit.
CREATE DEJAVIEW TABLE chrome_speedometer_iteration(
...
CREATE DEJAVIEW FUNCTION chrome_speedometer_score()
-- Speedometer score
RETURNS DOUBLE
AS
SELECT
...
CREATE DEJAVIEW FUNCTION chrome_speedometer_renderer_main_utid()
-- Renderer main utid
RETURNS INT
AS
SELECT
