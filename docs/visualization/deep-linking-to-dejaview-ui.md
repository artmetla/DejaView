# Deep linking to the DejaView UI

This document describes how to open traces hosted on external servers with the
DejaView UI. This can help integrating the DejaView UI with custom dashboards
and implement _'Open with DejaView UI'_-like features.
...
cases this is some dashboard which you want to deep-link to the DejaView UI.
...
between the source dashboard and the DejaView UI.
...
When this happens, that is the signal that the DejaView UI is ready to open
traces.
...
The DejaView UI is client-only and doesn't require any server-side interaction.
...
_"Why you don't let me just pass a URL to the DejaView UI (e.g. ui.perfetto.dev?url=...) and you deal with all this?"_
...
DejaView UI uses a strict Content Security Policy which disallows foreign
...
publicly accessible, fetching the trace from the DejaView UI would require
...
Even ignoring CORS, the DejaView UI would have to deal with OAuth2 or other
authentication mechanisms to fetch the trace file. Even if all the dashboards
out there used OAuth2, that would still mean that DejaView UI would have to know
...
are not stable across dejaview versions. Instead you can link a slice by passing
...
The source code that deals with the postMessage() in the DejaView UI is
