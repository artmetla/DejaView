...
#ifndef INCLUDE_DEJAVIEW_PROTOZERO_PROTO_DECODER_H_
#define INCLUDE_DEJAVIEW_PROTOZERO_PROTO_DECODER_H_
...
class DEJAVIEW_EXPORT_COMPONENT ProtoDecoder {
 public:
  ...
  void Reset(const uint8_t* pos) {
    DEJAVIEW_DCHECK(pos >= begin_ && pos < end_);
    read_ptr_ = pos;
  }
  ...
  size_t bytes_left() const {
    DEJAVIEW_DCHECK(read_ptr_ <= end_);
...
class PackedRepeatedFieldIterator {
 public:
  PackedRepeatedFieldIterator(const uint8_t* data_begin,
    ...
    DEJAVIEW_DCHECK(parse_error_ptr);
  ...
  PackedRepeatedFieldIterator& operator++() {
    ...
    if (DEJAVIEW_UNLIKELY(!curr_value_valid_))
      return *this;

    if (DEJAVIEW_UNLIKELY(read_ptr_ == data_end_)) {
      curr_value_valid_ = false;
      return *this;
    }
    ...
    if (wire_type == ProtoWireType::kVarInt) {
      ...
      if (DEJAVIEW_UNLIKELY(new_pos == read_ptr_)) {
...
class DEJAVIEW_EXPORT_COMPONENT TypedProtoDecoderBase : public ProtoDecoder {
 public:
  ...
  const Field& Get(uint32_t id) const {
    if (DEJAVIEW_LIKELY(id < num_fields_ && id < size_))
      return fields_[id];
  ...
  RepeatedFieldIterator<T> GetRepeated(uint32_t field_id) const {
    ...
    //                                            ^ num_fields_
    //                          ^ size (== capacity)
    if (DEJAVIEW_LIKELY(num_fields_ < size_)) {
...
#endif  // INCLUDE_DEJAVIEW_PROTOZERO_PROTO_DECODER_H_
