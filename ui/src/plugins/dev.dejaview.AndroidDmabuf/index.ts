...
class AndroidDmabuf implements DejaViewPlugin {
  async onTraceLoad(ctx: Trace): Promise<void> {
    const e = ctx.engine;
    await e.query(`INCLUDE DEJAVIEW MODULE android.memory.dmabuf`);
    await e.query(`
      CREATE DEJAVIEW TABLE _android_memory_cumulative_dmabuf AS
      SELECT
        upid, utid, ts,
...
export const plugin: PluginDescriptor = {
  pluginId: 'dev.dejaview.AndroidDmabuf',
