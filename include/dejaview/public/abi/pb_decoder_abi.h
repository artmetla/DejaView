...
#ifndef INCLUDE_DEJAVIEW_PUBLIC_ABI_PB_DECODER_ABI_H_
#define INCLUDE_DEJAVIEW_PUBLIC_ABI_PB_DECODER_ABI_H_
...
struct DejaViewPbDecoderField {
  ...
  union {
    // For wire type 2.
    struct DejaViewPbDecoderDelimitedField delimited;
...
// the field.
DEJAVIEW_SDK_EXPORT struct DejaViewPbDecoderField DejaViewPbDecoderParseField(
    struct DejaViewPbDecoder* decoder);

// Advances `*decoder->read_ptr` to point after the current field.
// Returns a `DejaViewPbDecoderStatus`.
DEJAVIEW_SDK_EXPORT uint32_t
DejaViewPbDecoderSkipField(struct DejaViewPbDecoder* decoder);

#ifdef __cplusplus
}
#endif

#endif  // INCLUDE_DEJAVIEW_PUBLIC_ABI_PB_DECODER_ABI_H_
