...
CREATE DEJAVIEW FUNCTION cpu_thread_utilization_per_period(
...
-- period, which is defined as a multiply of |interval|. For this reason
-- first and last period might have lower then real utilization.
CREATE DEJAVIEW FUNCTION cpu_thread_utilization_per_second(
  -- Utid of the thread.
  utid INT
)
RETURNS TABLE (
...
CREATE DEJAVIEW TABLE cpu_cycles_per_thread(
  -- Unique thread id
  utid INT,
...
CREATE DEJAVIEW FUNCTION cpu_cycles_per_thread_in_interval(
