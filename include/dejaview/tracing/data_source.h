...
#ifndef INCLUDE_DEJAVIEW_TRACING_DATA_SOURCE_H_
#define INCLUDE_DEJAVIEW_TRACING_DATA_SOURCE_H_
...
// DEJAVIEW_DECLARE_DATA_SOURCE_STATIC_MEMBERS_WITH_ATTRS). This avoids
// potential macro definition collisions between two libraries using DejaView.
//
// DEJAVIEW_COMPONENT_EXPORT is used to mark symbols in DejaView's headers
// (typically templates) that are defined by the user outside of DejaView and
// should be made visible outside the current module. (e.g., in Chrome's
// component build).
#if !defined(DEJAVIEW_COMPONENT_EXPORT)
#if DEJAVIEW_BUILDFLAG(DEJAVIEW_COMPILER_MSVC)
// Workaround for C4003: not enough arguments for function-like macro invocation
// 'DEJAVIEW_INTERNAL_DECLARE_TRACK_EVENT_DATA_SOURCE'
#define DEJAVIEW_COMPONENT_EXPORT __declspec()
#else
#define DEJAVIEW_COMPONENT_EXPORT
#endif
#endif
...
class DEJAVIEW_EXPORT_COMPONENT DataSourceBase {
 public:
  ...
  class DEJAVIEW_EXPORT_COMPONENT StopArgs {
   public:
    virtual ~StopArgs();
...
struct DataSourceHelper {
  static internal::DataSourceType& type() {
    static dejaview::internal::DataSourceType type_;
    return type_;
  }
};
...
class DataSource : public DataSourceBase {
 ...
 public:
  ...
  class TraceContext {
   public:
    ...
    typename DataSourceTraits::TlsStateType* GetCustomTlsState() {
      DEJAVIEW_DCHECK(tls_inst_->data_source_custom_tls);
  ...
  static void TraceWithInstances(
      ...
      Lambda tracing_fn,
      typename Traits::TracePointData trace_point_data = {}) {
    DEJAVIEW_DCHECK(cached_instances);
 ...
 private:
  friend ::dejaview::test::DataSourceInternalForTest;
  friend ::dejaview::shlib::TrackEvent;
...
// semicolons.
#define DEJAVIEW_INTERNAL_SWALLOW_SEMICOLON() \
  extern int dejaview_internal_unused

// This macro must be used once for each data source next to the data source's
// declaration.
#define DEJAVIEW_DECLARE_DATA_SOURCE_STATIC_MEMBERS(...)  \
  DEJAVIEW_DECLARE_DATA_SOURCE_STATIC_MEMBERS_WITH_ATTRS( \
      DEJAVIEW_COMPONENT_EXPORT, __VA_ARGS__)

// Similar to `DEJAVIEW_DECLARE_DATA_SOURCE_STATIC_MEMBERS` but it also takes
...
#define DEJAVIEW_DECLARE_DATA_SOURCE_STATIC_MEMBERS_WITH_ATTRS(attrs, ...) \
  template <>                                                              \
  attrs dejaview::internal::DataSourceType&                                \
  dejaview::DataSourceHelper<__VA_ARGS__>::type()
...
#define DEJAVIEW_DEFINE_DATA_SOURCE_STATIC_MEMBERS(...)  \
  DEJAVIEW_DEFINE_DATA_SOURCE_STATIC_MEMBERS_WITH_ATTRS( \
      DEJAVIEW_COMPONENT_EXPORT, __VA_ARGS__)

// Similar to `DEJAVIEW_DEFINE_DATA_SOURCE_STATIC_MEMBERS` but it also takes
// custom attributes, which are useful when DataSource is defined in a component
// where a component specific export macro is used.
#define DEJAVIEW_DEFINE_DATA_SOURCE_STATIC_MEMBERS_WITH_ATTRS(attrs, ...) \
  template <>                                                             \
  dejaview::internal::DataSourceType&                                     \
  dejaview::DataSourceHelper<__VA_ARGS__>::type() {                       \
    static dejaview::internal::DataSourceType type_;                      \
    return type_;                                                         \
  }                                                                       \
...
#endif  // INCLUDE_DEJAVIEW_TRACING_DATA_SOURCE_H_
