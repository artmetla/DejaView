...
#ifndef INCLUDE_DEJAVIEW_TRACING_TRACK_H_
#define INCLUDE_DEJAVIEW_TRACING_TRACK_H_
...
//
//   TRACE_EVENT_BEGIN("category", "AsyncEvent", dejaview::Track(8086));
//   ...
//   TRACE_EVENT_END("category", dejaview::Track(8086));
//
...
//
struct DEJAVIEW_EXPORT_COMPONENT Track {
  const uint64_t uuid;
  ...
  static Track FromPointer(const void* ptr, Track parent = MakeProcessTrack()) {
    ...
    DEJAVIEW_DCHECK(parent.uuid != Track().uuid);
  ...
  // Usage: TRACE_EVENT_BEGIN("...", "...", dejaview::Track::ThreadScoped(this))
...
struct DEJAVIEW_EXPORT_COMPONENT ProcessTrack : public Track {
...
struct DEJAVIEW_EXPORT_COMPONENT ThreadTrack : public Track {
  const base::PlatformProcessId pid;
...
// /protos/dejaview/trace/track_event/counter_descriptor.proto for details.
class DEJAVIEW_EXPORT_COMPONENT CounterTrack : public Track {
 ...
 public:
  using Unit = dejaview::protos::pbzero::CounterDescriptor::Unit;
  using CounterType =
      dejaview::protos::gen::CounterDescriptor::BuiltinCounterType;
  ...
  constexpr explicit CounterTrack(StaticString name,
      ...
      : CounterTrack(
            name,
            dejaview::protos::pbzero::CounterDescriptor::UNIT_UNSPECIFIED,
            nullptr,
            parent) {}
 ...
 private:
  ...
  Unit unit_ = dejaview::protos::pbzero::CounterDescriptor::UNIT_UNSPECIFIED;
...
// track name).
// TODO(eseckler): Remove DEJAVIEW_EXPORT_COMPONENT once Chromium no longer
// calls TrackRegistry::InitializeInstance() directly.
class DEJAVIEW_EXPORT_COMPONENT TrackRegistry {
 public:
  ...
  void SerializeTrack(
    ...
    std::string desc_copy;
    {
      ...
      if (it != tracks_.end()) {
        desc_copy = it->second;
        DEJAVIEW_DCHECK(!desc_copy.empty());
      }
    }
    if (!desc_copy.empty()) {
...
#endif  // INCLUDE_DEJAVIEW_TRACING_TRACK_H_
