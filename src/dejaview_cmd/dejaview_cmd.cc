...
#include "dejaview/base/proc_utils.h"         // IWYU pragma: keep
#include "dejaview/ext/base/android_utils.h"  // IWYU pragma: keep
#include "dejaview/ext/base/ctrl_c_handler.h"
#include "dejaview/ext/base/file_utils.h"
#include "dejaview/ext/base/getopt.h"  // IWYU pragma: keep
...
#include "src/dejaview_cmd/config.h"
#include "src/dejaview_cmd/packet_writer.h"
#include "src/dejaview_cmd/pbtxt_to_pb.h"
#include "src/dejaview_cmd/rate_limiter.h"
#include "src/dejaview_cmd/trigger_producer.h"

#include "protos/dejaview/common/ftrace_descriptor.gen.h"
...
std::atomic<dejaview::DejaViewCmd*> g_dejaview_cmd;
...
bool IsUserBuild() {
  ...
  if (build_type.empty()) {
    DEJAVIEW_ELOG("Unable to read ro.build.type: assuming user build");
    return true;
  }
  return build_type == "user";
#else
  return false;
#endif  // DEJAVIEW_BUILDFLAG(DEJAVIEW_ANDROID_BUILD)
}

std::optional<DejaViewStatsdAtom> ConvertRateLimiterResponseToAtom(
    RateLimiter::ShouldTraceResponse resp) {
  switch (resp) {
    case RateLimiter::kNotAllowedOnUserBuild:
      return DejaViewStatsdAtom::kCmdUserBuildTracingNotAllowed;
...
const char* kStateDir = "/data/misc/dejaview-traces";

DejaViewCmd::DejaViewCmd() {
  // Only the main thread instance on the main thread will receive ctrl-c.
  DejaViewCmd* set_if_null = nullptr;
  g_dejaview_cmd.compare_exchange_strong(set_if_null, this);
}

DejaViewCmd::~DejaViewCmd() {
  DejaViewCmd* self = this;
  if (g_dejaview_cmd.compare_exchange_strong(self, nullptr)) {
...
void DejaViewCmd::PrintUsage(const char* argv0) {
  fprintf(stderr, R"(
Usage: %s
...
Statsd-specific and other Android-only flags:
  ...
  --dropbox        TAG : DEPRECATED: Use --upload instead
                         TAG should always be set to 'dejaview'.
...
std::optional<int> DejaViewCmd::ParseCmdlineAndMaybeDaemonize(int argc,
                                                              char** argv) {
#if !DEJAVIEW_BUILDFLAG(DEJAVIEW_OS_WIN)
  ...
  for (;;) {
    ...
    if (option == 'c') {
      ...
      if (strcmp(optarg, "-") == 0) {
        ...
        DEJAVIEW_CHECK(CreateConfigFromOptions(opts, &test_config));
        ...
        // the new DejaViewCmd instance, we need to know upfront trace config
        // fields that affect the behaviour of dejaview_cmd, e.g., the guardrail
        // overrides, the unique_session_name, the reporter API package etc.
        DEJAVIEW_CHECK(!snapshot_config_.empty());
        ...
        if (!base::ReadFile(optarg, &trace_config_raw)) {
          ...
          DEJAVIEW_PLOG(
              ...
              "placing the config in /data/misc/dejaview-configs: DejaView "
              "should always be able to access this directory.",
              optarg);
#else
          DEJAVIEW_PLOG("Could not open %s", optarg);
#endif
          return 1;
        }
      }
    ...
    if (option == OPT_UPLOAD) {
      ...
      continue;
#else
      DEJAVIEW_ELOG("--upload is only supported on Android");
      return 1;
#endif
    }
...
#if DEJAVIEW_BUILDFLAG(DEJAVIEW_OS_ANDROID)
      DEJAVIEW_CHECK(optarg);
      upload_flag_ = true;
      continue;
#else
      DEJAVIEW_ELOG("--dropbox is only supported on Android");
      return 1;
#endif
    }
    ...
    if (option == OPT_RESET_GUARDRAILS) {
      DEJAVIEW_ILOG(
          "Guardrails no longer exist in dejaview_cmd; this option only exists "
    ...
    if (option == OPT_DETACH) {
      ...
      DEJAVIEW_CHECK(!detach_key_.empty());
      continue;
    }

    if (option == OPT_ATTACH) {
      attach_key_ = std::string(optarg);
      DEJAVIEW_CHECK(!attach_key_.empty());
      continue;
    }
  ...
  if (query_service_ && (is_detach() || is_attach() || background_)) {
    DEJAVIEW_ELOG("--query cannot be combined with any other argument");
    return 1;
  }

  if (query_service_long_ && !query_service_) {
    DEJAVIEW_ELOG("--long can only be used with --query");
    return 1;
  }

  if (is_detach() && is_attach()) {
    DEJAVIEW_ELOG("--attach and --detach are mutually exclusive");
    return 1;
  }

  if (is_detach() && background_) {
    DEJAVIEW_ELOG("--detach and --background are mutually exclusive");
    return 1;
  }

  if (stop_trace_once_attached_ && !is_attach()) {
    DEJAVIEW_ELOG("--stop is supported only in combination with --attach");
    return 1;
  }
  ...
  if ((bugreport_ || clone_all_bugreport_traces_) &&
      (is_attach() || is_detach() || query_service_ || has_config_options ||
       background_wait_)) {
    DEJAVIEW_ELOG("--save-for-bugreport cannot take any other argument");
    return 1;
  }

  if (clone_for_bugreport_ && !clone_tsid_) {
    DEJAVIEW_ELOG("--clone-for-bugreport requires --clone");
    return 1;
  }

  // --save-for-bugreport is the equivalent of:
  // --clone kBugreportSessionId -o /data/misc/dejaview-traces/bugreport/...
  if (bugreport_ && trace_out_path_.empty()) {
    DEJAVIEW_LOG("Invoked dejaview with --save-for-bugreport");
  ...
  if (!will_trace_or_trigger) {
    if ((!trace_config_raw.empty() || has_config_options)) {
      DEJAVIEW_ELOG("Cannot specify a trace config with this option");
      return 1;
    }
    ...
    if (trace_config_raw.empty() && !clone_tsid_) {
      DEJAVIEW_ELOG("The TraceConfig is empty");
      return 1;
    }
    DEJAVIEW_DLOG("Parsing TraceConfig, %zu bytes", trace_config_raw.size());
    if (parse_as_pbtxt) {
  ...
  if (parsed) {
    *trace_config_->mutable_statsd_metadata() = std::move(statsd_metadata);
    ...
    DEJAVIEW_ELOG("The trace config is invalid, bailing out.");
    if (cfg_could_be_txt) {
      DEJAVIEW_ELOG(
          "Looks like you are passing a textual config but I'm expecting a "
          "proto-encoded binary config.");
      DEJAVIEW_ELOG("Try adding --txt to the cmdline.");
    }
    return 1;
  }
  ...
  if (delay.has_max_delay_ms() != delay.has_min_delay_ms()) {
    DEJAVIEW_ELOG("cmd_trace_start_delay field is only partially specified.");
    return 1;
  }
  ...
  // |activate_triggers| in the trace config is shorthand for trigger_dejaview.
  ...
  if (!will_trace_or_trigger) {
    open_out_file = false;
    if (!trace_out_path_.empty() || upload_flag_) {
      DEJAVIEW_ELOG("Can't pass an --out file (or --upload) with this option");
      return 1;
    }
    ...
    DEJAVIEW_ELOG("Either --out or --upload is required");
    return 1;
  ...
  if (will_trace_indefinitely && save_to_incidentd_ && !ignore_guardrails_) {
    DEJAVIEW_ELOG("Can't trace indefinitely when tracing to Incidentd.");
    return 1;
  }
  ...
  if (background_) {
    ...
    DEJAVIEW_CHECK(snapshot_threads_.empty());  // No threads before Daemonize.
...
void DejaViewCmd::NotifyBgProcessPipe(BgProcessStatus status) {
#if !DEJAVIEW_BUILDFLAG(DEJAVIEW_OS_WIN)
  if (!background_wait_pipe_.wr) {
    return;
  }
  static_assert(sizeof status == 1, "Enum bigger than one byte");
  DEJAVIEW_EINTR(write(background_wait_pipe_.wr.get(), &status, 1));
  background_wait_pipe_.wr.reset();
...
DejaViewCmd::BgProcessStatus DejaViewCmd::WaitOnBgProcessPipe() {
#if !DEJAVIEW_BUILDFLAG(DEJAVIEW_OS_WIN)
  ...
  int ret = DEJAVIEW_EINTR(poll(&pollfds[0], pollfds.size(), 30000 /*ms*/));
  DEJAVIEW_CHECK(ret >= 0);
  if (ret == 0) {
    fprintf(stderr, "Timeout waiting for all data sources to start\n");
    return kBackgroundTimeout;
  }
  ssize_t read_ret = DEJAVIEW_EINTR(read(fd.get(), &msg, 1));
  DEJAVIEW_CHECK(read_ret >= 0);
  if (read_ret == 0) {
...
int DejaViewCmd::ConnectToServiceRunAndMaybeNotify() {
...
int DejaViewCmd::ConnectToServiceAndRun() {
  ...
  if (!triggers_to_activate_.empty()) {
    LogTriggerEvents(DejaViewTriggerAtom::kCmdTrigger, triggers_to_activate_);
    ...
    if (!finished_with_success) {
      LogTriggerEvents(DejaViewTriggerAtom::kCmdTriggerFail,
                       triggers_to_activate_);
    }
  ...
  if (delay.has_min_delay_ms()) {
    DEJAVIEW_DCHECK(delay.has_max_delay_ms());
    std::random_device r;
  ...
  if (clone_tsid_) {
    if (snapshot_trigger_name_.empty()) {
      LogUploadEvent(DejaViewStatsdAtom::kCmdCloneTraceBegin);
    } else {
      LogUploadEvent(DejaViewStatsdAtom::kCmdCloneTriggerTraceBegin,
                     snapshot_trigger_name_);
    }
  } else if (trace_config_->trigger_config().trigger_timeout_ms() == 0) {
    LogUploadEvent(DejaViewStatsdAtom::kTraceBegin);
  } else {
    LogUploadEvent(DejaViewStatsdAtom::kBackgroundTraceBegin);
  }
  ...
  if (!background_ && !is_detach() && !upload_flag_ &&
    ...
    fprintf(stderr,
            ...
            "are running dejaview via adb shell, use the -tt arg (adb shell "
            "-t dejaview ...) or consider using the helper script "
            "tools/record_android_trace from the DejaView repository.\n\n");
  }
#endif
...
void DejaViewCmd::OnConnect() {
  connected_ = true;
  LogUploadEvent(DejaViewStatsdAtom::kOnConnect);
  ...
  if (clone_tsid_.has_value()) {
    task_runner_.PostDelayedTask(std::bind(&DejaViewCmd::OnTimeout, this),
                                 kCloneTimeoutMs);
  ...
  if (expected_duration_ms_) {
    DEJAVIEW_LOG("Connected to the DejaView traced service, TTL: %ds",
                 (expected_duration_ms_ + 999) / 1000);
  } else {
    DEJAVIEW_LOG("Connected to the DejaView traced service, starting tracing");
  }

  DEJAVIEW_DCHECK(trace_config_);
...
void DejaViewCmd::OnDisconnect() {
  if (connected_) {
    DEJAVIEW_LOG("Disconnected from the traced service");
  } else {
#if DEJAVIEW_BUILDFLAG(DEJAVIEW_OS_ANDROID)
...
void DejaViewCmd::OnTimeout() {
  DEJAVIEW_ELOG("Timed out while waiting for trace from the service, aborting");
  LogUploadEvent(DejaViewStatsdAtom::kOnTimeout);
  task_runner_.Quit();
}

void DejaViewCmd::CheckTraceDataTimeout() {
  if (trace_data_timeout_armed_) {
    DEJAVIEW_ELOG("Timed out while waiting for OnTraceData, aborting");
    FinalizeTraceAndExit();
  }
  trace_data_timeout_armed_ = true;
  task_runner_.PostDelayedTask(
      std::bind(&DejaViewCmd::CheckTraceDataTimeout, this),
      kOnTraceDataTimeoutMs);
}

void DejaViewCmd::OnTraceData(std::vector<TracePacket> packets, bool has_more) {
  trace_data_timeout_armed_ = false;

  DEJAVIEW_CHECK(packet_writer_.has_value());
  if (!packet_writer_->WritePackets(packets)) {
    DEJAVIEW_ELOG("Failed to write packets");
    FinalizeTraceAndExit();
  }
...
void DejaViewCmd::OnTracingDisabled(const std::string& error) {
  ReadbackTraceDataAndQuit(error);
}

void DejaViewCmd::ReadbackTraceDataAndQuit(const std::string& error) {
  if (!error.empty()) {
    ...
    DEJAVIEW_LOG("Service error: %s", error.c_str());
    ...
    // reasons: when guardrails used to exist in dejaview_cmd, this codepath
  ...
  LogUploadEvent(DejaViewStatsdAtom::kOnTracingDisabled);
...
void DejaViewCmd::FinalizeTraceAndExit() {
  LogUploadEvent(DejaViewStatsdAtom::kFinalizeTraceAndExit);
  ...
  if (save_to_incidentd_) {
    ...
    if (trace_config_->write_into_file()) {
      ...
      DEJAVIEW_LOG("Trace written into the output file");
    } else {
      DEJAVIEW_LOG("Wrote %" PRIu64 " bytes into %s", bytes_written_,
...
bool DejaViewCmd::OpenOutputFile() {
  base::ScopedFile fd;
...
void DejaViewCmd::SetupCtrlCSignalHandler() {
  ...
  task_runner_.AddFileDescriptorWatch(ctrl_c_evt_.fd(), [weak_this] {
    if (!weak_this)
      return;
    DEJAVIEW_LOG("SIGINT/SIGTERM received: disabling tracing.");
    ...
    weak_this->consumer_endpoint_->Flush(
        ...
        [weak_this](bool flush_success) {
          if (!weak_this)
            return;
          if (!flush_success)
            DEJAVIEW_ELOG("Final flush unsuccessful.");
          weak_this->consumer_endpoint_->DisableTracing();
        },
        FlushFlags(FlushFlags::Initiator::kDejaViewCmd,
                   FlushFlags::Reason::kTraceStop));
  });
}

void DejaViewCmd::OnDetach(bool success) {
  if (!success) {
    DEJAVIEW_ELOG("Session detach failed");
    exit(1);
  }
  exit(0);
}

void DejaViewCmd::OnAttach(bool success, const TraceConfig& trace_config) {
  if (!success) {
    if (!redetach_once_attached_) {
      ...
      DEJAVIEW_ELOG("Session re-attach failed. Check service logs for details");
    }
  ...
  DEJAVIEW_DCHECK(trace_config_->write_into_file());
...
void DejaViewCmd::OnTraceStats(bool /*success*/,
                               const TraceStats& /*trace_config*/) {
  // TODO(eseckler): Support GetTraceStats().
}

void DejaViewCmd::OnSessionCloned(const OnSessionClonedArgs& args) {
  DEJAVIEW_DLOG("Cloned tracing session %" PRIu64 ", success=%d",
  ...
  if (snapshot_trigger_name_.empty()) {
    LogUploadEvent(DejaViewStatsdAtom::kCmdOnSessionClone);
  } else {
    LogUploadEvent(DejaViewStatsdAtom::kCmdOnTriggerSessionClone,
                   snapshot_trigger_name_);
  }
  ReadbackTraceDataAndQuit(full_error);
}

void DejaViewCmd::PrintServiceState(bool success,
                                    const TracingServiceState& svc_state) {
  if (!success) {
    DEJAVIEW_ELOG("Failed to query the service state");
    return;
  }
...
void DejaViewCmd::OnObservableEvents(
...
void DejaViewCmd::OnCloneSnapshotTriggerReceived(TracingSessionID tsid,
                                                 std::string trigger_name) {
  std::string cmdline;
  cmdline.reserve(128);
  ArgsAppend(&cmdline, "dejaview");
...
void DejaViewCmd::CloneSessionOnThread(
  ...
  snapshot_threads_.back().PostTask(
      [tsid, cmdline, trace_config_copy, trigger_name, on_clone_callback] {
        ...
        // the underlying string storage that gets std::moved to DejaViewCmd.
        base::StringSplitter splitter(std::move(cmdline), '\0');
        while (splitter.Next()) {
          argv[argc++] = splitter.cur_token();
          DEJAVIEW_CHECK(static_cast<size_t>(argc) < base::ArraySize(argv));
        }
        dejaview::DejaViewCmd cmd;
        ...
        DEJAVIEW_CHECK(!cmdline_res.has_value());  // No daemonization expected.
        int res = cmd.ConnectToServiceRunAndMaybeNotify();
        if (res)
          DEJAVIEW_ELOG("Cloning session %" PRIu64 " failed (%d)", tsid, res);
      });
}

void DejaViewCmd::CloneAllBugreportTraces(
    bool success,
    const TracingServiceState& service_state) {
  if (!success)
    DEJAVIEW_FATAL("Failed to list active tracing sessions");
  ...
  if (sessions.empty()) {
    DEJAVIEW_LOG("No tracing sessions eligible for bugreport were found.");
    exit(0);
  }
  ...
  for (auto it = sessions.begin(); it != sessions.end(); ++it) {
    ...
    DEJAVIEW_LOG("Cloning tracing session %" PRIu64 " with score %d into %s",
  ...
  DEJAVIEW_DLOG("Issuing %zu CloneSession requests", num_sessions);
  sync_point->Wait(num_sessions);
  DEJAVIEW_DLOG("All %zu sessions have acked the clone request", num_sessions);

  // After all sessions are done, quit.
  // Note that there is no risk that thd.PostTask() will interleave with the
  // sequence of tasks that DejaViewCmd involves, there is no race here.
  // There are two TaskRunners here, nested into each other:
  // 1) The "outer" ThreadTaskRunner, created by `thd`. This will see only one
  //    task ever, which is "run dejaview_cmd until completion".
  // 2) Internally DejaViewCmd creates its own UnixTaskRunner, which creates
  ...
  for (auto& thd : snapshot_threads_) {
    thd.PostTask([done_count] {
      if (done_count->fetch_sub(1) == 1) {
        DEJAVIEW_DLOG("All sessions cloned. quitting");
        exit(0);
      }
    });
  }
...
void DejaViewCmd::LogUploadEvent(DejaViewStatsdAtom atom) {
  if (!statsd_logging_)
    return;
...
void DejaViewCmd::LogTriggerEvents(
...
int DEJAVIEW_EXPORT_ENTRYPOINT DejaViewCmdMain(int argc, char** argv) {
  dejaview::DejaViewCmd cmd;
