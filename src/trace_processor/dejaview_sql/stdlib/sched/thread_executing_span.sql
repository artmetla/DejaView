...
--
CREATE DEJAVIEW TABLE _runnable_state
AS
SELECT
  thread_state.id,
  thread_state.ts,
...
CREATE DEJAVIEW TABLE _first_runnable_state
AS
WITH
  first_state AS (
    SELECT
...
CREATE DEJAVIEW TABLE _sleep_state
AS
SELECT
  thread_state.id,
  thread_state.ts,
...
--
CREATE DEJAVIEW TABLE _thread_end_ts
AS
SELECT
...
CREATE DEJAVIEW TABLE _first_sleep_state
AS
SELECT
  MIN(s.id) AS id,
  s.ts,
...
CREATE DEJAVIEW TABLE _wakeup_map
AS
WITH x AS (
...
--
CREATE DEJAVIEW TABLE _wakeup_graph
AS
WITH
  _wakeup_events AS (
    SELECT
...
CREATE DEJAVIEW VIEW _wakeup_userspace_edges
AS
SELECT
...
CREATE DEJAVIEW VIEW _wakeup_kernel_edges
AS
SELECT
  id AS source_node_id,
...
CREATE DEJAVIEW VIEW _wakeup_intervals
AS
...
CREATE DEJAVIEW MACRO _intervals_to_roots(_source_table TableOrSubQuery,
...
-- correctly.
CREATE DEJAVIEW MACRO _critical_path_userspace_adjusted(_critical_path_table TableOrSubQuery,
                                                        _node_table TableOrSubQuery)
RETURNS TableOrSubQuery
AS (
    SELECT
...
CREATE DEJAVIEW MACRO _critical_path_kernel_adjusted(_userspace_critical_path_table TableOrSubQuery,
...
CREATE DEJAVIEW MACRO _critical_path_merged(_userspace_critical_path_table TableOrSubQuery,
                                            _kernel_critical_path_table TableOrSubQuery,
                                            _node_table TableOrSubQuery)
RETURNS TableOrSubQuery
AS (
...
CREATE DEJAVIEW MACRO _critical_path_by_roots(_roots_table TableOrSubQuery,
                                              _node_table TableOrSubQuery)
RETURNS TableOrSubQuery
AS (
...
CREATE DEJAVIEW MACRO _critical_path_by_intervals(_intervals_table TableOrSubQuery,
                                                  _node_table TableOrSubQuery)
...
CREATE DEJAVIEW FUNCTION _thread_executing_span_critical_path(
