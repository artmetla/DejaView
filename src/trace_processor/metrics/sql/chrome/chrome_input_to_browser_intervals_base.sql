...
CREATE OR REPLACE DEJAVIEW FUNCTION {{function_prefix}}PRECEDING_IO_THREAD_EVENT_FLOW_ID(id LONG)
RETURNS LONG AS
SELECT MAX(flow.slice_out) AS id
FROM PRECEDING_FLOW(($id)) flow;

-- Returns a Chrome task which contains the given slice.
CREATE OR REPLACE DEJAVIEW FUNCTION {{function_prefix}}GET_ENCLOSING_CHROME_TASK_NAME(
  slice_id LONG
)
RETURNS STRING AS
SELECT
...
CREATE OR REPLACE DEJAVIEW FUNCTION {{function_prefix}}GET_SCROLL_TYPE(
...
CREATE OR REPLACE DEJAVIEW FUNCTION _has_descendant_slice_with_name(
...
CREATE DEJAVIEW VIEW chrome_valid_gesture_updates
AS
SELECT
  name,
...
CREATE DEJAVIEW VIEW chrome_flow_slices_for_gestures
AS
SELECT
  s.ts,
  EXTRACT_ARG(
...
CREATE DEJAVIEW VIEW chrome_flow_slices_for_gestures_tied_process
AS
SELECT
  ts,
  trace_id,
...
CREATE DEJAVIEW TABLE chrome_indexed_flow_per_gesture
AS
SELECT
  ts,
  trace_id,
...
CREATE DEJAVIEW VIEW chrome_first_flow_per_gesture
AS
SELECT
  *
FROM
...
CREATE DEJAVIEW VIEW chrome_categorized_first_flow_events
AS
SELECT
  *,
  NOT COALESCE((
...
CREATE DEJAVIEW VIEW chrome_input_to_browser_interval_slice_ids
AS
SELECT
