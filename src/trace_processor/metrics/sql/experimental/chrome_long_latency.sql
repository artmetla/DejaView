...
CREATE DEJAVIEW VIEW long_eventlatency_slice AS
SELECT
  ts + dur AS ts,
  dur,
  id,
...
CREATE DEJAVIEW VIEW long_latency_with_upid AS
SELECT
...
CREATE DEJAVIEW VIEW long_latency_with_process_info AS
SELECT
...
CREATE DEJAVIEW VIEW chrome_long_latency_event AS
SELECT
  'slice' AS track_type,
...
CREATE DEJAVIEW VIEW chrome_long_latency_output AS
