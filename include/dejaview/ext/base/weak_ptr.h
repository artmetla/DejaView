...
#ifndef INCLUDE_DEJAVIEW_EXT_BASE_WEAK_PTR_H_
#define INCLUDE_DEJAVIEW_EXT_BASE_WEAK_PTR_H_
...
class WeakPtrFactory {
 public:
  ...
  void Reset(T* owner) {
    ...
    DEJAVIEW_DETACH_FROM_THREAD(thread_checker);
    DEJAVIEW_DCHECK_THREAD(thread_checker);

    // We should not have passed out any weak pointers yet at this point.
    DEJAVIEW_DCHECK(weak_ptr_.handle_.use_count() == 1);
...
#endif  // INCLUDE_DEJAVIEW_EXT_BASE_WEAK_PTR_H_
