...
#ifndef INCLUDE_DEJAVIEW_TRACING_INTERCEPTOR_H_
#define INCLUDE_DEJAVIEW_TRACING_INTERCEPTOR_H_

// An interceptor is used to redirect trace packets written by a data source
// into a custom backend instead of the normal DejaView tracing service. For
...
class DEJAVIEW_EXPORT_COMPONENT InterceptorBase {
 public:
  ...
  class DEJAVIEW_EXPORT_COMPONENT ThreadLocalState {
   public:
  ...
  // |SetupArgs|. Called on an internal DejaView service thread, but not
  // concurrently.
  virtual void OnSetup(const SetupArgs&) {}

  // Called when an intercepted data source starts. Called on an internal
  // DejaView service thread, but not concurrently.
...
class DEJAVIEW_EXPORT_COMPONENT Interceptor : public InterceptorBase {
 public:
...
#endif  // INCLUDE_DEJAVIEW_TRACING_INTERCEPTOR_H_
