...
CREATE OR REPLACE DEJAVIEW FUNCTION describe_symbol(symbol STRING, frame_name STRING)
RETURNS STRING AS
...
CREATE DEJAVIEW VIEW chrome_targeted_task AS
SELECT
...
CREATE DEJAVIEW VIEW chrome_non_symbolized_frames AS
SELECT
...
CREATE DEJAVIEW VIEW chrome_symbolized_child_frames AS
SELECT
...
CREATE DEJAVIEW VIEW chrome_thread_symbolized_child_frames AS
SELECT
...
CREATE DEJAVIEW VIEW chrome_non_symbolized_frames_timed AS
SELECT
...
CREATE DEJAVIEW VIEW chrome_frames_timed_and_symbolized AS
SELECT
  describe_symbol(symbol.name, frame_name) AS description,
  ts,
  depth,
...
CREATE DEJAVIEW VIEW all_frames AS
SELECT
  *
FROM
  (SELECT
...
CREATE DEJAVIEW VIEW chrome_stack_samples_for_task AS
SELECT
  all_frames.*
FROM
  all_frames JOIN
