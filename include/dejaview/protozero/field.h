...
#ifndef INCLUDE_DEJAVIEW_PROTOZERO_FIELD_H_
#define INCLUDE_DEJAVIEW_PROTOZERO_FIELD_H_
...
struct ConstChars {
  // Allow implicit conversion to dejaview's base::StringView without depending
  // on dejaview/base or viceversa.
...
class Field {
 public:
  ...
  proto_utils::ProtoWireType type() const {
    ...
    DEJAVIEW_DCHECK(res == proto_utils::ProtoWireType::kVarInt ||
  ...
  bool as_bool() const {
    DEJAVIEW_DCHECK(!valid() || type() == proto_utils::ProtoWireType::kVarInt);
  ...
  float as_float() const {
    DEJAVIEW_DCHECK(!valid() || type() == proto_utils::ProtoWireType::kFixed32);
    float res;
  ...
  double as_double() const {
    DEJAVIEW_DCHECK(!valid() || type() == proto_utils::ProtoWireType::kFixed64);
    double res;
...
#endif  // INCLUDE_DEJAVIEW_PROTOZERO_FIELD_H_
