...
CREATE DEJAVIEW VIEW chrome_cpu_power_slice(
...
CREATE DEJAVIEW VIEW _cpu_power_valid_sched_slice AS
  SELECT *
  FROM sched_slice
  WHERE utid != 0;
...
-- CPU power up.
CREATE DEJAVIEW TABLE chrome_cpu_power_first_sched_slice_after_powerup(
  -- The timestamp at the start of the slice.
  ts INT,
  -- The duration of the slice.
  dur INT,
...
CREATE DEJAVIEW VIEW _cpu_power_thread_and_toplevel_slice AS
  SELECT
    t.utid AS utid,
    s.id AS slice_id,
    s.ts,
...
CREATE DEJAVIEW VIEW chrome_cpu_power_first_toplevel_slice_after_powerup(
