...
#ifndef INCLUDE_DEJAVIEW_TRACING_TRACING_H_
#define INCLUDE_DEJAVIEW_TRACING_TRACING_H_
...
struct TracingError {
  ...
  TracingError(ErrorCode cd, std::string msg)
    ...
    DEJAVIEW_CHECK(!message.empty());
  }
};

using LogLev = ::dejaview::base::LogLev;
using LogMessageCallbackArgs = ::dejaview::base::LogMessageCallbackArgs;
using LogMessageCallback = ::dejaview::base::LogMessageCallback;
...
struct TracingInitArgs {
  ...
  // [Optional] If set, log messages generated by dejaview are passed to this
  ...
  // dejaview::GetProducerSocket(), connect to it, and return the corresponding
 ...
 protected:
  ...
  bool dcheck_is_on_ = DEJAVIEW_DCHECK_IS_ON();
};

// The entry-point for using dejaview.
class DEJAVIEW_EXPORT_COMPONENT Tracing {
 public:
  // Initializes DejaView with the given backends in the calling process and/or
  ...
  static DEJAVIEW_ALWAYS_INLINE inline std::unique_ptr<TracingSession> NewTrace(
      BackendType backend = kUnspecifiedBackend);

  // Shut down DejaView, releasing any allocated OS resources (threads, files,
  // sockets, etc.). Note that DejaView cannot be reinitialized again in the
  // same process[1]. Instead, this function is meant for shutting down all
  // DejaView-related code so that it can be safely unloaded, e.g., with
  // dlclose().
  //
  ...
  // Uninitialize DejaView. Only exposed for testing scenarios where it can be
  ...
  struct SetupStartupTracingOpts {
    ...
    // If set, this callback is executed (on an internal DejaView thread) when
    // startup tracing was set up.
    std::function<void(OnStartupTracingSetupCallbackArgs)> on_setup;

    // If set, this callback is executed (on an internal DejaView thread) if any
    // data sources were aborted, e.g. due to exceeding the timeout or as a
    // response to Abort().
    std::function<void()> on_aborted;

    // If set, this callback is executed (on an internal DejaView thread) after
...
class DEJAVIEW_EXPORT_COMPONENT TracingSession {
 public:
  ...
  // tracing has started. This callback will be invoked on an internal dejaview
  // thread.
  ...
  // callback will be invoked on an internal dejaview thread.
  ...
  // Args:
  //  callback: will be invoked on an internal dejaview thread when all data
  ...
  // This callback will be invoked on an internal dejaview thread.
  ...
  // on an internal dejaview thread.
...
class DEJAVIEW_EXPORT_COMPONENT StartupTracingSession {
 public:
...
DEJAVIEW_ALWAYS_INLINE inline std::unique_ptr<TracingSession> Tracing::NewTrace(
    BackendType backend) {
  ...
  // In case DEJAVIEW_IPC is disabled, a fake system backend is used, which
  // always panics. NewTrace(kSystemBackend) should fail if DEJAVIEW_IPC is
  // diabled, not panic.
...
#endif  // INCLUDE_DEJAVIEW_TRACING_TRACING_H_
