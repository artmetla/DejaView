...
// open-dejaview-trace is a standalone JS/TS library that can be used in other
// projects to facilitate the deep linking into dejaview. It allows opening
// trace files or URL with ui.perfetto.dev, handling all the handshake with it.

const DEJAVIEW_UI_URL = 'https://ui.perfetto.dev';
...
// Opens a trace in the DejaView UI.
// `source` can be either:
// - A blob (e.g. a File).
// - A URL.
export default function openDejaViewTrace(
  source: Blob | string,
...
function openTraceBlob(blob: Blob, opts?: OpenTraceOptions) {
  ...
  const uiUrl = opts?.uiUrl ?? DEJAVIEW_UI_URL;
...
function updateProgressDiv(progress: ProgressDivOpts, opts?: OpenTraceOptions) {
  ...
  const kDivId = 'open_dejaview_trace';
  ...
  if (!div) {
    ...
    title.className = 'dejaview-open-title';
    title.innerText = 'Opening dejaview trace';
    title.style.fontWeight = '12px';
    title.style.textAlign = 'center';
    div.appendChild(title);

    const progressbar = document.createElement('progress');
    progressbar.className = 'dejaview-open-progress';
  ...
  const title = div.querySelector('.dejaview-open-title') as HTMLElement;
  if (progress.status !== undefined) {
    title.innerText = progress.status;
  }
  const bar = div.querySelector('.dejaview-open-progress') as HTMLInputElement;
