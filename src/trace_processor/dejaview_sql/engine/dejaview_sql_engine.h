...
#ifndef SRC_TRACE_PROCESSOR_DEJAVIEW_SQL_ENGINE_DEJAVIEW_SQL_ENGINE_H_
#define SRC_TRACE_PROCESSOR_DEJAVIEW_SQL_ENGINE_DEJAVIEW_SQL_ENGINE_H_
...
class DejaViewSqlEngine {
 public:
  ...
  DejaViewSqlEngine(StringPool* pool, bool enable_extra_checks);
  ...
  uint64_t SqliteRegisteredObjectCount() {
    ...
    DEJAVIEW_CHECK(stmt.ok());
    uint32_t query_count =
        static_cast<uint32_t>(sqlite3_column_int(stmt->stmt.sqlite_stmt(), 0));
    DEJAVIEW_CHECK(!stmt->stmt.Step());
    DEJAVIEW_CHECK(stmt->stmt.status().ok());
 ...
 private:
  base::Status ExecuteCreateFunction(const DejaViewSqlParser::CreateFunction&);

  base::Status ExecuteInclude(const DejaViewSqlParser::Include&,
  ...
  base::Status ExecuteCreateView(const DejaViewSqlParser::CreateView&);

  base::Status ExecuteCreateMacro(const DejaViewSqlParser::CreateMacro&);

  base::Status ExecuteCreateIndex(const DejaViewSqlParser::CreateIndex&);

  base::Status ExecuteDropIndex(const DejaViewSqlParser::DropIndex&);

  enum class CreateTableType {
    kCreateTable,
    // For now, bytes columns are not supported in CREATE DEJAVIEW TABLE,
    // but supported in CREATE DEJAVIEW VIEW, so we skip them when validating
    // views.
    kValidateOnly
  };
  ...
  // Given that DejaViewSQL supports an arbitrary order of columns in the
...
namespace dejaview_sql_internal {
...
template <typename Function>
base::Status DejaViewSqlEngine::RegisterStaticFunction(
    const char* name,
    int argc,
    typename Function::Context* ctx,
    bool deterministic) {
  ...
  return engine_->RegisterFunction(
      name, argc, dejaview_sql_internal::WrapSqlFunction<Function>, ctx,
      nullptr, deterministic);
}

template <typename Function>
base::Status DejaViewSqlEngine::RegisterSqliteFunction(
    typename Function::UserDataContext* ctx,
    bool deterministic) {
  static_function_count_++;
...
template <typename Function>
base::Status DejaViewSqlEngine::RegisterSqliteAggregateFunction(
    typename Function::UserDataContext* ctx,
    bool deterministic) {
...
template <typename Function>
base::Status DejaViewSqlEngine::RegisterSqliteWindowFunction(
    const char* name,
    int argc,
    typename Function::Context* ctx,
    bool deterministic) {
...
template <typename Function>
base::Status DejaViewSqlEngine::RegisterFunctionWithSqlite(
    const char* name,
    int argc,
    std::unique_ptr<typename Function::Context> ctx,
    bool deterministic) {
  ...
  return engine_->RegisterFunction(
      name, argc, dejaview_sql_internal::WrapSqlFunction<Function>,
      ctx.release(), ctx_destructor, deterministic);
}

}  // namespace dejaview::trace_processor

#endif  // SRC_TRACE_PROCESSOR_DEJAVIEW_SQL_ENGINE_DEJAVIEW_SQL_ENGINE_H_
