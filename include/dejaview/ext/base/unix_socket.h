...
#ifndef INCLUDE_DEJAVIEW_EXT_BASE_UNIX_SOCKET_H_
#define INCLUDE_DEJAVIEW_EXT_BASE_UNIX_SOCKET_H_
...
// Returns the socket family from the full addres that dejaview uses.
// Addr can be:
...
//                             V
//                       OnDisconnect()
class DEJAVIEW_EXPORT_COMPONENT UnixSocket {
 public:
  class EventListener {
   public:
  ...
  void SetTxTimeout(uint32_t timeout_ms) {
    DEJAVIEW_CHECK(sock_raw_.SetTxTimeout(timeout_ms));
  }
  void SetRxTimeout(uint32_t timeout_ms) {
    DEJAVIEW_CHECK(sock_raw_.SetRxTimeout(timeout_ms));
  }
  ...
  uid_t peer_uid_posix(bool skip_check_for_testing = false) const {
    DEJAVIEW_DCHECK((!is_listening() && peer_uid_ != kInvalidUid) ||
  ...
  pid_t peer_pid_linux(bool skip_check_for_testing = false) const {
    DEJAVIEW_DCHECK((!is_listening() && peer_pid_ != kInvalidPid) ||
                    skip_check_for_testing);
    return peer_pid_;
  }
#endif
...
#endif  // INCLUDE_DEJAVIEW_EXT_BASE_UNIX_SOCKET_H_
