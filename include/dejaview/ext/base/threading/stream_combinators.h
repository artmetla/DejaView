...
#ifndef INCLUDE_DEJAVIEW_EXT_BASE_THREADING_STREAM_COMBINATORS_H_
#define INCLUDE_DEJAVIEW_EXT_BASE_THREADING_STREAM_COMBINATORS_H_
 ...
 public:
  ...
  StreamPollResult<T> PollNext(PollContext* upstream) override {
    for (uint32_t i = 0; i < streams_.size(); ++i) {
      ...
      if (res.IsPending()) {
        DEJAVIEW_CHECK(!registered_handles_[i].empty());
        continue;
      }
      if (!res.IsDone()) {
        return res;
...
#endif  // INCLUDE_DEJAVIEW_EXT_BASE_THREADING_STREAM_COMBINATORS_H_
