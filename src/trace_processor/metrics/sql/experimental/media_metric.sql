...
CREATE DEJAVIEW VIEW thread_slice AS
...
CREATE DEJAVIEW VIEW PlaybackStart AS
SELECT
...
CREATE DEJAVIEW VIEW VideoStart AS
SELECT
  playback_id,
  playback_start,
  PlaybackStart.upid,
...
CREATE DEJAVIEW VIEW AudioStart AS
SELECT
  playback_id,
  playback_start,
  PlaybackStart.upid,
...
CREATE DEJAVIEW VIEW DroppedFrameCount AS
SELECT
  playback_id,
  vs.upid,
  SUM(
...
CREATE DEJAVIEW VIEW SeekStart AS
SELECT
  playback_id,
  PlaybackStart.upid,
  s.ts AS seek_start,
...
CREATE DEJAVIEW VIEW SeekPartitioned AS
SELECT
  *,
...
CREATE DEJAVIEW VIEW PipelineSeek AS
SELECT
  seek.*,
  (
    SELECT MIN(s.ts)
...
CREATE DEJAVIEW VIEW SeekComplete AS
SELECT
  seek.*,
  (
    SELECT MIN(s.ts)
...
CREATE DEJAVIEW VIEW ValidSeek AS
SELECT
  s.*
FROM SeekComplete s
WHERE
...
CREATE DEJAVIEW VIEW AVStart AS
SELECT
  v.playback_id,
  v.playback_start,
  v.upid,
...
CREATE DEJAVIEW VIEW PlaybackEnd AS
SELECT
  AVStart.*,
...
CREATE DEJAVIEW VIEW VideoRoughness AS
SELECT
  playback_id,
  playback_start,
  PlaybackStart.upid,
...
CREATE DEJAVIEW VIEW VideoFreezing AS
SELECT
  playback_id,
  playback_start,
  PlaybackStart.upid,
...
CREATE DEJAVIEW VIEW media_metric_output AS
SELECT MediaMetric(
  'time_to_video_play', (
