...
CREATE DEJAVIEW MACRO _ii_df_agg(x Expr, y Expr)
RETURNS Expr AS __intrinsic_stringify!($x), input.$y;

CREATE DEJAVIEW MACRO _ii_df_bind(x Expr, y Expr)
RETURNS Expr AS __intrinsic_table_ptr_bind($x, __intrinsic_stringify!($y));

CREATE DEJAVIEW MACRO _ii_df_select(x Expr, y Expr)
RETURNS Expr AS $x AS $y;

CREATE DEJAVIEW MACRO __first_arg(x Expr, y Expr)
RETURNS Expr AS $x;

CREATE DEJAVIEW MACRO _interval_agg(
  tab TableOrSubquery,
...
CREATE DEJAVIEW MACRO _interval_intersect(
  tabs _TableNameList,
  agg_columns _ColumnNameList
)
RETURNS TableOrSubquery AS
...
CREATE DEJAVIEW MACRO _interval_intersect_single(
  ts Expr,
  dur Expr,
  t TableOrSubquery
)
